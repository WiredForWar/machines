...
{
    bool result;
    return result = ((inFile_ == nullptr) ? false : true);
}
...
GXError GXFile::getNextValidChar(char validChar, bool report)
{
    ...
    if (err == NO_ERROR_AGT)
    {
        if (currentLine_[linePos_] != validChar)
        {
            ...
            linePos_++; // position cursor after the char
    }
    return err;
}
...
GXError GXFile::readGXIdPos(GXIdPos& gxidpos, bool report, bool validate, void* pointer)
{
    ...
    if (err == NO_ERROR_AGT)
    {
        ...
        if (err == NO_ERROR_AGT)
        {
            gxidpos.gxid(strtoul(agidStr, nullptr, 16));
...
GXError GXFile::readMesh(GXMesh& mesh, const GXLabel& meshtype)
{
    ...
    err = readGXIdPos(agid, true, true, nullptr); // read AGID
        ...
        if (err == NO_ERROR_AGT)
        {
            ...
            if (err == NO_ERROR_AGT)
            {
                ...
                if (err == NO_ERROR_AGT)
                {
                    ...
                    {
                        polygon.flag(gxflag);
                        err = readGXIdPos(agid, true, false, nullptr); // read texture AGID
                        if (err == NO_ERROR_AGT)
                        {
        ...
        if (err == NO_ERROR_AGT)
        {
            while (err == NO_ERROR_AGT)
            {
                ...
                if (err == NO_ERROR_AGT)
                {
                    ...
                    {
                        reportAGTError(DUP_AGID_AGT, nullptr);
                    }
                    else
                    {
...
GXError GXFile::readHierarchy(GXHier& hierarchy)
{
    ...
    err = readGXIdPos(agid, true, true, nullptr);
    if (err == NO_ERROR_AGT)
    {
...
GXError GXFile::readChildInfo(GXHier& mesh, GXIdPos agidPos)
{
    ...
    if (err == NO_ERROR_AGT)
    {
        ...
        if (err == NO_ERROR_AGT)
        {
            ...
            if (err == NO_ERROR_AGT)
            {
                err = readGXIdPos(gxmeshId, false, false, nullptr); // read AGID (opt)
                if (gxmeshId == zeroId)
                {
...
GXError GXFile::readTransformFromMatrix4X4(GXHier& mesh)
{
    ...
    if (err == NO_ERROR_AGT)
    {
            ...
            || ((vector[3].z() != 0.0f) || (vector[3].h() != 1.0f)))
        {
            ...
            reportAGTError(MATRIX_ROW_AGT, nullptr);
        }
