...
std::ostream& operator<<(std::ostream& o, const GXMesh& t)
{
    ...
    if (t.hasTextures_)
    {
        ...
        for (GXTextures::const_iterator it = t.textures_.begin(); it != t.textures_.end(); ++it)
        {
...
GXError GXMesh::setTextures(GXTextures& textures)
{
    ...
    for (int polygonIndex = 0; polygonIndex < numPolygons(); ++polygonIndex)
    {
        ...
        if (txId != zeroId)
        {
            ...
            else
            {
                ...
                ASSERT(
                    (noNewTex || (textures[txId] == textures_[txId])),
    ...
    POST(((result == NO_ERROR_AGT) && hasTextures()) || ((result == NO_TEXTURE_AGT) && !hasTextures()));
