...
#include "gxin/Name.hpp"
#include "gxin/Matrix3.hpp"
...
public:
    ...
    const GXName& meshName() const { return meshName_; }
    ...
    long nChildrenSupplied() const { return children_.size(); }

    long nChildren() const { return nChildren_; }
    ...
    bool allChildrenSupplied() const { return nChildren() == nChildrenSupplied(); }

    GXHier& child(int i)
    {
        PRE(i < nChildrenSupplied());
        PRE(children_[i] != nullptr);
        return *(children_[i]);
    }
    const GXHier& child(int i) const { return *(children_[i]); }
