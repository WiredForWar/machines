...
bool operator==(const GXPolygon3& polygon1, const GXPolygon3& polygon2)
{
    ...
    if ((polygon1.vertices_ == polygon2.vertices_) && (polygon1.flag_ == polygon2.flag_)
        && (polygon1.flag2_ == polygon2.flag2_) && (polygon1.mat_ == polygon2.mat_))
        result = true;
...
void GXPolygon3::addVertex(const GXPolyVert3& newVertex)
{
    ...
    {
        ASSERT(
            iff(newVertex.hasColor(), allVerticesColored()),
            runtime_error("New vertex non consistent without previously recorded vertices"));
        vertices_.push_back(newVertex);
    }
