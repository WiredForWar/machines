...
void PhysLinearMotionPlan::computeInterpolatedTransform(const PhysRelativeTime& timeOffset, MexTransform3d* pResult)
    const
{
    CB_DEPIMPL(PhysLinearMotionPlanImpl::Entries, entries_);
    CB_DEPIMPL(MexPoint3d, translation_);
    ...
    MexPoint3d pos(translation_.x() * s, translation_.y() * s, translation_.z() * s);
