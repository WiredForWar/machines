...
#include "phys/ConfigSpace/Internal/DomainVertex.hpp"
...
public:
    ...
    using DomainMap = FtlSerialPMap<PhysCS2dDomain>;
    using DomainTree = PhysAlignedBSPTree2d<PhysCS2dDomain*>;

    using PortalId = PhysConfigSpace2d::PortalId;
    using PortalMap = FtlSerialPMap<PhysCS2dPortal>;

    using DomainVertexId = PhysConfigSpace2d::DomainVertexId;
    using DomainArcId = PhysConfigSpace2d::DomainArcId;

    using PolygonMap = FtlSerialPMap<PhysCS2dPolygon>;
    using PolygonTree = PhysAlignedBSPTree2d<PhysCS2dPolygon*>;

    using MotionChunkMap = FtlSerialPMap<PhysCS2dMotionChunk>;
    using MotionChunkTree = PhysAlignedBSPTree2d<PhysCS2dMotionChunk*>;

    using FindPathMap = FtlSerialPMap<PhysCS2dFindPath>;
    using FindPaths = ctl_pvector<PhysCS2dFindPath>;
    using DomainFindPathMap = FtlSerialPMap<PhysCS2dDomainFindPath>;
    using DomainFindPaths = ctl_pvector<PhysCS2dDomainFindPath>;
    ...
    // that vertex lies on.
    void addDomainArcs(const DomainVertexId& domainVertexId, const DomainId& domainId);

    // Add a vertex to the domain graph at point, returning the vertex id.
    DomainVertexId addDomainVertex(const MexPoint2d& point, MATHEX_SCALAR clearance, ObstacleFlags flags);
