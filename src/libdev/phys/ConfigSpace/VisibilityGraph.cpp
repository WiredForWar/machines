...
#ifndef _INLINE
#include "phys/ConfigSpace/Internal/VisibilityGraph.ipp"
#endif

#define FNTIME                                                                                                         \
    CS2VGRA_WHERE;                                                                                                     \
    bobTimer.currentTime()

class BobTimer
{
public:
    BobTimer() { entryTime_ = Phys::time(); }
    ~BobTimer()
    {
        exitTime_ = Phys::time();
...
PhysCS2dVisibilityGraph::PhysCS2dVisibilityGraph(
    ...
    PhysConfigSpace2d* pOriginalConfigSpace,
...
{
    INVARIANT(pConfigSpace_ != nullptr);
}
//////////////////////////////////////////////////////////////////////////////////////////
...
std::ostream& operator<<(std::ostream& o, const PhysCS2dVisibilityGraph& t)
{
    ...
    for (PhysCS2dVisibilityGraph::VertexId vertexId = t.vertexIdGenerator_.begin();
         vertexId != t.vertexIdGenerator_.end();
         ++vertexId)
    {
        if (t.graph_.containsVertex(vertexId))
        {
            const PhysCS2dVisibilityGraph::VertexData& vertex = t.graph_.vertex(vertexId);
            o << "(" << vertexId << ") " << *(vertex.pPoint_) << " from Polygon id " << vertex.polygonId_ << std::endl;
        }
    }

    o << "Arcs" << std::endl;
    for (PhysCS2dVisibilityGraph::ArcId arcId = t.arcIdGenerator_.begin(); arcId != t.arcIdGenerator_.end(); ++arcId)
    {
        if (t.graph_.containsArc(arcId))
        {
            o << "(" << arcId << ") from " << t.graph_.fromVertex(arcId) << " to " << t.graph_.toVertex(arcId)
              << std::endl;
        }
    }
        ...
        if (pConfigSpace_->exists(processingPolygonId_))
        {
            ...
            for (size_t i = 0; i != nVertices; ++i)
            {
                ...
                if (vertexOk)
                {
                    ...
                    // Add a graph vertex
                    graph_.addVertex(vertexId, VertexData(polygon.vertex(i), processingPolygonId_));
                }
                else
                {
...
bool PhysCS2dVisibilityGraph::processNewVertexWithOldVertex(ObstacleFlags flags)
{
    ...
    {
        const VertexData& processingVertexData = graph_.vertex(processingPolygonVertexIds_[processingPolygonVertex_]);
            ...
            if (graph_.containsVertex(processingGraphVertexId_))
            {
                    ...
                    && oldVertexData.polygonId_ != PolygonId::invalidId())
                {
                    ...
                    if (distance > eps)
                    {
                        ...
                        if (contains)
                        {
                            const MexPolygon2d& oldVertexPolygon = pConfigSpace_->polygon(oldVertexData.polygonId_);
                        ...
                        if (contains)
                        {
                            ...
                            if (pConfigSpace_->contains(processingPoint, oldPoint, flags, &intersectingId))
                            {
                                ...
                                graph_.addArc(
                                    processingPolygonVertexIds_[processingPolygonVertex_],
...
void PhysCS2dVisibilityGraph::endProcessingPolygon()
{
    ...
    processingPolygonVertexIds_.erase(processingPolygonVertexIds_.begin(), processingPolygonVertexIds_.end());
...
void PhysCS2dVisibilityGraph::setFindPath(const MexPoint2d& startPoint, const MexPoint2d& endPoint)
{
    ...
    if (addStartVertex)
    {
        ...
        // Add the vertex
        graph_.addVertex(startVertexId_, VertexData(endPoints_[startPointIndex_], PolygonId::invalidId()));
    ...
    if (addEndVertex)
    {
        ...
        // Add the vertex
        graph_.addVertex(endVertexId_, VertexData(endPoints_[1 - startPointIndex_], PolygonId::invalidId()));
    ...
    while (! doneOldVertex && processingGraphVertexId_ < vertexIdGenerator_.end())
    {
        if (graph_.containsVertex(processingGraphVertexId_))
        {
            ...
            if (! alreadyChecked && findPathVertexId != processingGraphVertexId_)
            {
                ...
                {
                    // JON_STREAM("  VGraph arc rejected " << processingPoint << " to " << oldPoint << " by polygon " <<
                    // intersectingId.asScalar() << std::endl;)
                }
            }
        }
...
bool PhysCS2dVisibilityGraph::updateFindPath(const PhysRelativeTime& maxTime, ObstacleFlags flags)
{
    PRE(findPathIsDefined_);
    ...
    if (upToDate_ && ! findPathCompleted_ && (Phys::time() - entryTime < maxTime))
    {
        ...
        do
        {
            if (pFindPathAlg_->update() == FindPathAlg::FINISHED)
                break;
...
PhysConfigSpace2d::Path PhysCS2dVisibilityGraph::findPath() const
{
    ...
    for (VertexIds::const_iterator it = findPathOutput_.begin(); it != findPathOutput_.end(); ++it)
    {
...
PhysCS2dVisibilityGraph::VertexData::VertexData(const MexPoint2d& point, const PolygonId& polygonId)
    : pPoint_(&point)
...
bool operator==(const PhysCS2dVisibilityGraph::ArcData&, const PhysCS2dVisibilityGraph::ArcData&)
{
    return true;
}
//////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////
// virtual
MATHEX_SCALAR PhysCS2dVisibilityGraph::FindPathAlg::estimatedCost(const VertexData& from, const VertexData& to) const
{
...
// virtual
bool PhysCS2dVisibilityGraph::FindPathAlg::vertexAvailable(const VertexData&) const
{
    return true;
}
