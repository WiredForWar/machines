...
public:
    ...
    using PolygonId = FtlSerialId; // A registered polygon
    using FindPathId = FtlSerialId; // A findPath algorithm run
    using DomainFindPathId = FtlSerialId; // A domainFindPath algorithm run
    using DomainId = FtlSerialId; // A registered domain
    using PortalId = FtlSerialId; // A registered portal
    using DomainVertexId = FtlSerialId; // A DomainVertex - a graph node on a portal
    using DomainArcId = FtlSerialId; // A DomainArc - an graph arc within a domain
    using ObjectId = FtlSerialId; // Client defined id
    using MotionChunkId = FtlSerialId; // A registered motion chunk
    ...
    // Useful collections
    using MexPolygon3ds = ctl_pvector<const MexPolygon3d*>;
    using Path = ctl_nb_vector<MexPoint2d>;
    using PolygonIds = ctl_nb_vector<PolygonId>;
    using PortalPoint = std::pair<PortalId, MATHEX_SCALAR>;
    using PortalPoints = ctl_nb_vector<PortalPoint>;
    ...
    DomainId addDomain(const MexAlignedBox2d& box, std::unique_ptr<MexPolygon2d>& boundaryAPtr);
    ...
    PortalId addPortal(const DomainId& id1, const DomainId& id2, const MexPoint2d& point1, const MexPoint2d& point2);
    // PRE( mode() == SUBTRACTIVE )
    ...
    PolygonId
    add(std::unique_ptr<MexPolygon2d>& polygonUPtr, MATHEX_SCALAR height, ObstacleFlags flags, Longevity type);
    ...
    using ObjectIds = ctl_nb_vector<ObjectId>;
    ...
    bool
    add(const PhysMotionChunk& motionChunk,
    ...
    CTL_PAIR(ChunkIntersectionDataNoTime, ObjectId, collisionObjectId, MotionChunkId, collisionChunkId);

    using ChunkIntersectionsNoTime = ctl_nb_vector<ChunkIntersectionDataNoTime>;
    ...
    public:
        ...
        ChunkIntersectionData(const ObjectId& id, const MotionChunkId& chunkId, const PhysAbsoluteTime& time)
            : collisionObjectId_(id)
            , collisionChunkId_(chunkId)
            , collisionTime_(time)
        {
        }
        ~ChunkIntersectionData() { }
    ...
    using ChunkIntersections = ctl_nb_vector<ChunkIntersectionData>;
    ...
    void findIntersections(const MexAlignedBox2d& box, ChunkIntersectionsNoTime* pChunkIntersections) const;
    ...
    using Intersections = ctl_nb_vector<IntersectionData>;
    ...
    void findIntersections(const PhysMotionChunk& motionChunk, const ObjectId& objectId, Intersections* pIntersections)
        const;
    ...
    DomainFindPathId addDomainFindPath(
        const MexPoint2d& startPoint,
        const MexPoint2d& endPoint,
        MATHEX_SCALAR clearance,
        ObstacleFlags flags,
...
private:
    ...
    void isMaintainingVisibilityGraph(
        bool doMaintain,
    ...
    //  or in an obstacle
    bool pointValid(const MexPoint2d& location, PhysCS2dDomain** ppCachedDomain) const;
