...
#ifndef _INLINE
#include "phys/ConfigSpace/ConfigSpace2d.ipp"
#endif
...
PhysConfigSpace2d::DomainId
PhysConfigSpace2d::addDomain(const MexAlignedBox2d& box, std::unique_ptr<MexPolygon2d>& boundaryAPtr)
{
    PRE(impl_.mode() == SUBTRACTIVE);
...
PhysConfigSpace2d::PortalId PhysConfigSpace2d::addPortal(
    const DomainId& id1,
    const DomainId& id2,
    const MexPoint2d& point1,
    const MexPoint2d& point2)
{
    PRE(mode() == SUBTRACTIVE);
...
bool PhysConfigSpace2d::contains(
...
{
    return impl_.pVisibilityGraph() != nullptr;
}
//////////////////////////////////////////////////////////////////////////////////////////
...
)
{
    ...
    for (PolygonId i = impl_.polygonIdGenerator().begin(); i != impl_.polygonIdGenerator().end(); ++i)
    {
        if (exists(i))
        {
    ...
    //  Add the domains
    for (DomainId i = impl_.domainIdGenerator().begin(); i != impl_.domainIdGenerator().end(); ++i)
    {
    ...
    for (PortalId i = impl_.portalIdGenerator().begin(); i != impl_.portalIdGenerator().end(); ++i)
    {
        if (portalExists(i))
        {
    ...
    PhysCS2dDomain* pCachedDomain = nullptr;
...
bool PhysConfigSpace2d::findSpace(
    const MexPoint2d& startPoint,
...
{
    PRE(pId != nullptr);
    PRE(pCollisionObjectId != nullptr);
    PRE(pCollideTime != nullptr);
...
const PhysConfigSpace2d::ObjectId& PhysConfigSpace2d::motionChunkObjectId(const MotionChunkId& id) const
{
    PRE(motionChunkExists(id));
...
PhysConfigSpace2d::MotionChunkId PhysConfigSpace2d::add(const PhysMotionChunk& motionChunk, const ObjectId& objectId)
{
    ...
    MexPoint2d* pResult)
{
    ...
    do
    {
        offset.setVector(mexRandomScalar(&r, -1.0, 10.0), mexRandomScalar(&r, -1.0, 10.0));
...
PhysCS2dDomainFindPath* PhysConfigSpace2d::pCurrentDomainFindPath()
{
    ...
    PhysCS2dDomainFindPath* result = nullptr;
...
PhysCS2dFindPath* PhysConfigSpace2d::pCurrentFindPath()
{
    ...
    PhysCS2dFindPath* result = nullptr;
...
void PhysConfigSpace2d::removeCurrentFindPathFromQueue()
{
    ...
    PhysCS2dImpl::FindPaths::iterator it = find(findPathQueue.begin(), findPathQueue.end(), impl_.pCurrentFindPath_);
...
void PhysConfigSpace2d::findIntersections(const MexAlignedBox2d& box, ChunkIntersectionsNoTime* pIntersections) const
{
    PRE(pIntersections != nullptr);
    ...
    for (size_t i = 0; i != n; ++i)
    {
        ...
        const MexSausage2d sausage2d(motionChunk.startPoint(), motionChunk.endPoint(), motionChunk.clearance());
...
bool PhysConfigSpace2d::intersectsWithMotionChunk(const MexAlignedBox2d& box) const
{
    // Get a vector of all the chunks in
    // the tree which intersect the box
...
{
    PRE(pChunkIntersections != nullptr);
...
void PhysConfigSpace2d::traceIllegalPoints(std::ostream& o) const
{
    ...
    const MATHEX_SCALAR xStart = impl_.boundary().minCorner().x() + mexRandomScalar(&r, 0.0, xIncr);
    const MATHEX_SCALAR xEnd = impl_.boundary().maxCorner().x();
    const MATHEX_SCALAR yIncr = 1.0;
    const MATHEX_SCALAR yStart = impl_.boundary().minCorner().y() + mexRandomScalar(&r, 0.0, yIncr);
    const MATHEX_SCALAR yEnd = impl_.boundary().maxCorner().y();
...
bool PhysConfigSpace2d::pointValid(const MexPoint2d& location, PhysCS2dDomain** ppCachedDomain) const
{
    bool result = false;

    //  Caching gives us a speed increase because of spatial coherence
    if (*ppCachedDomain != nullptr)
    {
...
void PhysConfigSpace2d::adjustLimits(
    MATHEX_SCALAR* pXStart,
    MATHEX_SCALAR* pXEnd,
    MATHEX_SCALAR* pYStart,
    MATHEX_SCALAR* pYEnd,
    uint* pNColumns)
{
    ...
    PhysAbsoluteTime* pCollideTime)
{
    ...
    return add(motionChunk, objectId, ignoreIds, pId, pCollisionObjectId, pCollideTime);
}
