...
#ifndef _INLINE
#include "phys/ConfigSpace/Internal/FindPath.ipp"
#endif
...
bool PhysCS2dFindPath::spaceIntersections(
    const MexPoint2d& p1,
    const MexPoint2d& p2,
    MATHEX_SCALAR clearance,
    PolygonIds* pPolygonIds) const
{
    CS2VGRA_STREAM(*this << std::endl);

    PRE(clearance > 0);
    PRE(pPolygonIds != nullptr);
...
void PhysCS2dFindPath::start()
{
    ...
    // JON_STREAM("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl << "Starting FindPath from " <<
    // startPoint_ << " to " << endPoint_ << std::endl );
