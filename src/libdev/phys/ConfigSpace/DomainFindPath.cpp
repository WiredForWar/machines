...
#include "phys/ConfigSpace/Internal/DomainFindPathCache.hpp"

#ifndef _INLINE
#include "phys/ConfigSpace/Internal/DomainFindPath.ipp"
#endif
...
void PhysCS2dDomainFindPath::startMacroSearch()
{
    ...
    if (PhysCS2dDomainFindPathCache::instance()
            .findPath(startDomainId_, startPoint_, endDomainId_, endPoint_, clearance_, flags(), &domainPath_))
    {
        state_ = FINISHED;
        foundPath_ = true;
    }
...
void PhysCS2dDomainFindPath::updateMacroSearch(const PhysRelativeTime& maxTime)
{
    ...
    else
    {
        do
        {
            if (pDomainAlg_->update() == DomainAStarAlg::FINISHED)
                break;
...
void PhysCS2dDomainFindPath::endMacroSearch()
{
    ...
    if (pDomainAlg_->isFinished() && domainGraphVersion_ == impl.domainGraph().version())
    {
        ...
        for (DomainAStarAlg::Vertices::iterator it = domainVertexIds.begin(); it != domainVertexIds.end(); ++it)
        {
        ...
        PhysCS2dDomainFindPathCache::instance()
            .addPath(startDomainId_, startPoint_, endDomainId_, endPoint_, clearance_, flags(), domainPath_);
    }
    ...
    do
    {
        switch (state_)
        {
            ...
            case MACRO:
                {
                    // Doing macro search
                    if (isMacroSearchFinished())
                        endMacroSearch();
                    else
                        updateMacroSearch(timeLeft);
                    break;
                }
...
MATHEX_SCALAR PhysCS2dDomainFindPath::DomainAStarAlg::estimatedCost(
    const PhysCS2dDomainVertex& from,
    const PhysCS2dDomainVertex& to) const
{
    ...
    A_STAR_STREAM("Estimated cost from " << from.point() << " to " << to.point() << " is " << result << std::endl);
...
// virtual
bool PhysCS2dDomainFindPath::DomainAStarAlg::vertexAvailable(const PhysCS2dDomainVertex& vertex) const
{
