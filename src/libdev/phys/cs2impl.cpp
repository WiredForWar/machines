...
PhysCS2dImpl::PhysCS2dImpl(
    ...
    , pActiveDomainFindPath_(nullptr)
    , pActiveFindPath_(nullptr)
    , domainClearance_(domainClearance)
    , domainTree_(boundary.minCorner(), boundary.maxCorner(), domainMinResolution, nDomainMaxItems, domainLipSize)
    , permanentPolygonTree_(
          boundary.minCorner(),
          boundary.maxCorner(),
          polygonMinResolution,
          nPolygonMaxItems,
          polygonLipSize)
    , temporaryPolygonTree_(
          boundary.minCorner(),
          boundary.maxCorner(),
          polygonMinResolution,
          nPolygonMaxItems,
    ...
    , pCurrentDomainFindPath_(nullptr)
    , pCurrentFindPath_(nullptr)
...
PhysCS2dImpl::~PhysCS2dImpl()
{
    ...
    for (PhysConfigSpace2d::MotionChunkId id = motionChunkIdGenerator_.begin(); id != motionChunkIdGenerator_.end();
         ++id)
    {
...
std::ostream& operator<<(std::ostream& o, const PhysCS2dImpl& t)
{
    ...
    // The domains
    for (PhysConfigSpace2d::DomainId id = t.domainIdGenerator_.begin(); id != t.domainIdGenerator_.end(); ++id)
    {
    ...
    // The portals
    for (PhysConfigSpace2d::PortalId id = t.portalIdGenerator_.begin(); id != t.portalIdGenerator_.end(); ++id)
    {
    ...
    // The polygons
    for (PhysConfigSpace2d::PolygonId id = t.polygonIdGenerator_.begin(); id != t.polygonIdGenerator_.end(); ++id)
    {
...
void PhysCS2dImpl::trace(std::ostream& o, const PhysCS2dDomainGraph& domainGraph) const
{
         ...
         ++id)
    {
        if (domainGraph.containsArc(id))
        {
            ...
            indentOstr << "DomainArc id " << id.asScalar() << " Vertexs " << domainGraph.fromVertex(id).asScalar()
                       << " " << domainGraph.toVertex(id).asScalar() << " Domain " << domainArc.domainId().asScalar()
                       << std::endl;
...
bool PhysCS2dImpl::contains(
    const MexPoint2d& testPoint,
...
void PhysCS2dImpl::addPortalDomainVertexs(const PortalId& portalId)
{
    ...
    else
    {
        ...
        {
            calculatePortalPoints(portalId, checkAtEnd1, checkAtEnd2, &foundAtEnd1, &foundAtEnd2, flags, polygons);
        }
    }
}
...
void PhysCS2dImpl::calculatePortalPoints(
    const PortalId& portalId,
    CheckAtEnd checkEnd1,
    CheckAtEnd checkEnd2,
    bool* pFoundAtEnd1,
    bool* pFoundAtEnd2,
    ObstacleFlags flags,
    ...
    *pFoundAtEnd2 = false;
    ...
    if (addPoint1 && addPoint2)
    {
        ...
        if (fabs(distance1 - distance2) <= close)
        {
            ...
            const MATHEX_SCALAR startDistance
                = (distance1 + distance2) / 2.0 - maxPortalClearance_ - MexEpsilon::instance();
            const MATHEX_SCALAR endDistance
                = (distance1 + distance2) / 2.0 + maxPortalClearance_ + MexEpsilon::instance();
...
bool PhysCS2dImpl::calculatePortalPoint(
    const MexPoint2d& end1,
    const MexPoint2d& end2,
    MATHEX_SCALAR startDistance,
    MATHEX_SCALAR endDistance,
    MATHEX_SCALAR* pPointDistance,
    MATHEX_SCALAR* pClearance,
    bool* pFoundAtEnd,
    ObstacleFlags flags,
    const PolygonTree::PItems& polygons)
{
...
void PhysCS2dImpl::addDomainVertex(
    const PortalId& portalId,
    PhysCS2dPortal* pPortal,
    MATHEX_SCALAR distance,
    MATHEX_SCALAR clearance,
    ObstacleFlags flags)
{
...
PhysConfigSpace2d::DomainVertexId
PhysCS2dImpl::addDomainVertex(const MexPoint2d& point, MATHEX_SCALAR clearance, ObstacleFlags flags)
{
    // Generate the id
    DomainVertexId domainVertexId = domainVertexIdGenerator().next();
...
void PhysCS2dImpl::addDomainArcs(const DomainVertexId& inVertexId, const DomainId& domainId)
{
    ...
    for (size_t i = 0; i != nPortals; ++i)
    {
        ...
        if (domainPortalId != inVertexPortalId)
        {
            ...
            for (size_t j = 0; j != nPortalDomainVertexs; ++j)
            {
                ...
                domainGraph_.addArc(inVertexId, domainVertexId, arcId, PhysCS2dDomainArc(domainId));
...
void PhysCS2dImpl::createVisibilityGraph(
    PhysConfigSpace2d* pOriginalSpace,
    PhysCS2dExpansionSpace* pExpansionSpace,
    MATHEX_SCALAR clearance)
{
...
bool PhysCS2dImpl::expansionSpacePolygonExists(MATHEX_SCALAR expansionDistance, PolygonId id) const
{
    PRE(expansionSpaceIsOpen(expansionDistance));
    PRE(polygons_.contains(id));
...
void PhysCS2dImpl::updateDomainRegion()
{
    ...
    if (domainRegionOutOfDate_)
    {
        ...
        domainUpdateRegion_.corners(
            MexPoint2d(minCorner.x() - domainClearance_, minCorner.y() - domainClearance_),
            MexPoint2d(maxCorner.x() + domainClearance_, maxCorner.y() + domainClearance_));
        ...
        for (PhysConfigSpace2d::PortalId id = portalIdGenerator_.begin(); id != portalIdGenerator_.end(); ++id)
        {
            if (portals_.contains(id))
            {
                ...
                if (portalBoundary.intersects(domainUpdateRegion_))
                {
                    ...
                    {
                        pPortal->removeDomainVertex(pPortal->domainVertexId(0), &domainGraph_);
                    }
                }
            }
        }
...
void PhysCS2dImpl::listArcs(std::ostream& ostr) const
{
         ...
         ++arcId)
    {
        if (domainGraph().containsArc(arcId))
        {
            ...
            ostr << "Arc " << arcId << " domain " << domainGraph().arc(arcId).domainId() << " from ( " << vertex1
                 << " ) to ( " << vertex2 << " )" << std::endl;
        }
    }
}
...
bool PhysCS2dImpl::intersectsAnyExpanded(
    const MexSausage2d& sausage,
    const MexAlignedBox2d& sausageBoundary,
    const PolygonTree& polygonTree,
    Mathex::TouchingRule rule,
