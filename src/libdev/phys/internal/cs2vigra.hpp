...
private:
    ...
    using VertexId = FtlSerialId;
    using ArcId = FtlSerialId;

    using GraphVertex = GraVertex<VertexData, ArcId>;
    using VertexMap = FtlSerialMap<GraphVertex>;

    using GraphArc = GraArc<ArcData, VertexId, MATHEX_SCALAR>;
    using ArcMap = FtlSerialMap<GraphArc>;

    using Graph = GraGraph<VertexMap, ArcMap>;

    // Stuff for the A* algorithm on the visibility graph
    using AStarVertex = GraAStarVertex<Graph>;
    using AStarVertexMap = FtlSerialMap<AStarVertex>;

    class FindPathAlg : public GraAStarAlg<Graph, AStarVertexMap>
    {
        MATHEX_SCALAR estimatedCost(const VertexData& from, const VertexData& to) const override;
        bool vertexAvailable(const VertexData& vertex) const override;
    };
    ...
    bool originalSpaceContainsSausage(const MexPoint2d& startPoint, const MexPoint2d& endPoint, ObstacleFlags flags);
