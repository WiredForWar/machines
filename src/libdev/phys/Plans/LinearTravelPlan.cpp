...
#include "phys/Plans/Internal/LinearTravelPlanImpl.hpp"
    ...
    , pImpl_(new PhysLinearTravelPlanImpl(transformsPtr))
{
    ...
    for (size_t i = 0; i < nSegments; ++i)
    {
        ...
        else
        {
            ...
            const MATHEX_SCALAR angle
                = (transforms[i].xBasis().signedAngleBetween(transforms[i + 1].xBasis())).asScalar();
...
void PhysLinearTravelPlan::computeCacheData()
{
    ...
    cacheSegmentTime_
        = (cacheSegment_ == 0 ? segmentTimes[0] : segmentTimes[cacheSegment_] - segmentTimes[cacheSegment_ - 1]);
...
void PhysLinearTravelPlan::computeInterpolatedTransform(const PhysRelativeTime& timeOffset, MexTransform3d* pResult)
    const
{
    CB_DEPIMPL(TimesPtr, segmentTimesPtr_);
    CB_DEPIMPL(bool, cacheValid_);
    ...
    {
        s = PhysRampAcceleration::proportionOfDistance(cacheSegmentTime_, cacheAccelerationTime_, segmentTimeOffset);
    }
    else
    {
        ...
        s = PhysRampAcceleration::proportionOfDistance(
            cacheStartSpeed_,
            translationAcceleration,
            translationDeceleration,
            cacheSegmentTime_,
    ...
    MexPoint3d pos(cacheTranslation_.x() * s, cacheTranslation_.y() * s, cacheTranslation_.z() * s);
