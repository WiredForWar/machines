...
#ifndef _INLINE
#include "phys/Plans/MotionChunk.ipp"
#endif
...
std::ostream& operator<<(std::ostream& o, const PhysMotionChunk& t)
{
    ...
    o << "  accelerations " << t.motionProfile_.firstAcceleration() << "  " << t.motionProfile_.secondAcceleration();
    o << "  times " << t.motionProfile_.totalTime() << " " << t.motionProfile_.firstAccelerationTime() << " "
...
void PhysMotionChunk::boundary(MexAlignedBox2d* pBox) const
{
    ...
    pBox->corners(MexPoint2d(xMin - clearance_, yMin - clearance_), MexPoint2d(xMax + clearance_, yMax + clearance_));
}

bool PhysMotionChunk::intersects(const PhysMotionChunk& rhs, PhysAbsoluteTime* pCollideTime) const
{
    ...
    while (result)
    {
        ...
        if (lhsHasAccn2)
        {
            MATHEX_SCALAR f = motionProfile_.proportionOfDistance(lhsMotionDuration - lhsAccn2Duration);
        ...
        if (rhsHasAccn2)
        {
            MATHEX_SCALAR f = rhs.motionProfile_.proportionOfDistance(rhsMotionDuration - rhsAccn2Duration);
        ...
        if (rhsHasAccn1)
            rhsAccn1Sausage = MexSausage2d(rhs.startPoint_, rhsConstantSpeedStartPoint, rhs.clearance_);

        // Construct constant speed sausages.
        MexSausage2d lhsConstantSpeedSausage(lhsFullSausage);
        if (lhsHasConstantSpeed)
            lhsConstantSpeedSausage = MexSausage2d(lhsConstantSpeedStartPoint, lhsConstantSpeedEndPoint, clearance_);
    ...
    PhysAbsoluteTime* pCollideTime)
{
    ...
    PhysAbsoluteTime overlapEndTime = (lhsHasEndTime ? std::min(lhsEndTime, rhsEndTime) : rhsEndTime);
    ...
    PhysAbsoluteTime* pCollideTime)
{
    ...
    while (result)
    {
        ...
        MexVec2 a(e1 * p1xf1 - s1 * q1xf1 - e2 * p2xf2 + s2 * q2xf2, e1 * p1yf1 - s1 * q1yf1 - e2 * p2yf2 + s2 * q2yf2);
        MexVec2 b(q1xf1 - p1xf1 - q2xf2 + p2xf2, q1yf1 - p1yf1 - q2yf2 + p2yf2);
