...
#ifndef _INLINE
#include "phys/Plans/RampAcceleration.ipp"
#endif
...
PhysRelativeTime PhysRampAcceleration::totalTime(
    ...
    MATHEX_SCALAR startVelocity,
    MATHEX_SCALAR terminalSpeed,
    MATHEX_SCALAR endVelocity,
    MATHEX_SCALAR acceleration,
    MATHEX_SCALAR deceleration,
    PhysRelativeTime* pAccelerationTime,
    PhysRelativeTime* pDecelerationTime)
{
    PRE_INFO(distance);
    ...
    // normal scenario
    PhysRelativeTime accelerationTime = (acceleration == 0.0 ? 0.0 : (terminalSpeed - startVelocity) / acceleration);
    PhysRelativeTime decelerationTime = (deceleration == 0.0 ? 0.0 : (terminalSpeed - endVelocity) / deceleration);
    ...
    else
    {
        ...
        if (minDistance <= distance)
        {
            ...
            root = (2 * acceleration * (-deceleration) * distance + (-deceleration) * startVelocity * startVelocity
            ...
            decelerationTime = (deceleration == 0.0 ? 0.0 : (root - endVelocity) / deceleration);
    ...
    *pDecelerationTime = decelerationTime;
    ...
    POST_DATA(
        ...
        = startVelocity * *pAccelerationTime + acceleration * *pAccelerationTime * *pAccelerationTime / 2.0);
    ...
    POST_DATA(
        ...
        = constantVelocity * *pDecelerationTime - deceleration * *pDecelerationTime * *pDecelerationTime / 2.0);
    ...
    const PhysRelativeTime& timeOffset)
{
    ...
    else
    {
        ...
        MATHEX_SCALAR distance = 0.5
                * ((startSpeed - terminalSpeed) * firstAccelerationTime
        ...
        {
            timeDistance = 0.5 * (startSpeed - terminalSpeed) * firstAccelerationTime + terminalSpeed * timeOffset;
        }
        else
        {
            MATHEX_SCALAR timeToGo = totalTime - timeOffset;
            timeDistance = distance - (endSpeed - 0.5 * secondAcceleration * timeToGo) * timeToGo;
        }
    ...
    const PhysRelativeTime& timeOffset)
{
    ...
    else
    {
        ...
        {
            distance = 0.5 * (startSpeed - terminalSpeed) * firstAccelerationTime + terminalSpeed * timeOffset;
        }
        else
        {
    ...
    , secondAccelerationTime_(secondAccelerationTime)
{
    ...
    firstAcceleration_
        = 2.0 * (distance1 - startSpeed * firstAccelerationTime) / (firstAccelerationTime * firstAccelerationTime);
    ...
    secondAcceleration_ = 2.0 * (distance3 - constVelocity * secondAccelerationTime)
        / (secondAccelerationTime * secondAccelerationTime);
    ...
    MATHEX_SCALAR decelerationRate)
{
    ...
    MATHEX_SCALAR distance1 = distance(0.0, startSpeed, firstAcceleration_, firstAccelerationTime_);
        ...
        || desiredSpeed < MexEpsilon::instance())
    {
        ...
        distance1 = distance(0.0, startSpeed, firstAcceleration_, firstAccelerationTime_);
        ...
        if (fabs(distance1) <= fabs(desiredDistance))
        {
            ...
            if (endSpeed == 0)
            {
                totalTime_ = totalTime(
                    ...
                    accelerationRate,
                    decelerationRate,
                    &firstAccelerationTime_,
                    &secondAccelerationTime_);
    ...
    MATHEX_SCALAR maxDeceleration)
{
    ...
    if (finished)
    {
        if (firstAccelerationTime_ + secondAccelerationTime_ > time)
        {
            ...
            const MATHEX_SCALAR proportion
                = firstAccelerationTime_ / (firstAccelerationTime_ + secondAccelerationTime_);
...
MATHEX_SCALAR PhysRampAcceleration::speed(const PhysRelativeTime& timeOffset) const
{
    ...
    else
    {
        ...
        PhysRelativeTime a2Time = timeOffset - (totalTime_ - firstAccelerationTime_ - secondAccelerationTime_);

        if (a2Time > 0.0)
        {
            result += secondAcceleration_ * (a2Time >= secondAccelerationTime_ ? secondAccelerationTime_ : a2Time);
        }
    }
    ...
    MATHEX_SCALAR* pT3) const
{
    ...
    if (canSolve && (*pT1 < 0.0 || *pT3 < 0.0))
    {
        ...
        const MATHEX_SCALAR distance
            = u1 * *pT1 + a1 * *pT1 * *pT1 / 2.0 + u2 * t2 + u3 * *pT3 + a3 * *pT3 * *pT3 / 2.0;
