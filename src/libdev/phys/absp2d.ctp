...
PhysAlignedBSPTree2d<P_ITEM>::PhysAlignedBSPTree2d(
    ...
    const MexPoint2d& maxCorner,
    MATHEX_SCALAR minResolution,
    size_t nMaxItems,
...
template <class P_ITEM> PhysAlignedBSPTree2d<P_ITEM>::~PhysAlignedBSPTree2d()
{
    TEST_INVARIANT;
...
template <class P_ITEM> void PhysAlignedBSPTree2d<P_ITEM>::CLASS_INVARIANT
{
    INVARIANT(this != nullptr);
}
//////////////////////////////////////////////////////////////////////////////////////////

template <class P_ITEM> void PhysAlignedBSPTree2d<P_ITEM>::add(P_ITEM pItem)
{
    PRE(pItem != NULL);
...
template <class P_ITEM>
bool PhysAlignedBSPTree2d<P_ITEM>::split(Partition* pPartition, const MexAlignedBox2d& partitionBoundary)
{
    bool wasSplit;
...
template <class P_ITEM> void PhysAlignedBSPTree2d<P_ITEM>::remove(P_ITEM pItem)
{
    PRE(pItem != NULL);
    ...
    const MexAlignedBox2d& partitionBoundary)
{
    ...
    // If subdivided check to see if the new item belongs in either
    // the left or right partition
    bool done = pPartition->pLeft_ != nullptr;
    if (done)
    {
        // Generate the left and right boundaries
        MexAlignedBox2d leftBoundary;
...
template <class P_ITEM>
size_t PhysAlignedBSPTree2d<P_ITEM>::overlapping(const MexAlignedBox2d& boundary, PItems* pItems) const
{
    PRE(pItems != nullptr);
...
size_t PhysAlignedBSPTree2d<P_ITEM>::overlapping(
    ...
    const MexAlignedBox2d& partitionBoundary) const
{
    PRE(pItems != nullptr);
...
template <class P_ITEM> std::ostream& operator<<(std::ostream& o, const PhysAlignedBSPTree2d<P_ITEM>& t)
{
    ...
    o << "Resolution " << t.minResolution_ << "  MaxItems " << t.nMaxItems_ << "  Current count " << t.nItems_
      << std::endl;
