...
template <class T, class UNARY_PREDICATE> bool ctl_erase_first_instance_if(ctl_vector<T>* pC, UNARY_PREDICATE p)
// returns true iff an instance of key was erased
{
    PRE(pC != NULL);
    POST_DATA(typename ctl_vector<T>::size_type old_size = pC->size());
...
template <class T> void ctl_erase_all_instances(ctl_vector<T>* pC, const T& key)
...
template <class T, class UNARY_PREDICATE> void ctl_erase_all_instances_if(ctl_vector<T>* pC, UNARY_PREDICATE p)
...
template <class T, class UNARY_PREDICATE> bool ctl_erase_first_instance_if(ctl_list<T>* pC, UNARY_PREDICATE p)
// returns true iff an instance of key was erased
{
    PRE(pC != NULL);
    POST_DATA(typename ctl_list<T>::size_type old_size = pC->size());
...
template <class T> void ctl_erase_all_instances(ctl_list<T>* pC, const T& key)
// erase all instances of key
{
    PRE(pC != NULL);
    POST_DATA(typename ctl_list<T>::size_type old_size = pC->size());
...
template <class T, class UNARY_PREDICATE> void ctl_erase_all_instances_if(ctl_list<T>* pC, UNARY_PREDICATE p)
// erase all members of pC which satisfy p
{
    PRE(pC != NULL);
    POST_DATA(typename ctl_list<T>::size_type old_size = pC->size());
...
template <class T> bool ctl_erase(ctl_set<T, std::less<T>>* pC, const T& key)
// returns true iff an instance of key was erased
{
    PRE(pC != NULL);
...
template <class KEY, class VALUE> bool ctl_erase(ctl_map<KEY, VALUE, std::less<KEY>>* pC, const KEY& key)
// returns true iff an instance of key was erased
{
    PRE(pC != NULL);
    POST_DATA(size_t old_size = pC->size());
    POST_DATA(
        size_t old_nvalues
        = ctl_count_if(*pC, compose1(bind1st(std::equal_to<int>(), key), ctl_select1st<const int, int>())));
