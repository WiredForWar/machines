...
std::ostream& operator<<(std::ostream& o, const MachPhysTreacheryWeapon& t)
{

    o << "MachPhysTreacheryWeapon " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysTreacheryWeapon " << static_cast<const void*>(&t) << " end" << std::endl;
...
PhysRelativeTime MachPhysTreacheryWeapon::traitorAnimation(
    ...
    MachPhys::Race oldRace,
    MachPhys::Race newRace)
{
    ...
    const W4dCompositeMaterialVecChanger& oldRaceChanger = theRaceChanger.changer(*pMachine, oldRace);
    const W4dCompositeMaterialVecChanger& newRaceChanger = theRaceChanger.changer(*pMachine, newRace);
    ...
    for (W4dComposite::W4dLinks::const_iterator it = links.begin(); it != links.end(); ++it)
    {
        ...
        if (nLinkLODs != 0 && oldRaceChanger.hasLinkChanger(linkId) && newRaceChanger.hasLinkChanger(linkId))
        {
            ...
            const W4dEntityMaterialVecChanger& newEntityChanger = newRaceChanger.linkIdChanger(linkId);
