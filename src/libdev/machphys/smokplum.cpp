    ...
    const PhysRelativeTime& duration)
{
    ...
    PhysLinearMotionPlan* pMotionPlan = generateMotionPlan(riseTime, maxHeight);
...
PhysLinearMotionPlan* MachPhysSmokePlume::generateMotionPlan(const PhysRelativeTime& riseTime, MATHEX_SCALAR maxHeight)
{
    //  Generate a random motion plan to use for the rising smoke
    MexEulerAngles orientation(MachPhysRandom::randomDouble(0.0, 2.0 * Mathex::PI), 0.0, 0.0);
...
std::ostream& operator<<(std::ostream& o, const MachPhysSmokePlume& t)
{

    o << "MachPhysSmokePlume " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysSmokePlume " << static_cast<const void*>(&t) << " end" << std::endl;
