    ...
    for (size_t i = 0; i < transformsPtr->size() - 1; ++i)
    {
        ...
        if (distance < MexEpsilon::instance())
        {
            ...
            PhysRampAcceleration rampAcceleration(
                turnAngle.asScalar(),
                startSpeed,
                rotationSpeed.asScalar(),
                endSpeed,
                rotationAcceleration.asScalar(),
                rotationAcceleration.asScalar(),
...
void MachPhysLocomotionMethod::forceUpright(TransformsPtr transformsPtr)
{
...
// static
void MachPhysLocomotionMethod::pitchOrRollAnimation(
    W4dLink* pEntity,
...
// static
MexTransform3d
MachPhysLocomotionMethod::transform(const MexRadians& angle, const MexVec3& axis, const MexPoint3d& position)
{
