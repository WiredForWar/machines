...
// Global statics
MachPhysTileData::WorkingData* MachPhysTileData::pWorkingData_ = nullptr;
...
MachPhysTileData::MachPhysTileData(int nXVertices, int nYVertices, const MexAlignedBox2d& boundary, const RenMesh& mesh)
...
int MachPhysTileData::cellsUnderLine(const MexPoint3d& p1, const MexPoint3d& p2, cellCallBackFn* pFn)
{
...
void MachPhysTileData::lineCellRange(
    int nXCells,
    MATHEX_SCALAR xCellSize,
    int nYCells,
    MATHEX_SCALAR yCellSize,
...
{
    // JON_STREAM("MachPhysTileData::lineIntersectsCell entry (" << xCell << "," << yCell << ")" << (void*)(&tileData)
    // << std::endl;)
    ...
    while (nTriangles--)
    {
            ...
            || find(trianglesDone_.begin(), trianglesDone_.end(), id) == trianglesDone_.end())
        {
            ...
            foundIntersection_ = testLine_.segmentIntersects(
                vertices[t.v1()],
                vertices[t.v2()],
                vertices[t.v3()],
                &distance_,
                MexLine3d::INTERSECT_FRONT_SIDE);

            // JON_STREAM("  Done triangle " << id << vertices[ t.v1() ] << vertices[ t.v2() ] << vertices[ t.v3() ] <<
            // std::endl;)
            if (foundIntersection_)
                break;
        }
...
std::ostream& operator<<(std::ostream& o, const MachPhysTileData& t)
{
    ...
    for (size_t i = 0; i != t.triangles_.size(); ++i)
        o << "  " << i << "  " << t.triangles_[i].v1() << " " << t.triangles_[i].v2() << " " << t.triangles_[i].v3()
          << std::endl;
...
/*
void MachPhysTileData::pathProfile( const MexPoint3d& startPoint, const MexPoint3d& finishPoint, Profile* pProfile )
const
{
    PRE_INFO( startPoint );
    ...
    PATH_PROFILE_STREAM( "Enter MachPhysTileData::pathProfile Start find path profile from " << startPoint << " to " <<
finishPoint << std::endl ); PATH_PROFILE_INDENT( 2 );
    ...
    if( intersectingTriangles.size() != 0 )
    {
        ...
        for( IntersectingTriangles::const_iterator i = intersectingTriangles.begin(); i != intersectingTriangles.end();
++i )
        {
            PATH_PROFILE_WHERE;
    ...
    PATH_PROFILE_STREAM( "Exit  MachPhysTileData::pathProfile Start find path profile from " << startPoint << " to " <<
finishPoint << std::endl ); PATH_PROFILE_STREAM( "......................................................" << std::endl
);
}
*/
...
bool MachPhysTileData::intersectWithTriangle(
    const MexLine2d& line,
    const Triangle& triangle,
    MexPoint3d* pPoint1,
    MexPoint3d* pPoint2) const
{
    ...
    PATH_PROFILE_STREAM(
        "Triangle : " << vertices_[triangle.v1()] << " " << vertices_[triangle.v2()] << " " << vertices_[triangle.v3()]
                      << std::endl);
    ...
    uint nIntersections = MexIntersections::lineVsTriangle2d(startPoint, direction, v1, v2, v3, &distance1, &distance2);
...
bool MachPhysTileData::withinTriangle(const MexPoint2d& point, const Triangle& triangle, MexPoint3d* pIntersectionPoint)
    const
{
    const MexPoint3d& vertex1 = vertices_[triangle.v1()];
    const MexPoint3d& vertex2 = vertices_[triangle.v2()];
    ...
    MATHEX_SCALAR distance;
    bool within
        = testLine.intersects(vertices_[triangle.v1()], vertices_[triangle.v2()], vertices_[triangle.v3()], &distance);
...
bool MachPhysTileData::intersectWithEdge(
    const MexLine2d& line,
    const MexPoint3d& end1,
    const MexPoint3d& end2,
    ...
    IntersectingTriangles* pIntersectingTriangles) const
{
    ...
    if (fabs(startPoint.x() - finishPoint.x()) > fabs(startPoint.y() - finishPoint.y()))
    {
    ...
    {
    public:
        Sorter(PointSortFunction pSort)
            : pSort_(pSort)
        {
        }
...
void MachPhysTileData::pathProfile(const MexPoint3d& startPoint, const MexPoint3d& finishPoint, Profile* pProfile) const
{
    PATH_PROFILE_STREAM(std::endl << "......................................................" << std::endl);
    PATH_PROFILE_STREAM(
        "Enter MachPhysTileData::pathProfile Start find path profile from " << startPoint << " to " << finishPoint
                                                                            << std::endl);
    PATH_PROFILE_INDENT(2);
    ...
    PATH_PROFILE_INDENT(-2);
    PATH_PROFILE_STREAM(
        "Exit  MachPhysTileData::pathProfile Start find path profile from " << startPoint << " to " << finishPoint
                                                                            << std::endl);
...
bool MachPhysTileData::profileTriangleIntersect(
    ...
    MATHEX_SCALAR yStart,
    MATHEX_SCALAR xUnitDir,
    MATHEX_SCALAR yUnitDir,
    MATHEX_SCALAR lineLength,
    TriangleId triangleId,
    TriangleIntersectDatas* pIntersectDatas) const
{
        ...
        if (sOut[0] < (lineLength - eps) && sOut[1] > eps)
        {
            ...
            if (separation > eps)
            {
                ...
                if (separation > eps)
                {
                    ...
                    pIntersectDatas->push_back(TriangleIntersectData(
                        sOut[0],
