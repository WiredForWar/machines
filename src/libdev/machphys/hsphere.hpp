...
public:
    ...
    // dtor
    ~MachPhysHemiSphere() override;

    W4dVisibilityPlanPtr
    startGlow(const PhysAbsoluteTime& startTime, const PhysRelativeTime& duration, const uint& frameOffset);

    // Inherited from W4dEntity. Returns false.
    bool intersectsLine(const MexLine3d& line, MATHEX_SCALAR* pDistance, Accuracy accuracy) const override;
    void CLASS_INVARIANT;

    friend std::ostream& operator<<(std::ostream& o, const MachPhysHemiSphere& t);
