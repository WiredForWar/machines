...
public:
    ...
    // dtor
    ~MachPhysPunchBlast() override;

    enum Crack_Type : unsigned char
    {
        PUNCH,
        NUKE
    }; // a nuclear explosion also creates cracks and fire( but not blast).
    // Begin the explosion animation at startTime.
    // Returns the duration of the explosion.
    PhysRelativeTime startPunchBlast(
        const PhysAbsoluteTime& startTime,
        const MachPhysPlanetSurface& surface,
        const Crack_Type& type = PUNCH);
