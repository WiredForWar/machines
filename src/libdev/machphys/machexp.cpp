    ...
    const PhysAbsoluteTime& time)
{
    ...
    for (MachPhysMachineExplosionData::LinkDatas::const_iterator i = data.links().begin(); i != data.links().end(); ++i)
    {
        ...
        else
        {
            ...
            pLink->entityPlanForEdit().visibilityPlan(W4dVisibilityPlanPtr(pVisibilityPlan), now + mainExplosionOffset);
        }
    ...
    const PhysRelativeTime& duration)
{
    ...
    if (fireballSize > 0)
    {
        ...
        fireballPosition.x(
            (machineBoundingVol.maxCorner().x() + machineBoundingVol.minCorner().x()) / 2
            + machineBoundingVol.xLength() * randomDouble(-0.2, 0.2));
        fireballPosition.y(
            (machineBoundingVol.maxCorner().y() + machineBoundingVol.minCorner().y()) / 2
            + machineBoundingVol.yLength() * randomDouble(-0.2, 0.2));
        fireballPosition.z(
        ...
        W4dGarbageCollector::instance().add(
            pExplosionSite,
            SimManager::instance().currentTime() + fireballStartTime + duration + 10);
...
//
//     PhysTime    duration = randomDouble( maxSeparationTime * 1.0 / 4.0, maxSeparationTime * 3.0 / 4.0 ) -
//     startTimeOffset;
//
...
double MachPhysMachineExplosion::randomDouble(MATHEX_SCALAR lowerLimit, MATHEX_SCALAR upperLimit)
{
    PRE_INFO(lowerLimit);
    PRE_INFO(upperLimit);
...
int MachPhysMachineExplosion::randomInt(int lowerLimit, int upperLimit)
{
    PRE(lowerLimit < upperLimit);
...
int MachPhysMachineExplosion::randomInt(int upperLimit)
{
