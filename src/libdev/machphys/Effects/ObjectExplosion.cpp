...
#ifndef _INLINE
#include "machphys/Effects/ObjectExplosion.ipp"
#endif
    ...
    : pObject_(pObject)
{
    ...
    // bounding volumes.
    PRE_DATA(
        const MexAlignedBox3d& box
        = (pObject_->isComposite() ? pObject_->asComposite().compositeBoundingVolume() : pObject_->boundingVolume()));
...
PhysRelativeTime MachPhysObjectExplosion::explode(const PhysAbsoluteTime& startTime, MachPhys::DemolitionType type)
{
    ...
    if (buildingSize > 40)
    {
        W4dSoundManager::instance().play(pExplosionSite, SID_XPLODE_CONSTR_LARGE, startTime + mainFireballOffset, 1);
    }
    else
    {
        W4dSoundManager::instance().play(pExplosionSite, SID_XPLODE_CONSTR_SMALL, startTime + mainFireballOffset, 1);
    }
    ...
    {
        MATHEX_SCALAR fireballSize
            = MachPhysRandom::randomDouble(std::min(xOffset, yOffset), std::max(xOffset, yOffset));

        MexPoint3d fireballPosition(
            boxCentre.x() + MachPhysRandom::randomDouble(-sizeX / 2., sizeX / 2.),
...
void MachPhysObjectExplosion::sinkObject(
    PhysAbsoluteTime startTime,
