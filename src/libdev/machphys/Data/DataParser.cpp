...
#include "machphys/Data/LevelIndexMap.hpp"
#include "machphys/machphys.hpp"
#include "machphys/Constructions/Internal/ConstructionDataImpl.hpp"
...
MachPhysDataParser::MachineDataStore
MachPhysDataParser::machineDataStore(const MachPhysDataImplementation::AggressorDataStore& store) const
{
    MachineDataStore machineDStore(
        MachPhysDataImplementation::STORE_HARDWARE_SIZE,
        ctl_fixed_vector<MachPhysMachineData*>(MachPhysDataImplementation::STORE_SOFTWARE_SIZE, nullptr));
    ...
    for (size_t hwLevel = 0; hwLevel < store.size(); ++hwLevel)
    {
        ...
        {
            if (store[hwLevel][swLevel] != nullptr)
            {
...
MachPhysDataParser::ConstructionDataStore
MachPhysDataParser::constructionDataStore(const HardwareLabDataStore& store) const
{
    ConstructionDataStore constructionDStore(MachPhysDataImplementation::STORE_HARDWARE_SIZE, nullptr);

    size_t hwIndex = 0;

    for (size_t hwLevel = 0; hwLevel < store.size(); ++hwLevel)
    {
        if (store[hwLevel] != nullptr)
...
MachPhysDataParser::ConstructionDataStore
MachPhysDataParser::constructionDataStore(const GarrisonDataStore& store) const
{
    ConstructionDataStore constructionDStore(MachPhysDataImplementation::STORE_HARDWARE_SIZE, nullptr);
...
MachPhysDataParser::ConstructionDataStore
MachPhysDataParser::constructionDataStore(const MissileEmplacementDataStore& store) const
{
    ConstructionDataStore constructionDStore(MachPhysDataImplementation::STORE_HARDWARE_SIZE, nullptr);
            ...
            if (pParser->tokens()[0] == "CAPACITY")
            {
                ...
                for (size_t hwIndex = 0; hwIndex < nHardwareLevels; ++hwIndex)
                {
                    ...
                    size_t hardwareLevel
                        = MachPhysLevels::instance().hardwareLevel(MachPhys::RESOURCE_CARRIER, hwIndex);

                    for (size_t swIndex = 0; swIndex < nSoftwareLevels; ++swIndex)
                    {
                        size_t softwareLevel
                            = MachPhysLevels::instance().softwareLevel(MachPhys::RESOURCE_CARRIER, swIndex);
                ...
                MachPhysLevels::instance().softwareLevelIndexMap(subType)))
        {
            ...
            else if (pParser->tokens()[0] == "MOST_ADVANCED_CONSSUBTYPE")
            {
                ...
                for (size_t hwIndex = 0; hwIndex < nHardwareLevels; ++hwIndex)
                {
                    ...
                    ASSERT(
                        (*pStore)[hardwareLevel][firstSoftwareLevel] != nullptr,
...
bool MachPhysDataParser::constructionDataValid(const ConstructionDataStore& store) const
{
    bool result = true;
