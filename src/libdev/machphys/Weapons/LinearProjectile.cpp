...
std::ostream& operator<<(std::ostream& o, const MachPhysLinearProjectile& t)
{
    ...
    o << "MachPhysLinearProjectile " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysLinearProjectile " << static_cast<const void*>(&t) << " end" << std::endl;
    ...
    MexVec3* pUnitDirection)
{
    ...
    bool result = target.intersectsLine(fromDirection, &distance, W4dEntity::Accuracy::Parts);
    if (result)
    {
