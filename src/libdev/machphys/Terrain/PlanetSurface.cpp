...
void MachPhysPlanetSurface::parseTile(const SysPathName& directoryname, UtlLineTokeniser* pParser, W4dEntity* pParent)
{
    ...
    if (pParser->finished() || pParser->tokens()[0] != "ROTATION")
    {
            ...
            || (boundary.maxCorner().y() - boundary.minCorner().y() > edgeLength_))
        {
            ...
            PATH_PROFILE_STREAM(
                "Tile Problem Detected : " << finder.files()[0].pathName() << std::endl
                                           << pTile->tileData() << std::endl);
        }
    }
    else
    {
            ...
            || (boundary.maxCorner().y() - boundary.minCorner().y() > edgeLength_))
        {
            ...
            PATH_PROFILE_STREAM(
                "Tile Problem Detected : " << tilePathname << std::endl
                                           << pTile->tileData() << std::endl);
        }
    }
}
...
bool MachPhysPlanetSurface::save()
{
    ...
    if (saveSuccessful)
    {
        ...
        psfFileStream << "TILE_BOUNDS " << tileXMin_ << " " << tileYMin_ << " " << tileXMax_ << " " << tileYMax_
                      << std::endl;
...
bool MachPhysPlanetSurface::contains(const MexPoint3d& position) const
{
    ...
    return (x > -eps && x < eps + edgeLength_ * nTilesX_) && (y > -eps && y < eps + edgeLength_ * nTilesY_);
}
...
{
    return terrainHeight(x, y, nullptr);
}
    ...
    for (size_t yTile = 0; yTile < nTilesY_; ++yTile)
    {
        for (size_t xTile = 0; xTile < nTilesX_; ++xTile)
        {
            ...
            if (xTile < nTilesX_ - 1)
            {
                ...
                pOwnerDomain->addPortal(tileDomainArray_[yTile][xTile + 1], quad, identityTransform);
                /*
            ...
            if (yTile < nTilesY_ - 1)
            {
                ...
                // Construct the portal
                pOwnerDomain->addPortal(tileDomainArray_[yTile + 1][xTile], quad, identityTransform);
...
void MachPhysPlanetSurface::pathProfile(const MexPoint3d& startPoint, const MexPoint3d& finishPoint, Profile* pProfile)
    const
{
    ...
    Profile* pProfile) const
{
    ...
    for (MexGrid2d::Cells::const_iterator i = cells.begin(); i != cells.end(); ++i, ++pointIterator)
    {
        ...
        for (size_t j = profileStartIndex; j < pProfile->size(); ++j)
        {
            ...
            {
                tx.position(MexPoint3d(tx.position().x(), tx.position().y(), pImpl_->minMachineHeight_));
            }
        }
    }
...
void MachPhysPlanetSurface::handleFloors(const Floors& floors, Profile* pProfile) const
{
    PRE(floors.size() != 0);
    ...
    for (size_t i = 0; i < pProfile->size(); ++i)
    {
        ...
        for (size_t j = 0; j < floors.size() && ! finished; ++j)
        {
            ...
            {
                const MexPoint3d newPosition(transform.position().x(), transform.position().y(), floors[j].height());
...
bool MachPhysPlanetSurface::pointsInLine(const MexPoint3d& p1, const MexPoint3d& p2, const MexPoint3d& p3) const
{
    PATH_PROFILE_WHERE;
...
void MachPhysPlanetSurface::addLocalLight(W4dLight* pLight, const MexAlignedBox3d& lightCoveredBox)
{
    ...
    MexAlignedBox2d box2D(
        MexPoint2d(lightCoveredBox.minCorner().x(), lightCoveredBox.minCorner().y()),
...
W4dDomain* MachPhysPlanetSurface::domainByXYIndex(size_t xIndex, size_t yIndex) const
{
    PRE_INFO(xIndex);
    PRE_INFO(yIndex);
    PRE(xIndex < nTilesX_);
...
MATHEX_SCALAR MachPhysPlanetSurface::terrainHeight(MATHEX_SCALAR x, MATHEX_SCALAR y, const Floors& floors) const
{
    return terrainHeight(x, y, floors, nullptr);
}

MATHEX_SCALAR
MachPhysPlanetSurface::terrainHeight(MATHEX_SCALAR x, MATHEX_SCALAR y, const Floors& floors, MexVec3* pUnitNormal) const
{
