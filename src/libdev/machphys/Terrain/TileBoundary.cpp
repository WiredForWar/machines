...
#ifndef _INLINE
#include "machphys/Terrain/TileBoundary.ipp"
#endif
...
void MachPhysTileBoundary::add(const MexPoint3d& v1, const MexPoint3d& v2, const MexPoint3d& v3)
{
    ...
    ctl_vector<MexPoint3d>* pPoints) const
{
    ...
    if (xDiv > eps)
    {
        xFirst = xMin_ + xSpacing;
        xProportionalIncrement = xSpacing / xDiv;
    }
    else if (xDiv < -eps)
    {
        xFirst = xMax_ - xSpacing;
        xProportionalIncrement = -xSpacing / xDiv;
    }
    else
    {
        xFirst = x2;
        xProportionalIncrement = 1.0;
    }
    ...
    if (yDiv > eps)
    {
        yFirst = yMin_ + ySpacing;
        yProportionalIncrement = ySpacing / yDiv;
    }
    else if (yDiv < -eps)
    {
        yFirst = yMax_ - ySpacing;
        yProportionalIncrement = -ySpacing / yDiv;
    }
    else
    {
        yFirst = y2;
        yProportionalIncrement = 1.0;
    }
    ...
    while (true)
    {
        ...
        if (fabs(xNextProp - yNextProp) < eps)
            yNextProp += yProportionalIncrement;

        if (xNextProp < yNextProp)
        {
            prop = xNextProp;
            xNextProp += xProportionalIncrement;
        }
        else
        {
            prop = yNextProp;
