...
#include "machphys/Machines/MachineDamageData.hpp"
    ...
    , pImpl_(new MachPhysMachineImpl(*copyMe.pImpl_, data, race, bodyLevel, brainLevel))
{
    ...
    if (copyMe.pImpl_->pUpperBodyTurnLink_ != nullptr)
    {
...
*/
    ...
    return doMoveInfo(profilePtr, startTime, startSpeed, capSpeed, finalState);
}
...
void MachPhysMachine::addTurnTransformIfNecessary(TransformsPtr transformsPtr) const
{
    ...
    ctl_vector<MexTransform3d>* pTransforms) const
{
    ...
    else
    {
        ...
        MexTransform3d transitionTransform(MexTransform3d::X_XY, xVector, thisTransform.yBasis(), position);
...
PhysRelativeTime MachPhysMachine::doTurn(MATHEX_SCALAR angle)
{
    ...
    if (pLocomotionMethod_)
    {
        ...
        MachPhysMachineMoveInfo info(transformsPtr, rampAccelerationsPtr, nowTime);
...
{
    static MachPhysObjectFactory<size_t, MachPhysFacePlate> factory_(MachPhysLevels::instance().nSoftwareIndices());
...
void MachPhysMachine::convertMaterials(const MachPhysMachineData& data, MachPhys::Race race)
{
    ...
    const W4dCompositeMaterialVecChanger& changer = MachPhysRaceChanger::instance().changer(*this, data, race);
...
void MachPhysMachine::defaultExplosionData()
{
    ...
    for (W4dLinks::const_iterator i = links().begin(); i != links().end(); ++i)
    {
        ...
        explosionDataForEdit().addData(MachPhysMachineExplosionData::LinkData((*i)->id(), size, 1.0, 0.3, 0.6));
    }
}
...
void MachPhysMachine::initialiseShakeLinks()
{
    ...
    const char* shakeNames[] = { "head", "neck", "body", "" };
...
PhysRelativeTime MachPhysMachine::shake(const PhysAbsoluteTime startTime)
{
    ...
    for (W4dComposite::W4dLinks::const_iterator it = shakeLinks_.begin(); it != shakeLinks_.end(); ++it)
    {
        // Get the link
        W4dLink& link = *(*it);
        PhysLinearMotionPlan* pShakePlan = nullptr;
        ...
        if (! linkEntityPlan.hasMotionPlan())
        {
            ...
            for (size_t j = 0; j != nIntermediatePositions; ++j)
            {
                ...
                MATHEX_SCALAR pz = (pxPlusPy == 1.0 ? 0.0 : MachPhysRandom::randomDouble(0.0, 1.0 - pxPlusPy));
...
void MachPhysMachine::addLocomotionSound(const MachPhysMachineData& data)
{
    ...
    // beneath the machine
    if (idleId == SID_GEO_FLY || idleId == SID_APC_FLY_LARGE || idleId == SID_APC_FLY_SMALL || idleId == SID_BEE_FLY
        || idleId == SID_WASP_FLY)
    {
        SOUND_STREAM("Attaching to hip Link" << std::endl);
        W4dLink* pHipLink;
        if (findLink("hip", &pHipLink))
            W4dSoundManager::instance().play(pHipLink, idleId, PhysAbsoluteTime(0), 0);
    }
    else
    {
        SOUND_STREAM("Playing normally" << std::endl);
        W4dSoundManager::instance().play(this, idleId, PhysAbsoluteTime(0), 0);
    }
}
...
const MachPhysLocomotionMethod& MachPhysMachine::locomotionMethod() const
{
    ...
    PRE(pLocomotionMethod_ != nullptr);
    ...
    const MachPhysPlanetSurface& planetSurface) const
{
    ...
    for (MachPhysPlanetSurface::Profile::iterator i = smoothedProfilePtr->begin(); i != smoothedProfilePtr->end(); ++i)
    {
