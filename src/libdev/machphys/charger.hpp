...
public:
    ...
    // dtor
    ~MachPhysSuperCharger() override;

    // Begin the healing animation to heal machine at startTime, continuing for duraion,
    // or until stopHealing() is called.
    void
    startHealing(const MachPhysMachine& machine, const PhysAbsoluteTime& startTime, const PhysRelativeTime& duration);
    ...
    friend std::ostream& operator<<(std::ostream& o, const MachPhysSuperCharger& t);
