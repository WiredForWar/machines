...
public:
    ...
    MachPhysTileData(int nXVertices, int nYVertices, const MexAlignedBox2d& boundary, const RenMesh& mesh);
    ...
    using Profile = ctl_vector<MexTransform3d>;
    ...
    friend std::ostream& operator<<(std::ostream& o, const MachPhysTileData& t);
...
private:
    ...
    PER_FRIEND_READ_WRITE(Triangle);
    using TriangleId = ushort; // Index into triangles_
    ...
    using cellCallBackFn = bool(MachPhysTileData&, int, int);
    ...
    bool
    intersectWithTriangle(const MexLine2d& line, const Triangle& triangle, MexPoint3d* pPoint1, MexPoint3d* pPoint2)
        const;
    ...
    CTL_TRIPLE(IntersectingTriangle, MexPoint3d, point1, MexPoint3d, point2, MexVec3, normal);

    using IntersectingTriangles = ctl_vector<IntersectingTriangle>;
    ...
    using TriangleIntersectDatas = ctl_nb_vector<TriangleIntersectData>;
