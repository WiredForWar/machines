...
std::ostream& operator<<(std::ostream& o, const MachPhysElectroCharger& t)
{
    ...
    o << "MachPhysElectroCharger " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysElectroCharger " << static_cast<const void*>(&t) << " end" << std::endl;
...
PhysRelativeTime MachPhysElectroCharger::fire(const PhysAbsoluteTime& startTime, int)
{
    ...
    // Play sound
    W4dSoundManager::instance().play(this, SID_ELECTRICCHARGE, startTime, 1);
...
MachPhysElectro* MachPhysElectroCharger::createElectro(
    ...
    const MexPoint3d& targetOffset)
{
    ...
    purpleDuration = pPurpleCloud2->startSmoking(
        nWisps,
        endPosition,
        randomOffset,
        burstTime,
        ...
        zDepthOffset,
        MachPhysEffects::smokeMaterialPlan(MachPhysEffects::PURPLE));
    purpleDuration = pPurpleCloud3->startSmoking(
        nWisps,
        endPosition,
        randomOffset,
        burstTime,
        ...
        zDepthOffset,
        MachPhysEffects::smokeMaterialPlan(MachPhysEffects::PURPLE));
    purpleDuration = pPurpleCloud4->startSmoking(
        nWisps,
        endPosition,
        randomOffset,
        burstTime,
    ...
    MachPhysMachine* pMachine)
{
    ...
    if (pMachine->hasFacePlate())
    {
        ...
        smokeDuration = pSmokeCloud->startSmoking(
            nWisps,
            endPosition,
            randomOffset,
            startTime + haloLightningTime,
            wispInterval,
            wispSize,
            zDepthOffset,
    ...
    W4dEntity* pVictim)
{
    const MexAlignedBox3d& victimBox3D
        = (pVictim->isComposite() ? pVictim->asComposite().compositeBoundingVolume() : pVictim->boundingVolume());
    const MexPoint3d& minCorner = victimBox3D.minCorner();
    const MexPoint3d& maxCorner = victimBox3D.maxCorner();
    MexPoint3d centre(
        0.5 * (minCorner.x() + maxCorner.x()),
