...

{
    ...
    pFan1_ = (pFan1 ? links()[pFan1->id()] : nullptr);
    pFan2_ = (pFan2 ? links()[pFan2->id()] : nullptr);
    pFan3_ = (pFan3 ? links()[pFan3->id()] : nullptr);
    pFan4_ = (pFan4 ? links()[pFan4->id()] : nullptr);
    ...
    if (setWorking)
    {
        if (not isWorking())
        {
            ...
            if (cycleAnims(&workingPlanPtr))
                plan(*workingPlanPtr, now, 1000000, MachPhys::CONSTRUCTION_WORKING);

            // materials with texture chimhot1.bmp pulse colour from RenColour(0.5, 0.5, 0.5) to RenCOlour( 1, 1, 1)
            // every 0.2 sec
            ...
            if (pFan1_)
            {
                ...
                pFan1_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
            }
            ...
            if (pFan2_)
            {
                ...
                // Add the plan
                PhysMotionPlanPtr planPtr(pPlan);
                pFan2_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
            }
            ...
            if (pFan3_)
            {
                ...
                // Add the plan
                PhysMotionPlanPtr planPtr(pPlan);
                pFan3_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
            }
            ...
            if (pFan4_)
            {
                ...
                // Add the plan
                PhysMotionPlanPtr planPtr(pPlan);
                pFan4_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
            }

            // Set the working sound going
            if (subType() == MachPhys::MILITARY)
            {
                W4dSoundManager::instance().play(this, SID_MILITARYFACTORY, PhysAbsoluteTime(0), 0);
            }
            else
            {
                W4dSoundManager::instance().play(this, SID_CIVILIANFACTORY, PhysAbsoluteTime(0), 0);
            }
        }
