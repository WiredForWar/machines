...
// Copy ctor.
MachPhysLinearWeapon::MachPhysLinearWeapon(
    const MachPhysLinearWeapon& copyMe,
    MachPhys::Mounting mounting,
    W4dEntity* pParent,
...
std::ostream& operator<<(std::ostream& o, const MachPhysLinearWeapon& t)
{

    o << "MachPhysLinearWeapon " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysLinearWeapon " << static_cast<const void*>(&t) << " end" << std::endl;
    ...
    MATHEX_SCALAR* distance) const
{
    ...
    W4dObjectTracker tracker(
        *this,
        data.launchOffsets()[launchIndex],
        target,
        targetOffset,
        maxAbsYawSine,
        maxAbsPitchSine,
