...
MachPhysLocomotionMethod* MachPhysHoverBoots::clone(MachPhysMachine* pNewMachine, const W4dLinks& links)
{
    ...
    W4dLink* pLHoverBoot = nullptr;
    W4dLink* pRHoverBoot = nullptr;
    W4dLink* pBobbingLink = nullptr;

    if (pLHoverBoot_ != nullptr)
        pLHoverBoot = links[pLHoverBoot_->id()];

    if (pRHoverBoot_ != nullptr)
    ...
    const MachPhysMachineMoveInfo& info)
{
    ...
    for (size_t i = 0; i < nSegments; ++i)
    {
        ...
        else
        {
            ...
            if (swingAngle > maxTurnAngle)
                swingAngle = maxTurnAngle;
            else if (swingAngle < -maxTurnAngle)
                swingAngle = -maxTurnAngle;
...
void MachPhysHoverBoots::startBobbing()
{
    ...
    if (bobHeight_ > 0.0)
    {
        ...
        pBobbingLink_->entityPlanForEdit().absoluteMotion(PhysMotionPlanPtr(pPlan), now, repeatForever, BOBBING);
    }
}
