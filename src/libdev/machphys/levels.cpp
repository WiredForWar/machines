...
bool MachPhysLevels::levelValid(MachPhys::AdministratorSubType subType, size_t hardwareLevel, size_t softwareLevel)
    const
{
    PRE_INFO(subType);
    PRE_INFO(hardwareLevel);
...
void MachPhysLevels::hardwareLevelData(MachPhys::AdministratorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::hardwareLevelData(MachPhys::AggressorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::hardwareLevelData(MachPhys::ConstructorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::hardwareLevelData(MachPhys::TechnicianSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::hardwareLevelData(MachPhys::HardwareLabSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::hardwareLevelData(
    MachPhys::MissileEmplacementSubType subType,
    size_t** pDataArray,
    size_t* pNElements) const
{
    switch (subType)
    {
...
void MachPhysLevels::softwareLevelData(MachPhys::AdministratorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::softwareLevelData(MachPhys::AggressorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::softwareLevelData(MachPhys::ConstructorSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
void MachPhysLevels::softwareLevelData(MachPhys::TechnicianSubType subType, size_t** pDataArray, size_t* pNElements)
    const
{
    switch (subType)
    {
...
size_t MachPhysLevels::uniqueHardwareIndex(MachPhys::AdministratorSubType subType, size_t hardwareLevel) const
{
    ...
    switch (subType)
    {
        case MachPhys::BOSS:
            ...
            break;
        case MachPhys::OVERSEER:
            result = elementsof(administratorBossHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::COMMANDER:
            result = elementsof(administratorBossHardwareLevels) + elementsof(administratorOverseerHardwareLevels)
...
size_t MachPhysLevels::uniqueHardwareIndex(MachPhys::AggressorSubType subType, size_t hardwareLevel) const
{
    ...
    switch (subType)
    {
        case MachPhys::GRUNT:
            ...
            break;
        case MachPhys::ASSASSIN:
            result = elementsof(aggressorGruntHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::KNIGHT:
            result = elementsof(aggressorGruntHardwareLevels) + elementsof(aggressorAssassinHardwareLevels)
                + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::BALLISTA:
            result = elementsof(aggressorGruntHardwareLevels) + elementsof(aggressorAssassinHardwareLevels)
                + elementsof(aggressorKnightHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::NINJA:
            result = elementsof(aggressorGruntHardwareLevels) + elementsof(aggressorAssassinHardwareLevels)
                + elementsof(aggressorKnightHardwareLevels) + elementsof(aggressorBallistaHardwareLevels)
                + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
    }
...
size_t MachPhysLevels::uniqueHardwareIndex(MachPhys::ConstructorSubType subType, size_t hardwareLevel) const
{
    ...
    switch (subType)
    {
        case MachPhys::DOZER:
            ...
            break;
        case MachPhys::BUILDER:
            result = elementsof(constructorDozerHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::BEHEMOTH:
            result = elementsof(constructorDozerHardwareLevels) + elementsof(constructorBuilderHardwareLevels)
                + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
    }
...
size_t MachPhysLevels::uniqueHardwareIndex(MachPhys::TechnicianSubType subType, size_t hardwareLevel) const
{
    ...
    switch (subType)
    {
        case MachPhys::LAB_TECH:
            ...
            break;
        case MachPhys::TECH_BOY:
            result = elementsof(technicianLabTechHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
        case MachPhys::BRAIN_BOX:
            result = elementsof(technicianLabTechHardwareLevels) + elementsof(technicianTechBoyHardwareLevels)
                + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
    }
...
size_t MachPhysLevels::uniqueHardwareIndex(MachPhys::FactorySubType subType, size_t hardwareLevel) const
{
    ...
    switch (subType)
    {
        case MachPhys::MILITARY:
            ...
            break;
        case MachPhys::CIVILIAN:
            result = elementsof(factoryMilitaryHardwareLevels) + hardwareLevelIndexMap(subType).index(hardwareLevel);
            break;
    }
...
size_t MachPhysLevels::nHardwareIndices(MachPhys::ConstructionType type) const
{
    ...
    switch (type)
    {
        case MachPhys::HARDWARE_LAB:
            result = elementsof(hardwareLabMilitaryHardwareLevels) + elementsof(hardwareLabCivilianHardwareLevels);
            break;
        ...
        case MachPhys::FACTORY:
            result = elementsof(factoryMilitaryHardwareLevels) + elementsof(factoryCivilianHardwareLevels);
            break;
...
size_t MachPhysLevels::nSoftwareIndices() const
{
    ...
    result
        = std::max(result, administratorCommanderSoftwareLevels[elementsof(administratorCommanderSoftwareLevels) - 1]);
...
std::ostream& operator<<(std::ostream& o, const MachPhysLevels& t)
{
    ...
    o << "MachPhysLevels " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachPhysLevels " << static_cast<const void*>(&t) << " end" << std::endl;
