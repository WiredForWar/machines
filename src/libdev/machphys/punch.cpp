...
static MachPhysCrackFire& crackFire(
    W4dEntity* pParent,
...
void MachPhysPunchBlast::CLASS_INVARIANT
{
    ...
    INVARIANT(implies(startPunchBlastCalled(), blastBeginTime_ != 0 || blastEndTime_ != 0));
}
...
PhysRelativeTime MachPhysPunchBlast::startPunchBlast(
    const PhysAbsoluteTime& startTime,
    const MachPhysPlanetSurface& surface,
    const Crack_Type& type)
{
...
bool MachPhysPunchBlast::reachDistance(const MATHEX_SCALAR& distance, PhysRelativeTime* pTime)
{
    ...
    MexLine2d line(
        MexPoint2d(allPoints()[0].x(), allPoints()[0].y()),
        MexPoint2d(allPoints()[30].x(), allPoints()[30].y()));
...
{
    static MATHEX_SCALAR maxRange = MexLine2d(
                                        MexPoint2d(allPoints()[0].x(), allPoints()[0].y()),
                                        MexPoint2d(allPoints()[30].x(), allPoints()[30].y()))
                                        .length();
    return maxRange;
