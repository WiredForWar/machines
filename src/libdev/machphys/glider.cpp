    ...
    : MachPhysLocomotionMethod(pImpl_ = new MachPhysGliderImpl(pMachine, this, height))
{
    ...
    if (pLFan_)
    {
        ...
        pLFan_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
    }
    ...
    if (pRFan_)
    {
        ...
        // Add the plan
        PhysMotionPlanPtr planPtr(pPlan);
        pRFan_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
    }
    ...
    if (pLTurb_)
    {
        ...
        // Add the plan
        PhysMotionPlanPtr planPtr(pPlan);
        pLTurb_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
    }
    ...
    if (pRTurb_)
    {
        ...
        // Add the plan
        PhysMotionPlanPtr planPtr(pPlan);
        pRTurb_->entityPlanForEdit().absoluteMotion(planPtr, SimManager::instance().currentTime(), 1000000);
    }
    ...
    const MachPhysMachineMoveInfo& info)
{
    ...
    for (size_t i = 0; i < nSegments; ++i)
    {
        ...
        PhysRelativeTime constantVelocityTime
            = (segmentEndTime - segmentStartTime) - (accelerationTime + decelerationTime);
        ...
        if (constantVelocityTime < 0)
            constantVelocityTime = 0;
        ...
        else
        {
            if (translationTime > 0)
            {
                ...
                pThrustLight->entityPlanForEdit().visibilityPlan(
                    thrustAndBrake(pThrust_, lightStartTime, pitchTime),
                    lightStartTime);
                ...
                pBrakeLight->entityPlanForEdit().visibilityPlan(
                    thrustAndBrake(pBrake_, startTime + pitchDownTime, pitchTime),
                    startTime + pitchDownTime);
            }
    ...
    if (translationTime > 0.0)
    {
        ...
        if (data.machineType() == MachPhys::GEO_LOCATOR && data.hwLevel() == 3)
        {
...
void MachPhysGlider::startBobbing(const PhysAbsoluteTime& startTime)
{
    ...
    if (height_ > 0.0)
    {
        ...
        if (transitTime > 0)
        {
            ...
            pPrepRampAccelerations->push_back(PhysRampAcceleration(
                distance,
                startSpeed,
                desiredSpeed,
                endSpeed,
        ...
        pHip_->entityPlanForEdit()
            .absoluteMotion(PhysMotionPlanPtr(pBobPlan), startTime + transitTime, repeatForever, BOBBING);
    }
}
