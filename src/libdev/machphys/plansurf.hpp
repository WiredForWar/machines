...
public:
    ...
    void pathProfile(const MexTransform3d& startTransform, const MexPoint3d& finishPoint, Profile* pProfile) const;
    ...
    //  set to the height of the floor.
    void
    pathProfile(const MexPoint3d& startPoint, const MexPoint3d& finishPoint, const Floors& floors, Profile* pProfile)
        const;
...
private:
    ...
    void handleFloors(const Floors& floors, Profile* pProfile) const;
    //  PRE( floors.size() != 0 );

    bool pointsInLine(const MexPoint3d& p1, const MexPoint3d& p2, const MexPoint3d& p3) const;

    using TileRow = ctl_vector<MachPhysTerrainTile*>;
    using TileArray2d = ctl_vector<TileRow>;

    using TileDomainRow = ctl_vector<W4dDomain*>;
    using TileDomainArray2d = ctl_vector<TileDomainRow>;
