...
void MachPhysEntityDamage::update()
{
    ...
    // The vertices of the building get progressively darker starting at 1% damage thru' to 100% damage. This damage is
    // shown on or      near to the point of impact of the weapon(s) causing the damage.
    if (damageLevel_ > 0)
        darkening();
    ...
    if (damageLevel_ >= 60)
    {
        ...
        if (pSplat_ == nullptr)
        {
            ...
            pSplat_->temporaryScale(
                RenNonUniformScale(sizeX * scale, sizeY * scale, sizeZ * scale),
                W4dEntity::NOT_PROPOGATE);
        }
    }
...
const MachPhysPuffType& MachPhysEntityDamage::smoke1Type() const
{
...
const MachPhysPuffType& MachPhysEntityDamage::smoke2Type() const
{
