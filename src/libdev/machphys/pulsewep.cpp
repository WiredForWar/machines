...
MachPhysPulseWeapon::MachPhysPulseWeapon(const SysPathName& compositeFileName, MachPhys::WeaponType type)
...
void MachPhysPulseWeapon::createFiringDiscs(const PhysAbsoluteTime& startTime)
{
    ...
    PhysRelativeTime interval = (nRoundsPerBurst == 1 ? 0.0 : burstDuration / (nRoundsPerBurst - 1));
    ...
    const PhysAbsoluteTime& startTime)
{
    ...
    if (firstTime)
    {
        ...
        PhysRampAcceleration ramp(0.0, duration, 1.0, -0.49, -0.1, -0.1, PhysRampAcceleration::BY_DISTANCE);
