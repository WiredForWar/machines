...
MachPhysMachineExplosionData& MachPhysMachineExplosionData::operator=(const MachPhysMachineExplosionData& copyMe)
{
...
void MachPhysMachineExplosionData::addData(const LinkData& data)
{
    CB_MacPhysMachineExplosionData_DEPIMPL()
        ...
        if (data.shootOffProbability() == 1.0) links_.push_front(data);
    else links_.push_back(data);

    maxSize_ = std::max(maxSize_, data.size());
}

const MachPhysMachineExplosionData::LinkDatas& MachPhysMachineExplosionData::links() const
{
    CB_MacPhysMachineExplosionData_DEPIMPL() return links_;
}
