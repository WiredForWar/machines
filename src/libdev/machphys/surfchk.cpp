...
MachPhysPlanetSurfaceChecker::MachPhysPlanetSurfaceChecker(const MachPhysPlanetSurface& surface)
    ...
    , minMaxGradient_(std::min(
          maxNormalGradient_.asScalar(),
          std::min(maxLowObstacleGradient_.asScalar(), maxWaterGradient_.asScalar())))
{
...
static void createProjectedSegment(const MachPhysPlanetSurfaceChecker::Triangle& triangle3d, MexLine2d* segment)
{
    ...
    MexTriangle2d triangleProjection(triangle3d.point1(), triangle3d.point2(), triangle3d.point3());
...
bool MachPhysPlanetSurfaceChecker::checkProjectedSegmentOK(
...
bool MachPhysPlanetSurfaceChecker::segmentInsideObstacle(
    const MexLine2d& segment,
    ObstacleFlag flags,
    const PhysConfigSpace2d& configSpace) const
{
...
bool MachPhysPlanetSurfaceChecker::checkTriangleOK(
    const Triangle& triangle,
    MexDegrees gradient,
    const PhysConfigSpace2d& configSpace) const
{
    ...
    MexTriangle2d triangle2d(triangle.point1(), triangle.point2(), triangle.point3());
...
bool MachPhysPlanetSurfaceChecker::triangleInsideObstacle(
    const MexTriangle2d& triangle,
    ObstacleFlag flags,
    const PhysConfigSpace2d& configSpace) const
{
    //  The flags value we have passed in is the flags value for the obstacles
