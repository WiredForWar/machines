...
void MachPhysRaceChangerBody::add(const MachPhysMachine& machine, const MachPhysMachineData& data)
{
...
void MachPhysRaceChangerBody::add(const MachPhysConstruction& construction, const MachPhysConstructionData& data)
{
...
MachPhysMachineSpec MachPhysRaceChangerBody::machineSpec(const MachPhysMachineData& data)
{
    ...
    return MachPhysMachineSpec(data.machineType(), data.subType(), data.hwLevel(), swLevel, MachPhys::RED);
}

// static
MachPhysConstructionSpec MachPhysRaceChangerBody::constructionSpec(const MachPhysConstructionData& data)
{
    // Get all the info needed to construct the spec.
    return MachPhysConstructionSpec(data.constructionType(), data.subType(), data.hwLevel(), MachPhys::RED);
}
...
MachPhysRaceChangerBody::changer(const MachPhysWeapon& weapon, MachPhys::Race toRace) const
{
...
const MachPhysRaceChangerBody::TexturesVec& MachPhysRaceChangerBody::texturesVec()
{
    ...
    if (first)
    {
        ...
        if (SysMetaFile::useMetaFile())
        {
            ...
            pIstream = std::unique_ptr<istream>(_NEW(SysMetaFileIstream(metaFile, logoFilename, std::ios::in)));
        }
        else
        {
            ASSERT_FILE_EXISTS(logoFilename.c_str());
            // pIstream = _NEW( ifstream( logoFilename.c_str(), ios::text | ios::in ) );
            pIstream = std::unique_ptr<istream>(_NEW(std::ifstream(logoFilename.c_str(), std::ios::in)));
        }
