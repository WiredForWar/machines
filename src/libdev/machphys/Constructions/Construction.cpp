...
using Entrances = MachPhysConstructionImpl::Entrances;
using LinkTransforms = MachPhysConstructionImpl::LinkTransforms;
using CompletionBand = MachPhysConstructionImpl::CompletionBand;
...
static const W4dMaterialPlanPtr& bandPlanPtr(CompletionBand band)
{
    ...
    W4dMaterialPlanPtr* pPlanPtr = nullptr;
    switch (band)
    {
...
void MachPhysConstruction::doPercentageComplete(double newPercentage)
{
    ...
    if (newPercentage != percentageComplete_ || ! completionVisualised_)
    {
        ...
        if (newPercentage < 10)
            band = MachPhysConstructionImpl::BAND1;
        else if (newPercentage < 20)
            band = MachPhysConstructionImpl::BAND2;
        else if (newPercentage < 30)
            band = MachPhysConstructionImpl::BAND3;
        else if (newPercentage < 40)
            band = MachPhysConstructionImpl::BAND4;
        else if (newPercentage < 50)
            band = MachPhysConstructionImpl::BAND5;
        else if (newPercentage < 60)
            band = MachPhysConstructionImpl::BAND6;
        else if (newPercentage < 70)
            band = MachPhysConstructionImpl::BAND7;
        else if (newPercentage < 80)
            band = MachPhysConstructionImpl::BAND8;
        else if (newPercentage < 90)
            band = MachPhysConstructionImpl::BAND9;
        else if (newPercentage < 100)
            band = MachPhysConstructionImpl::BAND10;
        else
            band = MachPhysConstructionImpl::NBANDS;
        ...
        if (band != pImpl_->completionBand_)
        {
            ...
            for (W4dEntityIter it(this); ! it.isFinished(); ++it)
            {
                ...
                if ((*it).clientData() != (int)MachPhys::ECD_DONTAPPLYANIMATION)
                {
                    ...
                    {
                        (*it).entityPlanForEdit().materialPlan(
                            bandPlanPtr(band),
                            SimManager::instance().currentTime(),
                            0,
                            MachPhys::CONSTRUCTION_CONSTRUCTING);
                    }
                }
            }
...
void MachPhysConstruction::convertInteriorMaterials(MachPhys::Race race)
{
    ...
    W4dCompositeMaterialVecChanger::fillMaterialMaps(
        *pInteriorComposite_, fromHue, toColours, fromTextures, singleToTexturesVec, materialMaps);
...
{
    CB_DEPIMPL(W4dDomain*, pInteriorDomain_);
    return pInteriorDomain_ != nullptr;
}
...
void MachPhysConstruction::setupEntrances()
{
    ...
    for (MachPhysConstructionData::Entrances::const_iterator i = entrances.begin(); i != entrances.end(); ++i)
    {
        W4dLink* pLDoor = nullptr;
        W4dLink* pRDoor = nullptr;
...
const double& MachPhysConstruction::damageLevel() const
{
    ...
    return (pDamage_ == nullptr ? zero : pDamage_->damageLevel());
}
...
{
    CB_DEPIMPL(MachPhysEntityDamage*, pDamage_);
    return (pDamage_ == nullptr ? false : pDamage_->damaged());
}
...
bool MachPhysConstruction::hasSavedPadsTransforms() const
{
    ...
    return pSavedPadsTransforms_ != nullptr;
}
...
void MachPhysConstruction::savePadsTransforms()
{
    ...
    for (W4dLinks::const_iterator it = links().begin(); it != links().end(); ++it)
    {
        ...
        {
            pImpl_->savedPadsTransforms().push_back(
                MachPhysConstructionImpl::LinkTransformData((*it)->id(), (*it)->localTransform()));
        }
    }
}
...
static MATHEX_SCALAR
sqrDistancePadVertices(const MexPoint3d& point, const MexVec3& normal, const ctl_vector<MexPoint3d>& vertices)
{
    PRE(vertices.size() == 4);
...
static bool
verticesBelowPlan(const MexPoint3d& planePoint, const MexVec3& planeNormal, const ctl_vector<MexPoint3d>& vertices)
{
    ...
    MATHEX_SCALAR* pDistance)
{
    ...
    groundVertices.push_back(MexPoint3d(
        padVertices[0].x(),
        padVertices[0].y(),
        terrain.terrainHeight(padVertices[0].x(), padVertices[0].y(), &normalVertex0)));
    groundVertices.push_back(MexPoint3d(
        padVertices[1].x(),
        padVertices[1].y(),
        terrain.terrainHeight(padVertices[1].x(), padVertices[1].y(), &normalVertex1)));
    groundVertices.push_back(MexPoint3d(
        padVertices[2].x(),
        padVertices[2].y(),
        terrain.terrainHeight(padVertices[2].x(), padVertices[2].y(), &normalVertex2)));
    groundVertices.push_back(MexPoint3d(
        padVertices[3].x(),
        padVertices[3].y(),
        terrain.terrainHeight(padVertices[3].x(), padVertices[3].y(), &normalVertex3)));
    ...
    planeVertices.definePlane(
        groundVertices[((indexPivot + 3) % nVertices)],
        groundVertices[indexPivot],
        groundVertices[((indexPivot + 1) % nVertices)]);
    ...
    MATHEX_SCALAR* pDistance)
{
    ...
    groundVertices.push_back(MexPoint3d(
        padVertices[0].x(),
        padVertices[0].y(),
        terrain.terrainHeight(padVertices[0].x(), padVertices[0].y())));
    groundVertices.push_back(MexPoint3d(
        padVertices[1].x(),
        padVertices[1].y(),
        terrain.terrainHeight(padVertices[1].x(), padVertices[1].y())));
    groundVertices.push_back(MexPoint3d(
        padVertices[2].x(),
        padVertices[2].y(),
        terrain.terrainHeight(padVertices[2].x(), padVertices[2].y())));
    groundVertices.push_back(MexPoint3d(
        padVertices[3].x(),
        padVertices[3].y(),
        terrain.terrainHeight(padVertices[3].x(), padVertices[3].y())));
...
static void layVerticesOnTerrain(
    const MachPhysPlanetSurface& terrain,
    const ctl_vector<MexPoint3d>& padVertices,
    MexPoint3d* pPivot,
    MexVec3* pNormal)
    ...
    if (! matchFound)
    {
        ...
        if (! matchFound)
        {
            for (std::size_t i = 0; i < padVertices.size(); ++i)
            {
                // Second vertex
                LIONEL_STREAM("Laying pad on vertex " << i << std::endl);
                if (tryToLayThreeVerticesOnGround(
                        i,
                        terrain,
                        padVertices,
                        VERTICES_ABOVE,
...
MexAlignedBox2d MachPhysConstruction::localLargestBoundary() const
{
    ...
    const MexAlignedBox2d meshBoundary2d(
        boundary3d.minCorner().x(),
