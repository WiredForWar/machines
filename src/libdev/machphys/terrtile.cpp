...
MachPhysTerrainTile::MachPhysTerrainTile(
    ...
    MATHEX_SCALAR yMin,
    MATHEX_SCALAR yMax)
    : W4dEntity(exemplar(lodFileName, nXVertices, nYVertices, xMin, xMax, yMin, yMax), pParent, transform)
    , pTileDataPtr_(nullptr)
...
bool MachPhysTerrainTile::intersectsLine(const MexLine3d& testLine, MATHEX_SCALAR* pDistance, Accuracy) const
{
    // JON_STREAM("MachPhysTerrainTile::intersectsLine entry " << testLine.end1() << " to " << testLine.end2() <<
    // std::endl;) Get the global transform, and invert it so that we can produce a copy of the line transformed into
    // the local coordinate system of this entity.
    ...
    // JON_STREAM("  After bvint " << result << " entryDistance " << entryDistance << " exitDistance " << exitDistance
    // << std::endl;)
    if (result)
    {
