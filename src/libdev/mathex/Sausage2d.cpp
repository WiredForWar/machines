...
#ifndef _INLINE
#include "mathex/Sausage2d.ipp"
#endif

MexSausage2d::MexSausage2d(const MexPoint2d& startPoint, const MexPoint2d& endPoint, MATHEX_SCALAR radius)
...
bool MexSausage2d::intersects(const MexPolygon2d& polygon, Mathex::TouchingRule rule) const
{
    ...
    MATHEX_SCALAR clearance
        = (rule == Mathex::TOUCH_IS_INTERSECT ? radius_ + MexEpsilon::instance() : radius_ - MexEpsilon::instance());
    ...
    for (size_t i = 0; i != nVertices; ++i)
    {
        ...
        if (sausageIsCircle)
            sqrDistance = MexLine2d::sqrEuclidianDistance(
                p1,
                p2,
                haveLengths ? edgeLengths[i] : p1.euclidianDistance(p2),
                startPoint_);
        else
            sqrDistance = MexLine2d::sqrEuclidianDistance(
                p1,
...
bool MexSausage2d::intersects(const MexSausage2d& rhs, Mathex::TouchingRule rule) const
{
    ...
    if (length_ <= epsilon)
    {
        ...
        {
            sqrDistance = MexLine2d::sqrEuclidianDistance(rhs.startPoint_, rhs.endPoint_, rhs.length_, startPoint_);
        }
    }
    else if (rhs.length_ <= epsilon)
    {
        sqrDistance = MexLine2d::sqrEuclidianDistance(startPoint_, endPoint_, length_, rhs.startPoint_);
    }
    else
    {
        sqrDistance = MexLine2d::sqrEuclidianDistance(
...
bool MexSausage2d::intersects(const MexCircle2d& circle, Mathex::TouchingRule rule) const
{
    ...
    if (length_ <= epsilon)
        ...
        sqrDistance = MexLine2d::sqrEuclidianDistance(startPoint_, endPoint_, length_, circle.centre());
