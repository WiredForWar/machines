...
bool MexPolygon2d::intersects(const MexPoint2d& q1, const MexPoint2d& q2, Mathex::TouchingRule rule) const
{
    bool result;
...
bool MexPolygon2d::intersects(const MexPolygon2d& rhs, Mathex::TouchingRule rule) const
{
    ...
    if (not result)
    {
        ...
        const MexPolygon2d::Lengths* pRhsEdgeLengths = (rhs.isCachingData() ? &(rhs.edgeLengths()) : nullptr);
        ...
        for (size_t i = 0, j = 1; i != nRhsVertices; ++i)
        {
            ...
            if (thisBoundary.intersects(MexAlignedBox2d(q1, q2)))
            {
                if (rule == Mathex::TOUCH_IS_INTERSECT)
                    ...
                    result = isPenetratedBy(
                        q1,
                        q2,
                        pRhsEdgeLengths == nullptr ? q1.euclidianDistance(q2) : (*pRhsEdgeLengths)[i]);
...
bool MexPolygon2d::isPenetratedBy(const MexPoint2d& q1, const MexPoint2d& q2, MATHEX_SCALAR lineLength) const
{
    ...
    for (size_t i = 0;;)
    {
        ...
        else if (lastVertexSide != Mathex::ONEDGE and lastVertexSide != side)
        {
            ...
            sOnLine[nLineIntersections++] = (hLastVertex * s - h * sLastVertex) / (hLastVertex - h);
        }
