...
)
{
    ...
    if (pt.x() < box.minCorner().x())
        code_ = code_ | left;
    else if (pt.x() > box.maxCorner().x())
        code_ = code_ | right;
    if (pt.y() < box.minCorner().y())
        code_ = code_ | below;
    else if (pt.y() > box.maxCorner().y())
        code_ = code_ | above;
...
bool OutCode::is_directly_opposite(const OutCode& oc) const
{
    return (code_ == left && oc.code_ == right) || (code_ == right && oc.code_ == left)
