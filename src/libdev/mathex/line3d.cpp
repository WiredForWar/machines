...
MATHEX_SCALAR
MexLine3d::sqrEuclidianDistance(const MexPoint3d& a, const MexPoint3d& b, MATHEX_SCALAR l, const MexPoint3d& p)
{
    MATHEX_SCALAR epsilon = MexEpsilon::instance();
...
bool MexLine3d::segmentIntersects(
    const MexPoint3d& p1,
    const MexPoint3d& p2,
    const MexPoint3d& p3,
    MATHEX_SCALAR* pDistance,
    IntersectSide side) const
{
    ...
    IntersectSide side) const
{
    ...
    *pLength = length();
    ...
    if (result)
    {
        ...
        MexPoint3d a(end1_.x() + lambda * d.x(), end1_.y() + lambda * d.y(), end1_.z() + lambda * d.z());
