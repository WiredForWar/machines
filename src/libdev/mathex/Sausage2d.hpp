...
public:
    ...
    MexSausage2d(const MexPoint2d& startPoint, const MexPoint2d& endPoint, MATHEX_SCALAR radius);
    ...
    bool intersects(const MexPolygon2d& polygon, Mathex::TouchingRule rule = Mathex::TOUCH_IS_INTERSECT) const;

    // True iff this intersects rhs. Touching alone may optionally be regarded
    // as intersection as defined by rule.
    // Note that no bounding box test is performed - the client is advised to
    // do this before calling this function.
    bool intersects(const MexSausage2d& rhs, Mathex::TouchingRule rule = Mathex::TOUCH_IS_INTERSECT) const;

    // True iff this intersects circle. Touching alone may optionally be regarded
    // as intersection as defined by rule.
    // Note that no bounding box test is performed - the client is advised to
    // do this before calling this function.
    bool intersects(const MexCircle2d& circle, Mathex::TouchingRule rule = Mathex::TOUCH_IS_INTERSECT) const;
