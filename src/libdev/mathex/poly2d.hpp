...
public:
    // Useful typedefs
    using Lengths = ctl_vector<MATHEX_SCALAR>;
    ...
    bool intersects(const MexPoint2d& p1, const MexPoint2d& p2, Mathex::TouchingRule rule = Mathex::TOUCH_IS_INTERSECT)
        const;
    ...
    bool isPenetratedBy(const MexPoint2d& q1, const MexPoint2d& q2, MATHEX_SCALAR lineLength) const;
    // PRE( fabs( q1.euclidianDistance( q2 ) - lineLength ) < MexEpsilon::instance() );

    // True if this polygon intersects rhs.
    // rule indicates whether penetration is required.
    bool intersects(const MexPolygon2d& rhs, Mathex::TouchingRule rule = Mathex::TOUCH_IS_INTERSECT) const;
...
// class ostream;
std::ostream& operator<<(std::ostream&, const MexPolygon2d&);
