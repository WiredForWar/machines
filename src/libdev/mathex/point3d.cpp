...
void MexPoint3d::rotateAboutAxis(const MexVec3& axis, const MexPoint3d& pointOnAxis, MexRadians angle)

{
    ...
    POST_DATA(
        MATHEX_SCALAR D = -(originalPoint[0] * axis[0] + originalPoint[1] * axis[1] + originalPoint[2] * axis[2]));

    /*
    cout << "Plane " << A << " " << B << " " << C << " " << D << endl;
    cout << "Transformed point " << *this << endl;
    cout << "A*point_[ X ] + B*point_[ Y ] + C*point_[ Z ] + D " << A*point_[ X ] + B*point_[ Y ] + C*point_[ Z ] + D <<
    endl;
    */
    ...
    POST_DATA(
        MATHEX_SCALAR t = -(A * pointOnAxis.x() + B * pointOnAxis.y() + C * pointOnAxis.z() + D)
    ...
    POST(
        vec1.isZeroVector() ? (originalPoint == *this)
                            : (Mathex::abs(
                                   (MATHEX_SCALAR)Mathex::abs(vec2.angleBetween(vec1).asScalar())
                                   - (MATHEX_SCALAR)Mathex::abs(normalisedAngle.asScalar()))
                               < MexEpsilon::instance()));
}
