...
MATHEX_SCALAR MexQuaternion::rotation(MexVec3* pUnitVector) const
{
    ...
    {
        pUnitVector->setVector(v_.x() / halfSine, v_.y() / halfSine, v_.z() / halfSine);
    }
...
void MexQuaternion::CLASS_INVARIANT
{
    ...
    MATHEX_SCALAR squareSum = v_.x() * v_.x() + v_.y() * v_.y() + v_.z() * v_.z() + s_ * s_;
...
// static
void MexQuaternion::normalise(MATHEX_SCALAR* pX, MATHEX_SCALAR* pY, MATHEX_SCALAR* pZ, MATHEX_SCALAR* pS)
{
