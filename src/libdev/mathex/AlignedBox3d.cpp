...
#ifndef _INLINE
#include "mathex/AlignedBox3d.ipp"
#endif
...
MexAlignedBox3d::MexAlignedBox3d(
    const MATHEX_SCALAR x1,
    const MATHEX_SCALAR y1,
    const MATHEX_SCALAR z1,
    const MATHEX_SCALAR x2,
    const MATHEX_SCALAR y2,
    const MATHEX_SCALAR z2)
    : isEmpty_(false)
    , spinSafe_(false)
{
...
MexAlignedBox3d::MexAlignedBox3d(
    ...
    MATHEX_SCALAR xDimension,
    MATHEX_SCALAR yDimension,
    MATHEX_SCALAR zDimension)
    : isEmpty_(false)
    , spinSafe_(false)
{
    PRE(xDimension >= 0);
    ...
    minCorner_.setPoint(centre.x() - xDimension / 2, centre.y() - yDimension / 2, centre.z() - zDimension / 2);

    maxCorner_.setPoint(centre.x() + xDimension / 2, centre.y() + yDimension / 2, centre.z() + zDimension / 2);
...
void MexAlignedBox3d::makeSpinSafe()
{
    ...
    minCorner.setPoint(centre.x() - radius, centre.y() - radius, centre.z() - radius);

    maxCorner.setPoint(centre.x() + radius, centre.y() + radius, centre.z() + radius);
...
bool MexAlignedBox3d::intersects(
    ...
    MATHEX_SCALAR* pEntryDistance,
    MATHEX_SCALAR* pExitDistance,
    Mathex::TouchingRule rule) const
{
    bool result = false;
