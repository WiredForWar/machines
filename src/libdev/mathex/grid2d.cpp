...
MexGrid2d::MexGrid2d(size_t nXCells, size_t nYCells, MATHEX_SCALAR xSize, MATHEX_SCALAR ySize, const MexPoint2d& origin)
    : nXCells_(nXCells)
    , nYCells_(nYCells)
    , origin_(origin)
    , xCellSize_(xSize)
...
void MexGrid2d::intersect(const MexLine2d& line, Cells* pCells, Points* pPoints)
{
    ...
    MexPoint2d end1((line.end1().x() - origin_.x()) / xCellSize_, (line.end1().y() - origin_.y()) / yCellSize_);

    MexPoint2d end2((line.end2().x() - origin_.x()) / xCellSize_, (line.end2().y() - origin_.y()) / yCellSize_);
    ...
    {
        convertedI = MexPoint2d((*j).x() * xCellSize_ + origin_.x(), (*j).y() * yCellSize_ + origin_.y());
    ...
    for (Points::iterator i = internalPoints.begin(); i != internalPoints.end(); ++i, ++j)
    {
        ...
        if (j != internalPoints.end())
        {
            convertedJ = MexPoint2d((*j).x() * xCellSize_ + origin_.x(), (*j).y() * yCellSize_ + origin_.y());
            ...
            else
            {
                ...
                // legal cells.
                size_t xCellIndex = ((*i).x() + (*j).x()) / 2.0;
                size_t yCellIndex = ((*i).y() + (*j).y()) / 2.0;
