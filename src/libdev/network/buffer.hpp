...
template <class BITWISE> UtlBuffer<BITWISE>::~UtlBuffer()
{
    _DELETE_ARRAY(data_);
}

template <class BITWISE> UtlBuffer<BITWISE>& UtlBuffer<BITWISE>::operator=(const UtlBuffer& rhs)
{
    if (this != &rhs)
    {
        if (size() != rhs.size())
...
template <class BITWISE> typename UtlBuffer<BITWISE>::size_type UtlBuffer<BITWISE>::size() const
{
    return size_;
}

template <class BITWISE> BITWISE* UtlBuffer<BITWISE>::data()
{
    return data_;
};

template <class BITWISE> const BITWISE* UtlBuffer<BITWISE>::data() const
{
    return data_;
};
...
template <class BITWISE> bool operator<(const UtlBuffer<BITWISE>& a, const UtlBuffer<BITWISE>& b)
{
