...
NetNode::~NetNode()
{
    ...
    CB_NetNode_DEPIMPL();
    NETWORK_STREAM(
        "NetNode::~NetNode() id " << pNodeUid_ << " network nodes size "
                                  << NetNetwork::instance().pNetINetwork_->nodes_.size() << std::endl);
    if (RecRecorder::instance().state() != RecRecorder::PLAYING)
    {
        NetAppSession& thisAppSession = NetNetwork::instance().session();
        RecRecorder::instance().recordingAllowed(false);
...
void NetNode::ping(const NetNodeUid& recipientUid)
{
    ...
    if (RecRecorder::instance().state() != RecRecorder::PLAYING)
    {
        ...
        {
            NETWORK_STREAM(
                "Ping map Key: " << (*i).first << std::endl
                                 << "Ping map value: " << (*i).second << std::endl);
        }
        ...
        NETWORK_STREAM(
            "lastPingTime " << pingInfo.lastPingTime_ << " timeForNewPing " << timeForNewPing << "( thisTime "
                            << thisTime << " lastTimePingAsked_ " << pingInfo.lastTimePingAsked_ << ") " << std::endl);
        if (pingInfo.lastPingTime_ != -1 or timeForNewPing)
        {
            ...
            // We will time out if the ping times are greater than
            // NetNetwork::instance().netINetwork().deterministicTimeoutPeriod()
            if (NetNetwork::instance().deterministicPingDropoutAllowed())
            {
                ...
                if (pingInfo.lastProperPingTime_ == -1
                    and (pingInfo.lastTimePingAsked_ - pingInfo.lastTimePingSet_) > deterministicDropOut)
                {
...
NetNode::NetNodeStatus NetNode::convertStatus(unsigned int castThis) const
{
    ...
    {
        //      NETWORK_STREAM( "NetNode:: convertStatus : return code is " << returnVal << " dperror " << hr << "
        //      Stack:" << std::endl );
...
ostream& operator<<(ostream& o, const NetNode::NetNodeStatus& t)
{
    switch (t)
    {
        case NetNode::NETNODE_OK:
            o << "NETNODE_OK";
            break;
        case NetNode::NETNODE_UNDEFINEDERROR:
            o << "NETNODE_UNDEFINEDERROR";
            break;
        case NetNode::NETNODE_PLAYERCREATIONERROR:
            o << "NETNODE_PLAYERCREATIONERROR";
            break;
        case NetNode::NETNODE_INTERNALDATAERROR:
            o << "NETNODE_INTERNALDATAERROR";
            break;
        case NetNode::NETNODE_INVALIDINTERNALDATA:
            o << "NETNODE_INVALIDINTERNALDATA";
            break;
        case NetNode::NETNODE_NETWORKERROR:
            o << "NETNODE_NETWORKERROR";
            break;
        case NetNode::NETNODE_INVALIDSUPPLIEDDATA:
            o << "NETNODE_INVALIDSUPPLIEDDATA";
            break;
        case NetNode::NETNODE_NETWORKACCESSREFUSED:
            o << "NETNODE_NETWORKACCESSREFUSED";
            break;
        case NetNode::NETNODE_MESSAGETOOBIG:
            o << "NETNODE_MESSAGETOOBIG";
            break;
        case NetNode::NETNODE_INVALIDPLAYER:
            o << "NETNODE_INVALIDPLAYER";
            break;
        case NetNode::NETNODE_PLAYERDOESNOTEXIST:
            o << "NETNODE_PLAYERDOESNOTEXIST";
            break;
        case NetNode::NETNODE_CONNECTIONLOST:
            o << "NETNODE_CONNECTIONLOST";
            break;
        case NetNode::NETNODE_UNSUPPORTED:
            o << "NETNODE_UNSUPPORTED";
            break;
    }
    return o;
}
...
void NetNode::actuallyTransmitCompoundMessage(NetCompoundMessage* pCompoundMessage)
{
    ...
    NET_ANALYSIS_STREAM(
        "COMPOUND,out," << NetNetwork::instance().netINetwork().timer_.time() << "," << pCompoundMessage->length()
                        << "," << (int)pCompoundMessage->subMessages() << std::endl);
}
