...
RenDriverSelectorImpl::RenDriverSelectorImpl(const RenDisplay* const display)
    : display_(display)
    , currentDDrawDriver_(nullptr)
    , automaticDDrawDriver_(nullptr)
    , currentD3dDriver_(nullptr)
    , automaticD3dDriver_(nullptr)
{
}
...
{
    RenDriverPtr bestDriverInList = nullptr;
...
void RenDriverSelector::buildD3dDrivers()
{
    ...
    RENDER_STREAM(
        "Current Direct3d Driver (" << currentD3dDriver_->name() << ", " << currentD3dDriver_->description() << ")"
                                    << std::endl);
...
RenDriverSelector::ReturnValue RenDriverSelector::useDDrawDriver(const RenDriverPtr& driver)
{
    ...
    if (result == SUCCEEDED)
    {
        ...
        {
            RENDER_STREAM(
                "Rebuilding list of Direct3d drivers for direct draw driver ("
                << currentDDrawDriver_->name() << ", " << currentDDrawDriver_->description() << ")" << std::endl);
            buildD3dDrivers();
        }
    }
    else
    {
        RENDER_STREAM(
            "Failed to find DirectDraw driver (" << driver->name() << ", " << driver->description() << ")"
                                                 << std::endl);
    }
...
RenDriverSelector::ReturnValue RenDriverSelector::useD3dDriver(const RenDriverPtr& driver)
{
    ...
    {
        RENDER_STREAM(
            "New current Direct3d driver (" << currentD3dDriver_->name() << ", " << currentD3dDriver_->description()
                                            << ")" << std::endl);
    }
    else
    {
        RENDER_STREAM(
            "Failed to find Direct3d driver (" << driver->name() << ", " << driver->description()
                                               << ") in list of drivers " << std::endl);
    }
...
void RenDriverSelector::streamOut(ostream& o) const
{
    ...
    o << "Automatic DirectDraw driver "
      << " (" << automaticDDrawDriver_->name() << ", " << automaticDDrawDriver_->description() << ")" << std::endl;
    o << "Current DirectDraw driver "
      << " (" << currentDDrawDriver_->name() << ", " << currentDDrawDriver_->description() << ")" << std::endl;
    ...
    if (display_)
    {
        ...
        o << "Automatic Direct3d driver "
          << " (" << automaticD3dDriver_->name() << ", " << automaticD3dDriver_->description() << ")" << std::endl;
        o << "Current Direct3d driver "
          << " (" << currentD3dDriver_->name() << ", " << currentD3dDriver_->description() << ")" << std::endl;
    }
}
