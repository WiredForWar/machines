...
#include "render/internal/FixedPointVec.hpp"
...
{
public:
    using Base = ctl_min_memory_vector<RenIVertex>;
    ...
    using Normals = ctl_min_memory_vector<RenIVec3FixPtS0_7>;
    ...
    RenIVertexIntensities* intensityMap() { return (intensityMap_.isDefined()) ? &(*intensityMap_) : nullptr; }

    void createIntensityMap(); // POST(intensityMap());
    void resetAllIntensities();

    const RenIVertexMaterials* materialMap() const { return (materialMap_.isDefined()) ? &(*materialMap_) : nullptr; }

    RenIVertexMaterials* materialMap() { return (materialMap_.isDefined()) ? &(*materialMap_) : nullptr; }
