...
bool RenILinesDiagnostic::pixelsMatchColour(
    const RenSurface& surface,
    const RenSurface::Points& points,
    RenColour colour)
{
    int index = 0;
...
void RenILinesDiagnostic::testLines(const RenSurface& surface)
{
    ...
    bool isOffsetSorted
        = pixelsMatchColour(surface, horizontalStockVertices(OFFSET_SORTED), lineColour_); // isOffsetSorted = true;
