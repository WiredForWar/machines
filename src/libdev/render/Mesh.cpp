...
#include "render/internal/MeshLoader.hpp"
#include "render/internal/GXMeshLoader.hpp"
#include "xin/XFileHelper.hpp"

#include "render/internal/MeshFactory.hpp"
...
{
public:
    MaterialEnumerator(RenMaterialVec* s)
        : set_(s)
    {
        PRE(s);
    }
    void operator()(const RenIMaterialGroup* group) { set_->push_back(group->material()); }
...
{
public:
    MaterialSetChanger(const RenMaterialVec* s)
        : set_(s)
        , iterator_(s->begin())
    {
        PRE(s);
...
public:
    ...
    void operator()(const RenITriangleGroup* group) { count_ += group->nTriangles(); }
...
// Whether a group can go into the merged run. The depth shader takes only a
// position and two matrices -- no texture, no colour, and an empty fragment
// shader -- so a material reaches the depth map through this one decision and
// nothing else. Semi-transparent surfaces are out, since they must not cast a
// full opaque shadow.
//
// Anything that later needs material state while the depth map is being drawn
// has to be excluded here and drawn on its own: alpha tested cutouts, whose
// shadow should have the holes their texture does, are the case to expect.
inline bool castsMergedShadow(const RenITriangleGroup* group)
{
...
template <class T> class GroupRenderFunctor
{
public:
...
template <class T> void GroupRenderFunctor<T>::operator()(const T* group) const
{
...
// A functor which can be used by std::for_each to draw a RenIMaterialGroup.
template <class T> class GroupRenderFunctorMatOverride
{
public:
...
template <class T> void GroupRenderFunctorMatOverride<T>::operator()(const T* group) const
{
    RenIDeviceImpl* devImpl = RenIDeviceImpl::currentPimpl();
...
void RenMesh::render(const MexTransform3d& world, const RenScale& scale) const
{
    ...
    if (ttfs_.size() > 0)
    {
        ...
        while (tIt != ttfs_.end())
        {
            ...
            poly->render(m, averageScale, nullptr);
            ++tIt;
        }
    }
    ...
    if (stfps_.size() > 0)
    {
        ...
        while (sIt != stfps_.end())
        {
            ...
            spinPoly->render(world, scale, nullptr);
            ++sIt;
        }
...
void RenMesh::render(const MexTransform3d& world, const RenMaterialVec* mats, const RenScale& scale) const
{
    ...
    if (ttfs_.size() > 0)
    {
        ...
        {
            const RenTTFPolygon* poly = *tIt;
            poly->render(m, *matIt, averageScale, nullptr);
            ++tIt;
            ++matIt;
        }
    }
    ...
    if (stfps_.size() > 0)
    {
        ...
        while (stIt != stfps_.end())
        {
            ...
            sPoly->render(world, *matIt, scale, nullptr);
            ++stIt;
            ++matIt;
        }
...
static bool findMatch(const RenIVertexData& vertices, int i1, int i2, int i3, int i4, int i5, int i6, int& m)
{
    bool match;

    const MATHEX_SCALAR epsilon = 0.0005;

    bool v14 = fabs(vertices[i1].x - vertices[i4].x) < epsilon && fabs(vertices[i1].y - vertices[i4].y) < epsilon
        && fabs(vertices[i1].z - vertices[i4].z) < epsilon;
    bool v15 = fabs(vertices[i1].x - vertices[i5].x) < epsilon && fabs(vertices[i1].y - vertices[i5].y) < epsilon
        && fabs(vertices[i1].z - vertices[i5].z) < epsilon;
    bool v16 = fabs(vertices[i1].x - vertices[i6].x) < epsilon && fabs(vertices[i1].y - vertices[i6].y) < epsilon
        && fabs(vertices[i1].z - vertices[i6].z) < epsilon;

    bool v24 = fabs(vertices[i2].x - vertices[i4].x) < epsilon && fabs(vertices[i2].y - vertices[i4].y) < epsilon
        && fabs(vertices[i2].z - vertices[i4].z) < epsilon;
    bool v25 = fabs(vertices[i2].x - vertices[i5].x) < epsilon && fabs(vertices[i2].y - vertices[i5].y) < epsilon
        && fabs(vertices[i2].z - vertices[i5].z) < epsilon;
    bool v26 = fabs(vertices[i2].x - vertices[i6].x) < epsilon && fabs(vertices[i2].y - vertices[i6].y) < epsilon
        && fabs(vertices[i2].z - vertices[i6].z) < epsilon;

    bool v34 = fabs(vertices[i3].x - vertices[i4].x) < epsilon && fabs(vertices[i3].y - vertices[i4].y) < epsilon
        && fabs(vertices[i3].z - vertices[i4].z) < epsilon;
    bool v35 = fabs(vertices[i3].x - vertices[i5].x) < epsilon && fabs(vertices[i3].y - vertices[i5].y) < epsilon
        && fabs(vertices[i3].z - vertices[i5].z) < epsilon;
    bool v36 = fabs(vertices[i3].x - vertices[i6].x) < epsilon && fabs(vertices[i3].y - vertices[i6].y) < epsilon
        && fabs(vertices[i3].z - vertices[i6].z) < epsilon;
...
)
{
    ...
    bool xEqConst
        = (fabs(vertices[i1].x - vertices[i2].x) < tollerance) && (fabs(vertices[i1].x - vertices[i3].x) < tollerance);
    bool yEqConst
        = (fabs(vertices[i1].y - vertices[i2].y) < tollerance) && (fabs(vertices[i1].y - vertices[i3].y) < tollerance);
    bool zEqConst
        = (fabs(vertices[i1].z - vertices[i2].z) < tollerance) && (fabs(vertices[i1].z - vertices[i3].z) < tollerance);
    ...
    ctl_vector<MexPoint2d>& uvs)
{
    ...
    bool xEqConst
        = (fabs(vertices[i1].x - vertices[i2].x) < epsilon) && (fabs(vertices[i1].x - vertices[i3].x) < epsilon);
    bool yEqConst
        = (fabs(vertices[i1].y - vertices[i2].y) < epsilon) && (fabs(vertices[i1].y - vertices[i3].y) < epsilon);
    bool zEqConst
        = (fabs(vertices[i1].z - vertices[i2].z) < epsilon) && (fabs(vertices[i1].z - vertices[i3].z) < epsilon);
...
{
    PRE(nVertices != nullptr);
    PRE(vertices != nullptr);
    PRE(faceData != nullptr);
    ...
    for (ctl_min_memory_vector<RenITriangleGroup*>::const_iterator i = triangles_.begin(); i != triangles_.end(); ++i)
    {
