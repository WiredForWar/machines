...
public:
    ...
    public:
        ...
        SpinAxis& operator=(const SpinAxis& copyMe)
        {
            base_ = copyMe.base_;
            direction_ = copyMe.direction_;
            return *this;
        }
        bool operator==(const SpinAxis& that)
        {
            return (this->base_ == that.base_) && (this->direction_ == that.direction_);
        }
    ...
    using Vertices = ctl_vector<MexPoint2d>;
...
private:
    ...
    void
    spinPlan(const MexPoint3d& cameraPosition, const MexPoint3d& origin, const MexVec3& xBasis, MexVec3* pNormal) const;
