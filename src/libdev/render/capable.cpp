...
void RenCapabilities::parseCardSpecificationFile(const SysPathName& fileName)
{
    ...
    while (! parser.finished())
    {
        if (parser.tokens()[0] == "MEMORY_AVAILABLE_FOR_TEXTURES")
        {
            ...
            RENDER_STREAM(
                "Read Memory available for textures: " << maxAvailableTextureMemory_ << " ("
                                                       << (maxAvailableTextureMemory_ / 0x100000) << " MBytes)"
                                                       << std::endl);
        }
        else if (parser.tokens()[0] == "MEMORY_AVAILABLE_FOR_DISPLAY")
        {
            ...
            RENDER_STREAM(
                "Read Memory available for display: " << maxAvailableDisplayMemoryAfterTextures_ << " ("
                                                      << (maxAvailableDisplayMemoryAfterTextures_ / 0x100000)
                                                      << " MBytes)" << std::endl);
        }
        else
        {
...
void RenCapabilities::updateMaxAvailableDisplayMemoryAfterTextures()
{
    ...
    {
        RENDER_STREAM(
            "Attempting to read from specification file "
            << cardSpecsFile << " the max amount of memory that can be used for display" << std::endl);
    ...
    if (maxAvailableDisplayMemoryAfterTexturesDirty_)
    {
        ...
        if (supportsSharedVideoMemory_)
        {
            ...
            RENDER_STREAM(
                "Total system memory available for textures: " << totalTextureSystemMemory << " ("
                                                               << (totalTextureSystemMemory / 0x100000) << " MBytes)"
                                                               << std::endl);
            uint32_t videoMemoryRequiredForTextureSet = (memoryRequiredByTexSet > totalTextureSystemMemory)
                ? memoryRequiredByTexSet - totalTextureSystemMemory
                : 0;
            RENDER_STREAM(
                "Video memory required by texture set: " << videoMemoryRequiredForTextureSet << " ("
                                                         << (videoMemoryRequiredForTextureSet / 0x100000) << " MBytes)"
                                                         << std::endl);
            maxAvailableDisplayMemoryAfterTextures_ = totalVideoMemory_ - videoMemoryRequiredForTextureSet;
            RENDER_STREAM(
                "Total video memory: " << totalVideoMemory_ << " (" << (totalVideoMemory_ / 0x100000) << " MBytes)"
                                       << std::endl);
        }
        else
        {
            ...
            RENDER_STREAM(
                "Total texture memory: " << totalTextureMemory_ << " (" << (totalTextureMemory_ / 0x100000)
                                         << " MBytes)" << std::endl);
            RENDER_STREAM(
                "Total video memory: " << totalVideoMemory_ << " (" << (totalVideoMemory_ / 0x100000) << " MBytes)"
                                       << std::endl);
...
uint32_t RenCapabilities::maxAvailableDisplayMemoryAfterTextures() const
{
    ...
    RENDER_STREAM(
        "Memory available for display " << maxAvailableDisplayMemoryAfterTextures_ << " ("
                                        << (maxAvailableDisplayMemoryAfterTextures_ / 0x100000) << " MBytes)"
                                        << std::endl);
...
uint32_t RenCapabilities::memoryRequiredByTextureSet() const
{
    ...
    RENDER_INDENT(-2);
    RENDER_STREAM(
        "Memory required by texture set: " << result << " (" << (result / 0x100000) << " MBytes)" << std::endl);
    return result;
}
...
bool RenCapabilities::supports4MBytesTextureSet() const
{
    ...
    RENDER_STREAM(
        "Memory required by the four Mbytes texture set: " << memoryRequiredBy4MBytesTexSet << " ("
                                                           << (memoryRequiredBy4MBytesTexSet / 0x100000) << " MBytes)"
                                                           << std::endl);
...
void RenCapabilities::updateMaxAvailableTextureMemory()
{
    ...
    if (cardSpecsFile.existsAsFile())
    {
        RENDER_STREAM(
            "Attempting to read from specification file "
            << cardSpecsFile << " the max amount of memory that can be used by textures" << std::endl);
        parseCardSpecificationFile(cardSpecsFile);
    }
    ...
    if (maxAvailableTextureMemoryDirty_)
    {
        ...
        {
            RENDER_STREAM(
                "Total shared texture memory: " << totalTextureMemory_ << " (" << (totalTextureMemory_ / 0x100000)
                                                << " MBytes)" << std::endl);
            RENDER_STREAM(
                "Memory required for lowest allowed display: "
                << minDisplayMemory_ << " (" << (minDisplayMemory_ / 0x100000) << " MBytes)" << std::endl);
...
uint32_t RenCapabilities::maxAvailableTextureMemory() const
{
    ...
    RENDER_STREAM(
        "Memory available for textures: " << maxAvailableTextureMemory_ << " ("
                                          << (maxAvailableTextureMemory_ / 0x100000) << " MBytes)" << std::endl);
