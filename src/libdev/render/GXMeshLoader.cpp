...
void RenIGXMeshLoader::deleteAll()
{
    ...
    for (GXMeshesMap::iterator meshesIt = meshesMap_.begin(); meshesIt != meshesMap_.end(); ++meshesIt)
    {
    ...
    for (GXHierMap::iterator hierIt = hierarchyMap_.begin(); hierIt != hierarchyMap_.end(); ++hierIt)
    {
...
GXMesh* RenIGXMeshLoader::load(const SysPathName& pathName, const std::string& combinedName)
{
    ...
    {
        DEBUGFACT(
            "Warning : no mesh has been loaded by RenIGXMeshLoader::load(" << pathName << "," << combinedName << ")"
                                                                           << std::endl);
    }

    POST(result == nullptr || result->name().str() == meshName);
...
void RenIGXMeshLoader::load(const SysPathName& pathName, RenHierarchyBuilder* pBuilder)
{
    ...
    PRE_INFO(pathName);
    PRE(pBuilder != nullptr);

    DEBUGFACT(
        "RenIGXMeshLoader::load(" << pathName.pathname() << ","
                                  << "RenHierarchyBuilder*)" << std::endl);
    fileBeingLoaded_ = pathName;
...
void RenIGXMeshLoader::load(const SysPathName& pathname)
{
    ...
    for (int i = 0; i < meshesBeingLoaded_->size(); ++i)
    {
        ...
        {
            DEBUGFACT(
                "Set name of mesh having id " << (*meshesBeingLoaded_)[i]->id() << " to "
                                              << (*meshesBeingLoaded_)[i]->name() << std::endl);
        }
    }
...
GXError RenIGXMeshLoader::loadObjects(GXFile& gxfile)
{
    ...
    gxerror = gxfile.readGXIdPos(gxid, true, true, nullptr);
    if (gxerror == NO_ERROR_AGT)
    {
...
GXMesh* RenIGXMeshLoader::searchForMesh(const std::string& searchedName, const GXMeshes& meshes)
{
