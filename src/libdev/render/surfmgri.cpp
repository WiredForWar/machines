...
RenSurface RenISurfaceManagerImpl::createDisplaySurface(const RenDevice* dev, RenI::DisplayType type)
{
    TEST_INVARIANT;
...
void RenISurfaceManagerImpl::useDevice(const RenDevice* dev, RenI::UpdateType updateType)
{
    PRE(dev);
    TEST_INVARIANT;
...
Ren::TexId RenISurfaceManagerImpl::addAnonymousEntry(RenISurfBody* newSurf)
{
    ...
    ASSERT(entries_[newId] == nullptr, logic_error());
