...
// static
RenMeshInstance* RenMeshInstance::createShared(const SysPathName& path, const std::string& mesh, double scale)
{
...
{
    RenMaterialVec* pMaterialVec = (materialVecPtr_.isDefined() ? &(*materialVecPtr_) : nullptr);
    RenUVTransform* pTexAnimation = (texAnimationPtr_.isDefined() ? &(*texAnimationPtr_) : nullptr);
...
void RenMeshInstance::addMaterials(RenMaterialSet* pSet) const
{
    ...
    for (RenMaterialVec::iterator i = materialSetAPtr->begin(); i != materialSetAPtr->end(); ++i)
    {
        pSet->insert(*i);
    }
}
...
void perWrite(PerOstream& ostr, const RenMeshInstance& instance)
{
    ...
    ostr << (instance.scale_ == RenMeshInstance::unityScale() ? nullptr : instance.scale_);
}
