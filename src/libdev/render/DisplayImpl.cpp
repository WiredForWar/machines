...
void RenISavedArea::save(RenSurface& surf, const Ren::Rect& rect)
{
    ...
    Ren::Painter(save_).blit(original_, area_);
}
...
void RenISavedArea::restore()
{
    ...
    Ren::Painter(original_).blit(save_, {}, Ren::Point{area_.originX, area_.originY});
}
...
void RenISavedArea::restoreTo(RenSurface* dest, const RenSurface& front, Ren::Rect srcArea, int destX, int destY) const
{
...
void RenIDisplay::drawCursor(RenSurface& backBuf)
{
    if (cursor_)
    {
        ...
        Ren::Painter(backBuf).blit(bm, {}, dstRect.origin());
...
void RenIDisplay::restoreUnderCursor(RenSurface* dest, const RenSurface& front, Ren::Rect srcArea, int destX, int destY)
    const
{
    PRE(dest);
