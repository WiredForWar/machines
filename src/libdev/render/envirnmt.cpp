...
directional_light:  [on|off]
  ...
  colour:           (0.7, 0.7, 0.7)
*/
static void parseDirLight(UtlLineTokeniser& parser, RenDevice* device, ctl_pvector<RenDirectionalLight>* dirs)
{
    ...
    while (parsingLight && !parser.finished())
    {
        ...
        else if (parser.tokens()[0] == "attenuation:")
        {
            ...
            if (tmp > 0)
                ...
                RENDER_STREAM("  Read bad point light attenuation " << tmp << " from " << parser.fileName() <<
std::endl);
        }
        else if (parser.tokens()[0] == "colour:")
        {
    ...
    while (parsingLight && !parser.finished())
    {
        ...
        else if (parser.tokens()[0] == "constant_attenuation:")
        {
            ...
            if (tmp >= 0)
                ...
                RENDER_STREAM(
                    "  Read bad " << type << " attenuation " << tmp << " from " << parser.fileName() << std::endl);
        }
    ...
    ctl_pvector<RenUniformLight>* uniforms)
{
    ...
    while (!parser.finished())
    {
        ...
        {
            RENDER_STREAM(
                "  Read bad input keyword " << parser.tokens()[0] << " from " << parser.fileName() << std::endl);
            parser.parseNextLine();
        }
    }
}
