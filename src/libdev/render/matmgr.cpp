...
void RenIMatManager::addBody(RenIMatBody* body)
{
    ...
    {
        Bodies::iterator it = find(bodies_->begin(), bodies_->end(), (RenIMatBody*)nullptr);
...
void RenIMatManager::checkCoplanarValue(short newPriority)
{
    ...
    ASSERT(
        maxPriority_ - minPriority_ <= maxDifference,
        "All inter-mesh coplanar values must differ by no more than 16.");
}
