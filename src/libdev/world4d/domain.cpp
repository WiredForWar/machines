...
bool W4dDomain::intersects(const W4dEntity& entity) const
{
    ...
    if (find(intersectingEntities_.begin(), intersectingEntities_.end(), &entity) == intersectingEntities_.end())
        result = false;
...
void W4dDomain::intersects(W4dEntity* pEntity, bool intersects)
{
    ...
    entityPosition = find(intersectingEntities_.begin(), intersectingEntities_.end(), pEntity);
...
W4dPortal* W4dDomain::addPortal(W4dDomain* pPartner, const MexQuad3d& aperture, const W4dTransform3d& newLocalTransform)
{
    TEST_INVARIANT;
    CB_W4dDomain_DEPIMPL();
...
{
    CB_W4dDomain_DEPIMPL();
    PRE(pObserver != nullptr);
    TEST_INVARIANT;

    // Check to see if the observer is not already an observer before adding it
    W4dObservers::iterator it = find(observers_.begin(), observers_.end(), pObserver);
    ...
    W4dEntityFilter* pEntityFilter)
{
    ...
    if (recursiveFindNearerEntity(this, line, limit, checkId, accuracy, ppEntity, &foundDistance, pEntityFilter))
    {
        result = true;
        limit = foundDistance;
    }
    ...
    for (W4dPortals::iterator it = portals_.begin(); not pEntityFilter->isFinished() and it != portals_.end(); ++it)
    {
        ...
        if (pPortal->apertureIntersectsLine(line, limit))
        {
            ...
            if (pNextDomain->findNearerEntity(line, limit, checkId, accuracy, ppEntity, &foundDistance, pEntityFilter))
            {
                result = true;
                limit = foundDistance;
                break;
