...
std::ostream& operator<<(std::ostream& o, const W4dCompositeImpl& t)
{
    ...
    o << "W4dCompositeImpl " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "W4dCompositeImpl " << static_cast<const void*>(&t) << " end" << std::endl;
...
void W4dCompositeImpl::parseModel(const SysPathName& directoryName, UtlLineTokeniser* pParser, W4dLOD id)
{
    ...
    if (not empty_)
    {
        ...
        buildHierarchy(nullptr, builder.pRoot(), distance, id);
    }
}
            ...
            and parser.tokens()[1] == animationName)
        {
            ...
            while (not parser.finished())
            {
                if (parser.tokens()[0] == "Animation")
                {
                    ...
                    ASSERT(foundLink && pLink != nullptr, "Unknown link name in XAN file");
...
void W4dCompositeImpl::addEmptyMeshes(double distance, W4dLOD id)
{
    for (W4dComposite::W4dLinks::const_iterator i = links_.begin(); i != links_.end(); ++i)
    {
        ...
        pEntity->add(nullptr, distance, id);
    }
}
...
bool W4dCompositeImpl::findLink(const string& name, W4dLink** ppLink) const
{
    *ppLink = nullptr;
    ...
    return *ppLink != nullptr;
}

PhysMotionPlanPtr W4dCompositeImpl::makeAnimationPlan(
    const KeyFrameOrientations& orientations,
...
// local
static void
linearInterpolation(const double& y1, const double& y2, const double& x1, const double& x2, const double& x, double* y)
{
    double s = x2 - x1;
    double b = (y2 - y1) / s;
...
MexQuaternion
W4dCompositeImpl::interpolatedOrientation(const KeyFrameOrientations& orientations, const KeyFrameId& id) const
{
...
void perWrite(PerOstream& ostr, const W4dCompositeImpl& t)
{
    ...
    bool hasHeldEntities = t.pHeldEntities_ != nullptr;
