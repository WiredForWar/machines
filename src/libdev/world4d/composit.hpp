...
public:
    ...
    using HeldEntities = ctl_pvector<W4dEntity>;

    ~W4dComposite() override;
    ...
    void
    plan(const W4dCompositePlan& newPlan, const PhysAbsoluteTime& startTime, uint nRepetitions = 0, uint animId = 0);
    ...
    void temporaryScale(const RenScale& scale, PropogateScaleFlag propogate) override;

    // Clear any temporary scale override.
    // propogate indicates whether any descendants should also be cleared.
    // Inherited from W4dEntity.
    // Default implementation is adequate for composites. Clients don't override.
    void clearTemporaryScale(PropogateScaleFlag propogate) override;
    ...
    // Inherited from W4dEntity.
    // Default implementation is adequate for composites. Clients don't override.
    void propogateScalePlan(
        const W4dScalePlanPtr& planPtr,
        const PhysAbsoluteTime& startTime,
        uint repetitions = 0,
        uint animId = 0) override;
    ...
    void addMaterials(RenMaterialSet* pSet) const override;

    //  Remap the materials that are being used
    void materialMap(const RenMaterialMap& map) override;
...
private:
    ...
    RenderType render(const W4dCamera& camera, W4dComplexity& complexity) override;
    ...
    void temporaryScale(const RenNonUniformScale& scale, const MexTransform3d& useTransform) override;
    ...
    void propogateScalePlan(
        ...
        uint repetitions,
        uint animId,
        const MexTransform3d& offsetTransform) override;
