...
public:
    ...
    void plan(const W4dEntityPlan& rhs, const PhysAbsoluteTime& startTime, uint nRepetitions = 0, uint animId = 0);
    ...
    PlanState transform(const PhysAbsoluteTime& time, MexTransform3d* pResult, uint* pNObsolete) const;
    // PRE( hasMotionPlan() )
    ...
    // pNObsolete returns the count of obsolete plans at time.
    PlanState visible(const PhysAbsoluteTime& time, bool* pVisible, uint* pNObsolete) const;
    ...
    // pNObsolete returns the count of obsolete plans at time.
    PlanState
    materialVec(const PhysAbsoluteTime& time, W4dLOD lodId, Ren::MaterialVecPtr* pMaterialVecPtr, uint* pNObsolete)
        const;
    // PRE( hasMaterialPlan() )
    ...
    // Attach a scale plan
    void
    scalePlan(const W4dScalePlanPtr& planPtr, const PhysAbsoluteTime& startTime, uint repetitions = 0, uint animId = 0);

    // Returns the state of any plans defining scale.
    // If state is DEFINED updates pEntityScale with the value defined at time.
    // pNObsolete returns the count of obsolete plans at time.
    PlanState scale(const PhysAbsoluteTime& time, W4dEntityScale* pEntityScale, uint* pNObsolete) const;
    //  PRE( hasScalePlan() );
    ...
    void uvPlan(const W4dUVPlanPtr& planPtr, const PhysAbsoluteTime& startTime, uint repetitions = 0, uint animId = 0);
