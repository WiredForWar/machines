...
void W4dComposite::sharedCopyCtor(const W4dComposite& copyMe, CopyLights copyLights)
{
    ...
    for (W4dLinks::const_iterator i = copyMe.links().begin(); i != copyMe.links().end(); ++i)
    {
        ...
        {
            W4dLinks::const_iterator parent = find(copyMe.links().begin(), copyMe.links().end(), copyMeLink.pParent());
...
void W4dComposite::renderLinks(const W4dCamera& camera, MATHEX_SCALAR range, const W4dComplexity& complexity) const
{
    CB_W4dComposite_DEPIMPL();
...
// virtual
W4dEntity::RenderType W4dComposite::render(const W4dCamera& camera, W4dComplexity& complexity)
{
    CB_W4dComposite_DEPIMPL();
    PRE_INFO(name());
...
void W4dComposite::plan(
    const W4dCompositePlan& newPlan,
...
void W4dComposite::updateCompositeBoundingVolume()
{
    ...
    if (pHeldEntities_ != nullptr)
    {
        ...
        for (HeldEntities::iterator it = heldEntities.begin(); it != heldEntities.end(); ++it)
        {
            ...
            const MexAlignedBox3d* heldVol = nullptr;
            if (held.isComposite())
...
// virtual
void W4dComposite::temporaryScale(const RenNonUniformScale& scale, const MexTransform3d& useTransform)
{
    CB_W4dComposite_DEPIMPL();
    ...
    if (links_.size() != 0)
    {
        ...
        for (W4dLinks::iterator it = links_.begin(); it != links_.end(); ++it)
        {
            ...
            if (isNonUniform)
            {
                ...
                pLink->propogateScalePlan(planPtr, startTime, repetitions, animId, toLinkTransform);
            }
            else
            {
