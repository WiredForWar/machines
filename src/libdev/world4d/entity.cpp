...
bool W4dEntity::intersects(const W4dDomain& domain) const
{
    ...
    bool result = intersectingDomains_ != nullptr;
    if (result)
    {
        W4dDomains::const_iterator domainPosition;

        domainPosition = find(intersectingDomains_->begin(), intersectingDomains_->end(), &domain);
...
{
    CB_W4dEntity_DEPIMPL();
    return intersectingDomains_ != nullptr;
}
...
// purposes.
{
    ...
    if (childList_)
    {
        for (W4dEntities::iterator i = childList_->begin(); i != childList_->end(); ++i)
        {
            (*i)->debugOutput(ostr);
        }
    }
...
// virtual
W4dEntity::RenderType W4dEntity::render(const W4dCamera& camera, W4dComplexity& complexity)
{
    TEST_INVARIANT;
    PRE_INFO(name());
    PRE(hasMesh());
...
// virtual
void W4dEntity::renderAtRange(const W4dCamera& camera, const W4dDistance& range, const W4dComplexity& complexity)
{
    TEST_INVARIANT;
    ...
    if (rangeToMesh(range, &pMesh, &lodId, complexity))
    {
        ...
        if (pPlan_)
        {
            ...
            if (pPlan_->hasMaterialPlan() and pPlanUpdateTimes_->materialTime != timeNow)
            {
                ...
                if (pPlan_->materialVec(timeNow, lodId, &materialVecPtr, &nObsoletePlans) == W4dEntityPlan::DEFINED)
                {
...
bool W4dEntity::rangeToMesh(
    const W4dDistance& rangeConst,
    RenMeshInstance** ppMesh,
    W4dLOD* pLodId,
    const W4dComplexity& complexity) const
{
    ...
    if (complexity.enabled())
    {
        ...
        if (complexity.enabledLOD())
            modifiedRange = complexity.modifiedRange();
    ...
    POST(implies(drawMesh, *ppMesh != nullptr));
    return drawMesh;
}
...
void W4dEntity::replaceMeshes(const W4dEntity& entity)
{
    ...
    size_t id = 0;
    for (W4dEntityImpl::Meshes::const_iterator i = entity.entityImpl().meshes_->begin();
         i != entity.entityImpl().meshes_->end();
         ++i)
    {
...
void W4dEntity::updateLODUVTransforms(const Ren::UVTransformPtr& uvTransformPtr, W4dLOD maxLOD, W4dLOD exceptId)
{
    CB_W4dEntity_DEPIMPL();
    // Iterate through the levels of detail
    W4dLOD lodId = 0;
...
// virtual
void W4dEntity::temporaryScale(const RenNonUniformScale& scale, const MexTransform3d& useTransform)
{
    CB_W4dEntity_DEPIMPL();
    ...
    uint animId)
{
    ...
    if (childList_ != nullptr)
    {
        for (W4dEntities::iterator it = childList_->begin(); it != childList_->end(); ++it)
        {
            ...
            {
                // Use special recursive function to propogate non-uniform scaling
                child.propogateScalePlan(planPtr, startTime, repetitions, animId, child.localTransform());
            }
            else
            {
