    ...
    MATHEX_SCALAR duration)
{
    ...
    else
    {
        ...
        W4dSoundManagerImpl::EntitySound newSound(
            pEntity,
            pData->maxPercentageVolume(),
            pData->nearAttenuationDistance(),
            pData->farAttenuationDistance(),
            0,
            pData->path(),
            (Repetition)0,
            realStartTime,
...
void W4dSoundManager::play(const W4dEntity* pEntity, uint id, const PhysAbsoluteTime& startTime, size_t repetition)
{
    CB_DEPIMPL(MexPoint3d, lastEarLocation_);
    CB_DEPIMPL(MexTransform3d, lastEarTransform_);
    CB_DEPIMPL(bool, hasPendingSound_);
        ...
        if ((*it).pEntity_ == pEntity)
        {
            ...
            else
            {
                ...
                SOUND_STREAM(
                    ...
                    << _CONST_CAST(void*, reinterpret_cast<const void*>(pEntity)) << std::endl);
            }
            sounds_.erase(it);
    ...
    else if (lastUpdateTime != timeNow)
    {
        ...
        for (W4dSoundManagerImpl::EntitySounds::iterator it = sounds_.begin(); it != sounds_.end(); it = nextIt)
        {
            ...
            else
            {
                ...
                MATHEX_SCALAR s = 1
                    - ((sqrDistance - squareMinAudibleDistance)
                       / (squareMaxAudibleDistance - squareMinAudibleDistance));

                const int FUDGE = 100;
                entitySound.selectionValue_ = entitySound.priority_ * FUDGE * (s / sqrDistance);
            }
...
void W4dSoundManager::readSoundDefinitionFile(const SysPathName& definitionFileName)
{
    ...
    while (! parser.finished())
    {
        ...
        if (availableSounds[registeredSounds_[tokens[0]]])
        {
            ...
            SOUND_STREAM(
                "This " << availableSounds[registeredSounds_[tokens[0]]]->path() << " already in "
                        << registeredSounds_[tokens[0]] << std::endl);
        }
        else
        {
