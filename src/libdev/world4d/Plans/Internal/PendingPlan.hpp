...
{
public:
    W4dPendingPlan(PLAN_TYPE newPlanPtr, PhysAbsoluteTime newStartTime, uint newNRepetitions, uint animId)
        : planPtr_(newPlanPtr)
    ...
    {
        return startTime_ == rhs.startTime_ and nRepetitions_ == rhs.nRepetitions_ and planPtr_ == rhs.planPtr_
            and animId_ == rhs.animId_;
    }

    bool operator<(const W4dPendingPlan<PLAN_TYPE>& rhs) { return startTime_ < rhs.startTime_; }

    //  Default assignment operator is adequate

    PLAN_TYPE planPtr() const { return planPtr_; }

    PhysAbsoluteTime startTime() const { return startTime_; }

    uint nRepetitions() const { return nRepetitions_; }

    void nRepetitions(uint nNewRepetitions) { nRepetitions_ = nNewRepetitions; }

    uint animId() const { return animId_; }
...
template <class PLAN_TYPE> W4dPendingPlan<PLAN_TYPE>::W4dPendingPlan(PerConstructor)
{
}
...
#include "world4d/Plans/Internal/PendingPlan.ctf"
// #endif
