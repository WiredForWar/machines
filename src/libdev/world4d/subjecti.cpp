...
W4dSubjectImpl::~W4dSubjectImpl()
{
    ...
    if (physObjectPtr_.isDefined())
        physObjectPtr_->subject(nullptr);
...
void W4dSubjectImpl::initialiseDomainObservers()
{
    ...
    for (W4dObservers::const_iterator cit = domainObservers.begin(); cit != domainObservers.end(); ++cit)
    {
...
void W4dSubjectImpl::updateDomainObservers(const W4dEntity& oldParent)
{
    ...
    W4dDomain* pOldDomain
        = (oldParent.isDomain() ? ((W4dEntity&)oldParent).getAsDomain() : oldParent.containingDomain());
    ...
    if (pOldDomain != pNewDomain)
    {
        ...
        for (W4dObservers::const_iterator cit = newDomainObservers.begin(); cit != newDomainObservers.end(); ++cit)
        {
            W4dObserver* pObserver = *cit;
        ...
        for (W4dObservers::const_iterator cit = oldDomainObservers.begin(); cit != oldDomainObservers.end(); ++cit)
        {
            W4dObserver* pObserver = *cit;
        ...
        for (W4dObservers::iterator it = allObservers.begin(); it != allObservers.end(); ++it)
        {
