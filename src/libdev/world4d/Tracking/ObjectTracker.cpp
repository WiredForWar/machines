...
#ifndef _INLINE
#include "world4d/Tracking/ObjectTracker.ipp"
#endif

PER_DEFINE_PERSISTENT(W4dObjectTracker);

W4dObjectTracker::W4dObjectTracker(
    const W4dEntity& parent,
    const MexPoint3d& localOrigin,
    const W4dEntity& target,
    const MexPoint3d& targetOffset,
    MATHEX_SCALAR maxAbsYawSine,
    MATHEX_SCALAR maxAbsPitchSine,
    MATHEX_SCALAR minLength,
    MATHEX_SCALAR maxLength,
...
MATHEX_SCALAR W4dObjectTracker::track(MexTransform3d* pTrackTransform) const
{
    ...
    if (now != lastUpdateTime_)
    {
        ...
        pParent_->globalTransform().transform(translateTransform, &nonConstThis->trackerTransform_);
        ...
        MATHEX_SCALAR targetDistance
            = track(nowTarget, &rotationTransform, maxAbsYawSine_, maxAbsPitchSine_, yawDeltaSine_, yawDeltaCosine_);
        ...
        nonConstThis->trackerLength_
            = (targetDistance > maxLength_ ? maxLength_ : (targetDistance < minLength_ ? minLength_ : targetDistance));
    }
...
MATHEX_SCALAR W4dObjectTracker::track(
    const MexPoint3d& target,
    MexTransform3d* pTrackTransform,
    MATHEX_SCALAR maxAbsYawSine,
    MATHEX_SCALAR maxAbsPitchSine,
    MATHEX_SCALAR yawDeltaSine,
    MATHEX_SCALAR yawDeltaCosine)
{
