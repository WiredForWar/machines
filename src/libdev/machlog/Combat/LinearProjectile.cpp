...
MATHEX_SCALAR MachLogLinearProjectile::rangeToStationaryObject(
    ...
    MATHEX_SCALAR range)
{
    PRE(fabs(range - globalStartPosition.euclidianDistance(globalEndPosition)) < MexEpsilon::instance());
...
{
    PRE(fabs(range3d - globalStartPosition.euclidianDistance(globalEndPosition)) < MexEpsilon::instance());
    ...
    if (nPossibleCollisions != 0)
    {
        ...
        while (! haveCollision && nPossibleCollisions != 0)
        {
            ...
            MATHEX_SCALAR segmentLength
                = (isStationary ? range3d - segmentStartDistance : range3d * (timeIncrement / duration));
            ...
            for (PhysAbsoluteTime testTime = earliestTime; testTime < endTime; testTime += timeIncrement)
            {
                ...
                MATHEX_SCALAR sqrDistanceFromObject
                    = segmentStartPoint.sqrEuclidianDistance(possibleHitActor.position());
...
PhysRelativeTime MachLogLinearProjectile::update(const PhysRelativeTime& /* maxCPUTime */, MATHEX_SCALAR)
{
...
void MachLogLinearProjectile::checkForDamage(
    MATHEX_SCALAR range,
    MachLogLinearProjectile::DamageCurveShape curve,
    MachPhys::WeaponType wt)
{
    MachActor* pActor = nullptr;
    if (currentlyAttached_)
        pActor = pOwner_;
    genericCheckForDamage(globalDestructionPosition(), range, curve, damage(), wt, nullptr, pActor);
    isDead(true);
}
                    ...
                    case R_SQUARED_DAMAGE:
                        {
                            if (boxSqrRange > epsilon)
                            {
                                ...
                                if (sqrRange <= 1)
                                    iActor.beHit(damage, wt, nullptr, &line);
                                else
