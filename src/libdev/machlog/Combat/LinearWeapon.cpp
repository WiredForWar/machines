...
void MachLogLinearWeapon::doFire(const MexPoint3d& position)
{
    ...
    W4dDomain* pDomain = MachLogPlanetDomains::pDomainPosition(position, 0, &localTileTransform);
    ...
    if (isPublishing && network.remoteStatus(owner().race()) == MachLogNetwork::LOCAL_PROCESS)
    {
        broker.sendEchoLinearProjectileMessage(
            ...
            0,
            launchTime,
            physWeapon().weaponData().nRoundsPerBurst(),
            extraData,
            position);
...
void MachLogLinearWeapon::doFire(MachActor* pTarget, const MachLogFireData& fireData)
{
    ASSERT(
        ...
        "MachLogLinearWeapon::doFire has received a dummy fireData struct as parameter.");
    ...
    MachLogLinearWeapon::CreateHolder createHolder)
{
    ...
    {
        MachPhysLinearProjectile* pPhysLinearProjectile
            = createPhysLinearProjectile(launchTime, i, pParent, cactor.physObject(), targetOffset);
...
void MachLogLinearWeapon::doEchoFireForTerrain(
    MexPoint3d position,
    const PhysAbsoluteTime& launchTime,
    int nRounds,
    const MachLogMessageBroker::ProjectileDestroyData& extraData,
    MachLogLinearWeapon::CreateHolder createHolder)
{
    ...
    {
        MachPhysLinearProjectile* pPhysLinearProjectile
            = createPhysLinearProjectile(launchTime, i, pParent, *pDomain, localTileTransform.position());
        PhysAbsoluteTime destructTime = extraData[i]->destroyAtTime_;
        PhysRelativeTime relTime = pPhysLinearProjectile->beDestroyedAt(destructTime, extraData[i]->strikeType_);
        if (createHolder == MachLogLinearWeapon::CREATE_HOLDER)
        {
    ...
    const MexPoint3d& targetOffset)
{
    ...
    // dtlw("", true );
    MachPhysLinearProjectile* result
        = pPhysLinearWeapon_->createProjectile(burstStartTime, index, pParent, target, targetOffset);
...
void MachLogLinearWeapon::dtlw(const char* text, bool start)
{
    ...
    else
    {
        ...
        DEBUG_STREAM(
            DIAG_MISC,
            owner().id() << "," << physWeapon().type() << "," << text << "," << endTime - startTime << std::endl);
    }
}
