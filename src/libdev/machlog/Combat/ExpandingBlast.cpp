    ...
    , pImpl_(new MachLogExpandingBlastImpl(pOwnerActor, pRace, startPosition, blastRadius, canHitAirUnits))
{
    ...
    {
        id(MachLogRaces::allocateId(pRace->race() * 250 + 1, pRace->race() * 250 + 251));
    }
    else
    {
        id(MachLogRaces::allocateId());
    }
...
// virtual
PhysRelativeTime
MachLogExpandingBlast::update(const PhysRelativeTime& /*maxCPUTime*/, MATHEX_SCALAR /*clearanceFromDisplayedVolume*/)
{
    ...
    ASSERT(
        firstWaveFinishTime() <= destructionTime(),
        "A wave's end time must be on or before the total destruction time.");
    ASSERT(
        secondWaveFinishTime() <= destructionTime(),
        "A wave's end time must be on or before the total destruction time.");
    ASSERT(
        thirdWaveFinishTime() <= destructionTime(),
        "A wave's end time must be on or before the total destruction time.");
...
std::ostream& operator<<(std::ostream& o, const MachLogExpandingBlast& t)
{
    ...
    o << "MachLogExpandingBlast " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogExpandingBlast " << static_cast<const void*>(&t) << " end" << std::endl;
...
void MachLogExpandingBlast::buildVictimMachinesList()
{
    ...
    {
        ASSERT(
            pConstruction_->physConstruction().hasInterior(),
            "Something clearly going wrong here - expected construction to have interior.");
...
bool MachLogExpandingBlast::centreOfActorWithinRadius(const MachActor& victim, MATHEX_SCALAR distanceFromEpicentre)
    const
{
    bool result = false;
...
bool MachLogExpandingBlast::constructionWithinRadius(
    const MachLogConstruction& victim,
    MATHEX_SCALAR distanceFromEpicentre) const
{
    return MachLogSpacialManipulation::withinRangeOfMexAlignedBox2d(
        victim.globalBoundary(),
        startPosition(),
        distanceFromEpicentre);
}
...
// virtual
bool MachLogExpandingBlast::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
    CB_MachLogExpandingBlast_DEPIMPL();
        ...
        case W4dSubject::DELETED:
            {
                ...
                if (pConstruction_ && pSubject == (W4dSubject*)pConstruction_)
                {
                    ...
                    pConstruction_ = nullptr; // NULLify dangling pointer
                }
            }
            break;
