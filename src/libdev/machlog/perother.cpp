...
std::ostream& operator<<(std::ostream& o, const MachLogOtherPersistence& t)
{
    ...
    o << "MachLogOtherPersistence " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogOtherPersistence " << static_cast<const void*>(&t) << " end" << std::endl;
...
void MachLogOtherPersistence::setUpLocalVectorFromViews() const
{
    ...
    for (MachPhys::Race i : MachPhys::AllRaces)
    {
        if (MachLogRaces::instance().raceObjects(i).size() > 0)
            for (MachLogRaces::Squadrons::iterator j = MachLogRaces::instance().squadrons(i).begin();
                 j != MachLogRaces::instance().squadrons(i).end();
                 ++j)
    ...
    for (MachLogConstructionTree::ConstructionItems::iterator i
         = MachLogRaces::instance().constructionTree().constructionItems().begin();
         i != MachLogRaces::instance().constructionTree().constructionItems().end();
         ++i)
...
void perRead(PerIstream& istr, MachLogOtherPersistence& per)
{
    ...
    for (MachLogOtherPersistence::ResearchItems::iterator i = per.researchItems_.begin(); i != per.researchItems_.end();
         ++i)
