...
#include "machlog/Persistence/PersistConstructions.hpp"
...
std::ostream& operator<<(std::ostream& o, const MachLogPersistence& t)
{
    ...
    o << "MachLogPersistence " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogPersistence " << static_cast<const void*>(&t) << " end" << std::endl;
...
void perRead(PerIstream& istr, MachLogPersistence& per)
{
    ...
    /*
    for( MachLogPlanet::Sites::const_iterator i = MachLogPlanet::instance().sites().begin(); i !=
    MachLogPlanet::instance().sites().end(); ++i )
    {
