...
// static
MachPhysSpyLocator*
MachLogSpyLocator::pNewPhysSpyLocator(Level hwLevel, Level swLevel, MachLogRace* pRace, const MexPoint3d& location)
{
...
// virtual
PhysRelativeTime MachLogSpyLocator::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR clearance)
{
...
void MachLogSpyLocator::executeTheft()
{
    ...
    if (isDownloading())
    {
        ASSERT(
            pResItemCurrentlyStealing_ != nullptr,
            "MachLogSpyLocator::executeTheft - isDownloading() == TRUE yet null pointer assignment for research "
            "item pointer pResItemCurrentlyStealing_");

        int potentialResearchUnits
            = (int)((float)data().downloadRate() / 60.0 * (SimManager::instance().currentTime() - lastUpdateTime_));
        if (potentialResearchUnits > 0)

        {
            HAL_STREAM(
                "going to advanceResearchItem " << (void*)pResItemCurrentlyStealing_ << " by " << potentialResearchUnits
                                                << std::endl);
        ...
        // else potentialResearchUnits must have == 0, and the difference between "now" and "the last time I was
        // updated" is
...
int MachLogSpyLocator::nResItemsICouldSteal(MachPhys::Race otherRace, MachPhys::HardwareLabSubType hardwareLabSubType)
{
    ...
    for (MachLogResearchTree::ResearchItems::iterator i
         = MachLogRaces::instance().researchTree().researchItems().begin();
         i != MachLogRaces::instance().researchTree().researchItems().end();
         ++i)
    {
...
MachLogResearchItem*
MachLogSpyLocator::stealNewResearchItem(MachPhys::Race otherRace, MachPhys::HardwareLabSubType hardwareLabSubType)
{
    ...
    ASSERT(
        nPossibleResItems > 0,
        "MachLogSpyLocator::stealNewResearchItem: The acquisition of a ResItem shouldn't be being made if the "
        "number of stealable items == 0");
         ...
         ++i)
    {
        if (stealable((**i), otherRace, hardwareLabSubType))
        {
            // If there are e.g. five stealable techs, the chance of the first being chosen is 1/5, then if it isn't
            // selected, the second tech has a 1/4 chance etc. As 4/5 X 1/4 == 1/5, 4/5 X 3/4 X 1/3 == 1/5 etc, all
            // techs have an equal chance of being selected. The last eligible tech is clearly guaranteed (1/1) to be
            // selected if none of the others were.
    ...
    ASSERT(
        false,
        "MachLogSpyLocator::stealResearchItem(): the probability function should ALWAYS be returning the last "
        "possible tech if it reaches it.");
...
bool MachLogSpyLocator::stealable(
...
void perRead(PerIstream& istr, MachLogSpyLocator& actor)
{
    ...
    MachPhysSpyLocator* pPhysSpyLocator
        = MachLogSpyLocator::pNewPhysSpyLocator(base1.hwLevel(), base1.swLevel(), &actor.logRace(), trans.position());
