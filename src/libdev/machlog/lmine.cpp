...
MachLogLandMine::MachLogLandMine(MachLogRace* pRace, const MexPoint3d& location)
...
MachLogLandMine::MachLogLandMine(MachLogRace* pRace, const MexPoint3d& location, UtlId withId)
...
// static
MachPhysLandMine* MachLogLandMine::pNewLandMine(MachLogRace* pRace, const MexPoint3d& location)
{
                     ...
                     ++i)
                {
                    if ((*i)->objectIsMachine())
                    {
                        ...
                        if (not(machine.machineIsGlider()) and not machine.isDead())
                        {
                            ...
                            {
                                // well, we're certainly close enough. But is there another landmine that's actually
                                // closer?
                                if (thisIsClosestLandmineTo(machine.position()))
                                {
                                    ...
                                    //                                  position(), 10,
                                    //                                  MachLogLinearProjectile::CONSTANT_DAMAGE, 80,
...
// virtual
void MachLogLandMine::beHitWithoutAnimation(
    int damage,
