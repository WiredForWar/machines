...
#else
#define LOG_STREAM(exp) MOTSEQ_STREAM(exp), pImpl_->logStreamOn_ ? (pImpl_->logStream_ << exp, 0) : 1

#define LOG_STREAM2(exp, pThis)                                                                                        \
    MOTSEQ_STREAM(exp), pThis->pImpl_->logStreamOn_ ? (pThis->pImpl_->logStream_ << exp, 0) : 1

#define LOG_INDENT(n) (MOTSEQ_INDENT(n), pImpl_->logStreamOn_ ? (pImpl_->indent(n), 0) : 1)

#define LOG_INDENT2(n, pThis) (MOTSEQ_INDENT(n), pThis->pImpl_->logStreamOn_ ? (pThis->pImpl_->indent(n), 0) : 1)

#define LOG_INSPECT(exp) MOTSEQ_INSPECT(exp), pImpl_->logStreamOn_ ? (INSPECT_ON(pImpl_->logStream_, exp), 0) : 1
...
#define CHANGE_STATE(STATE, REASON)                                                                                    \
    LOG_STREAM(SimManager::instance().currentTime() << "  "),                                                          \
        ...
        internalState_ = STATE, pImpl_->reasonForStateChange(REASON)
