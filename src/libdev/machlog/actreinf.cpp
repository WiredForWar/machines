...
std::ostream& operator<<(std::ostream& o, const MachLogReinforcementsAction& t)
{
    ...
    o << "MachLogReinforcementsAction " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogReinforcementsAction " << static_cast<const void*>(&t) << " end" << std::endl;
    ...
    for (ProductionList::iterator i = productionList_.begin(); i != productionList_.end(); ++i)
    {
        ...
        if (assignToSquadron_)
        {
            ...
            for (MachLogRaces::Squadrons::iterator i = MachLogRaces::instance().squadrons(race_).begin();
                 i != MachLogRaces::instance().squadrons(race_).end();
                 ++i)
            {
...
void MachLogReinforcementsAction::parseFromScenario(MachPhys::Race race, UtlLineTokeniser* pParser)
{
