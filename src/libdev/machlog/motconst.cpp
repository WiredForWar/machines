...
void MachLogZenithCameraMotionConstraint::move(MexTransform3d& trans, PhysMotion& motion, double elapsedTime)
{
    ...
    if (! isUpToDate_)
    {
        ...
        MexTransform3d work1(
            MexEulerAngles(heading_, pitchAngle_, 0.0),
        ...
        MATHEX_SCALAR nearClipDistance
            = mexInterpolate(zoomDistance_, minZoomDistance, maxZoomDistance, minNearClipDistance, maxNearClipDistance);
...
void MachLogZenithCameraMotionConstraint::setCameraDataFromTransform()
{
    ...
    ASSERT(pCameraDomain != nullptr, "Camera start position not in a domain");
    ...
    bool foundCentre = pCameraDomain->findNearerEntity(
        lineOfSight,
        limit,
...
MachLogThirdPersonCameraConstraint::MachLogThirdPersonCameraConstraint(
    PhysThirdPerson* pThirdPerson,
    MachLogCamera* pCamera)
