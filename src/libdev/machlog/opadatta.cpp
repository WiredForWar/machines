...
void MachLogAdminAttackOperation::doOutputOperator(std::ostream& o) const
{
    ...
    if (pDirectObject_)
    {
        o << " pDirectObject_.objecttype() (" << pDirectObject_->id() << ") " << pDirectObject_->objectType()
          << std::endl;
    }
    else
    {
...
PhysRelativeTime MachLogAdminAttackOperation::doUpdate()
{
    ...
    if (pActor_->squadron())
    {
                    ...
                    if (not machineAsCanAttack.hasCurrentTarget())
                    {
                        ...
                        else
                        {
                                ...
                                > recalculateRatio)
                            {
                                ...
                                // yes, this is a bit shoddy - ideally, we'd want a lastTargetPosition_ for each
                                // machine. This is a bit of a hack and the 0.45 ratio heuristic will break down when
                                // large numbers of squad machines attacking this target are spread out over the map
        ...
        if (actors.size() > 0)
            MachLogGroupSimpleMove groupMove(
                actors,
                points,
...
// virtual
bool MachLogAdminAttackOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
