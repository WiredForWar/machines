...
// static
bool MachLogSpacialManipulation::inExtendedMexAlignedBox2d(
...
// static
bool MachLogSpacialManipulation::getNearestFreeSpacePoint(
    const MexTransform3d& orTrans,
    const MATHEX_SCALAR innerRadius,
    MATHEX_SCALAR clearance,
    MexPoint2d* pResult,
    MATHEX_SCALAR distanceToOuterRadius,
    UseWhichPattern pattern)
{
    ...
    if (!(useRadius < (innerRadius + distanceToOuterRadius)))
    {
        HAL_STREAM(
            ...
            << orTrans << " innerRadius " << innerRadius << " clearance " << clearance << " use innerRadius "
            << useRadius << std::endl
            << "Has NOT found a point" << std::endl);
        foundSpace = false;
    }
    else
    {
        HAL_STREAM(
            ...
            << orTrans << " innerRadius " << innerRadius << " clearance " << clearance << " use innerRadius "
            << useRadius << std::endl
            << "Has found point at " << *pResult << std::endl);
        foundSpace = true;
    }
...
// static
bool MachLogSpacialManipulation::getNextInCircularPattern(
    const MexTransform3d& orTrans,
    const MATHEX_SCALAR radius,
    MATHEX_SCALAR clearance,
    MexPoint2d* pResult)
{
...
std::ostream& operator<<(std::ostream& o, const MachLogSpacialManipulation& t)
{
    ...
    o << "MachLogSpacialManipulation " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogSpacialManipulation " << static_cast<const void*>(&t) << " end" << std::endl;
...
// static
void MachLogSpacialManipulation::getCollidingObjects(
    const MexAlignedBox2d& /*testingBox*/,
    ctl_vector<UtlId>* /*pCollidingObjectIds*/)
{
}
...
// static
MexRadians
MachLogSpacialManipulation::angleToTurnToFace(const MexTransform3d& fromTransform, const MexPoint3d& toPosition)
{
...
// static
void MachLogSpacialManipulation::genericCheckForIntersections(
    const MexPoint3d& epicentre,
    MATHEX_SCALAR range,
...
void MachLogSpacialManipulation::heightRange(
    ...
    int hwLevel,
    const MexTransform3d& transform,
    MATHEX_SCALAR* pMinHeight,
    MATHEX_SCALAR* pMaxHeight)
{
    PRE_INFO(type);
    PRE(MachLogMapper::isConstruction(type));

    const MachPhysConstructionData& constructionData
        = MachPhysData::instance().constructionData(MachLogMapper::mapToPhysConstruction(type), subType, hwLevel);
...
// static
MexTransform3d MachLogSpacialManipulation::sourceToDestination3dTransform(
    const MexPoint2d& sourcePosition,
    const MexPoint2d& destinationPosition,
    UseWhichEnd whichEnd)
{
...
// static
bool MachLogSpacialManipulation::pointInsideConfigSpaceIsFree(
    const MexPoint2d& testPoint,
    MATHEX_SCALAR clearance,
    const PhysConfigSpace2d& cs)
{
    bool result = false;
...
// static
bool MachLogSpacialManipulation::getNextInFlankingPattern(
    const MexTransform3d& orTrans,
    const MATHEX_SCALAR radius,
    MATHEX_SCALAR clearance,
    MexPoint2d* pResult)
...
// static
bool MachLogSpacialManipulation::withinRangeOfMexAlignedBox2d(
    const MexAlignedBox2d& box,
    const MexPoint2d& pos,
    MATHEX_SCALAR range)
{
    bool result = false;
