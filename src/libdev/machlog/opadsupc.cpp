...
PhysRelativeTime MachLogAdminSuperConstructOperation::doUpdate()
{

    ASSERT(
        constructionArray_.size() != 0,
        "MachLogAdminSuperConstructOperation::doUpdate : we shouldn't have an empty construction vector here.");
    ASSERT(
        nConstructorsRemaining() != 0,
        "MachLogAdminSuperConstructOperation::doUpdate : we shouldn't have an empty constructors vector here.");
    ...
    MachLogConstruction* pTargetConstruction = nullptr;
    ...
    {
        case MachLogOperation::CONSTRUCT_OPERATION:
            subOperation(pActor_, std::make_unique<MachLogAdminConstructOperation>(pActor_, pTargetConstruction));
            break;

        case MachLogOperation::DECONSTRUCT_OPERATION:
            subOperation(pActor_, std::make_unique<MachLogAdminDeconstructOperation>(pActor_, pTargetConstruction));
            break;

        case MachLogOperation::CAPTURE_OPERATION:
            subOperation(pActor_, std::make_unique<MachLogAdminCaptureOperation>(pActor_, pTargetConstruction));
            break;

        case MachLogOperation::REPAIR_OPERATION:
            subOperation(pActor_, std::make_unique<MachLogAdminRepairOperation>(pActor_, pTargetConstruction));
            break;
...
// virtual
bool MachLogAdminSuperConstructOperation::beNotified(
    W4dSubject* pSubject,
    ...
    int clientData)
{
    ...
    ASSERT(
        i != constructionArray_.end(),
        "MachLogAdminSuperConstructOperation::beNotified : notified by an unexpected subject.");
