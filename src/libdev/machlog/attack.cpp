...
void MachLogAttackOperation::doOutputOperator(std::ostream& o) const
{
    ...
    if (pDirectObject_)
    {
        ...
        o << " SQR range to target " << pActor_->position().sqrEuclidianDistance(directObject().position())
          << std::endl;
        if (targetBehindCover_)
        {
            ...
            {
                o << " object is cons. obstacle polygon " << pDirectObject_->asConstruction().obstaclePolygonId()
                  << std::endl;
            }
        }
        ...
        if( pAgg_ )
        {
            if( pAgg_->hasCurrentTarget() )
            {
                if( not pAgg_->currentTarget().isDead() )
                    o << " (" << pAgg_->currentTarget().id() << ") [" << pAgg_->currentTarget().objectType() << "] " <<
        std::endl; else
                {
        ...
        if( pAdm_ )
        {
            if( pAdm_->hasCurrentTarget() and MachLogRaces::instance().objectExists( &pAdm_->currentTarget() ) )
                o << " (" << pAdm_->currentTarget().id() << ") [" << pAdm_->currentTarget().objectType() << "] " <<
        std::endl; else
            {
...
bool MachLogAttackOperation::doStart()
{
    ...
    if (pDirectObject_)
    {
        // try to leave if our target's outside, we're inside, and our target's not a machine who's already on his way
        // in (If he is, best to wait for him.)
...
MexRadians angleToTurnToFace(const MachLogMachine& actor, const MexPoint3d& pos);
...
PhysRelativeTime MachLogAttackOperation::doUpdate()
{
    ...
    MachLogConstruction* pTargetBuilding = nullptr;
    ...
    if (target.objectIsMachine())
    {
        ...
        MachLogConstruction* pAttackerBuilding = nullptr;
    ...
    for (MachLogCanAttack::Weapons::const_iterator it = weapons.begin(); it != weapons.end(); ++it)
    {
        ...
        {
            HAL_STREAM(
                " weapon is a normal type firing pDirectObject is at this point " << (void*)pDirectObject_
                                                                                  << std::endl);
            ...
            MachLogCanAttack::WeaponDisposition disposition = MachLogCanAttack::rangeData(
                globalWeaponTransform,
                targetObject,
...
// virtual
bool MachLogAttackOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
    CB_DEPIMPL(MachLogMachine*, pActor_);
    CB_DEPIMPL(MachActor*, pDirectObject_);
...
void MachLogAttackOperation::stopTargetting()
{
    ...
    if (&(attacker.currentTarget()) == pDirectObject_)
        attacker.currentTarget(nullptr);
}
    ...
    */
    ...
    else
    {
        ...
        if (not pActor_->asCanAttack().withinSqrMaximumWeaponRange(sqrDistanceBetweenDestinationAndTarget))
        {
            ...
            MATHEX_SCALAR distanceTargetMovedSinceAttackerChoseDestination
    ...
    if (recalculateDestination)
        interval
            = calculateNewApproachPoint(targetPositionNow, attackerPositionNow, chasingEvader, distanceToTargetNow);
...
MachLogAttackOperation::Action MachLogAttackOperation::actionForDisposition(
    MachLogCanAttack::WeaponDisposition worstDisposition,
    const MachLogCanAttack& attacker) const
{
    CB_DEPIMPL(MachLogMachine*, pActor_);
    CB_DEPIMPL(MachActor*, pDirectObject_);
...
PhysRelativeTime MachLogAttackOperation::calculateNewApproachPoint(
    const MexPoint2d& targetPositionNow,
    const MexPoint2d& attackerPositionNow,
    bool chasingEvader,
    MATHEX_SCALAR distanceToTargetNow)
{
    ...
    else
    {
        ...
        if (distanceToTargetNow > maxWeaponRange)
        {
            ...
            else
            {
                ...
                MATHEX_SCALAR sqrDistanceToTargetIn1Second
                    = attackerPositionNow.sqrEuclidianDistance(attackerPositionInOneSecond);
                ...
                {
                    // Target is not moving away from us, therefore do not attempt to close to any significant degree
                    // within our maximum weapon range
                    desiredRange = maxWeaponRange * 0.97;
                }
            }
        }
    ...
    MexTransform3d testTransform = MachLogSpacialManipulation::sourceToDestination3dTransform(
        targetPositionNow,
        attackerPositionNow,
        MachLogSpacialManipulation::SOURCE_END_IS_CENTRE);
    ...
    bool foundSpace = MachLogSpacialManipulation::getNearestFreeSpacePoint(
        testTransform,
        desiredRange,
        useClearance,
        &attackPoint,
