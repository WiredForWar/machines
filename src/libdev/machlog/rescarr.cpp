...
bool MachLogResourceCarrier::hasSmeltingBuilding() const
{
    ...
    return pDestinationSmeltingBuilding_ != nullptr;
}
...
void MachLogResourceCarrier::clearSmeltingBuilding()
{
    ...
    if (pDestinationSmeltingBuilding_ != nullptr)
    {
...
bool MachLogResourceCarrier::movingToNextSupplier()
{
    ...
    ASSERT(
        currentElement_ < suppliers_.size(),
        "The currentElement_ has been allowed to exceed the suppliers size somewhere.");
...
void MachLogResourceCarrier::reorderTransportRoute()
{
    ...
    for (Suppliers::iterator iReordered = reorderedSuppliers.begin(); iReordered != reorderedSuppliers.end();
         ++iReordered)
    {
...
MachPhys::BuildingMaterialUnits MachLogResourceCarrier::amountCarried() const
{
    CB_MachLogResourceCarrier_DEPIMPL();
    return amountCarried_;
}

void MachLogResourceCarrier::setAmountCarried(MachPhys::BuildingMaterialUnits carried)
{
    CB_MachLogResourceCarrier_DEPIMPL();
...
void MachLogResourceCarrier::assignResourceCarrierTask(MachLogResourceCarrier* obj)
{
    ...
    else
    {
        ...
        if (nMyTotalMines > 0 and (smelters.size() > 0 or pods.size() > 0))
        {
            ...
            {
                // right, we need to select a subset of these mines. Let's try going for, I dunno, roughly a third of
                // them.
...
// virtual
bool MachLogResourceCarrier::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
                ...
                case MachLog::RACE_CHANGED:
                    {
                        ...
                        else
                        {
                            ...
                            // But if it is a pod or smelter, we should not use it as a supplier as we'd only be
                            // stealing from ourselves - it should now be removed from the list of suppliers.
                ...
                case MachLog::MINE_CHANGED_OPERATIONAL_STATUS:
                    {
                        ...
                        {
                            ASSERT(
                                (*i)->objectType() == MachLog::MINE,
                                "Received MINE_CHANGED_OPERATIONAL_STATUS message from a non-mine actor.");
...
// virtual
PhysRelativeTime MachLogResourceCarrier::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR clearance)
{
...
void perRead(PerIstream& istr, MachLogResourceCarrier& actor)
{
    ...
    MachPhysResourceCarrier* pPhysResourceCarrier = MachLogResourceCarrier::pNewPhysResourceCarrier(
        base1.hwLevel(),
        base1.swLevel(),
        &actor.logRace(),
        trans.position());
