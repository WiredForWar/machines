...
std::ostream& operator<<(std::ostream& o, const MachLogTaskStateAction& t)
{
    ...
    o << "MachLogTaskStateAction " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogTaskStateAction " << static_cast<const void*>(&t) << " end" << std::endl;
...
MachLogTaskStateAction::newFromParser(SimCondition* pCondition, bool enabled, UtlLineTokeniser* pParser)
{
    ...
    for (std::size_t i = 0; i < pParser->tokens().size(); ++i)
    {
        ...
        if (token == "NUMBER")
        {
            ...
            ASSERT(
                taskIndex <= MachLogRaces::instance().databaseHandler().nTasksInCurrentScenario(),
