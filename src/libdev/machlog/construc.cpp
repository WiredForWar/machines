...
void MachLogConstructor::constructing(bool newIsConstructingValue, UtlId constructionId)
{
    ...
    if (isConstructing_)
    {
        ...
        physConstructor().startConstructing(
            constructionActor.asConstruction().physConstruction(),
...
void perRead(PerIstream& istr, MachLogConstructor& actor)
{
    ...
    MachPhysConstructor* pPhysConstructor = MachLogConstructor::pNewPhysConstructor(
        actor.subType_,
        base1.hwLevel(),
        base1.swLevel(),
        &actor.logRace(),
