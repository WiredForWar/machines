    ...
    MoveResult* pMoveResult)
{
    ...
    if (motionChunkIds_.size() + 2 < MachLogMessageBroker::maximumMotionChunks() && swerveRecursionCount_ < 3)
    {
        ...
        for (size_t i = 0; i < swervePoints.size() && ! foundPath; ++i)
        {
            ...
            size_t pointsReserved = reservePath(
                startTransform,
                orientationInfo,
                path,
                startTime,
    ...
    while (! finished)
    {
        ...
        *pMoveResult = reserveChunk(
        ...
        else if (*pMoveResult == COLLIDED_WITH_MACHINE)
        {
            ...
            motionPossible = trySwerveMove(
                fromTransform,
                orientationInfo,
                toPoint,
                startTime,
                startSpeed,
                pCollisionInfo->collisionObjectId(),
                collisionPoint,
                *pActualFinalState,
                ignoreIds,
                pActualFinalState,
...
MachLogMachineMotionSequencer::MoveResult MachLogMachineMotionSequencer::reserveChunk(
    ...
    MoveInfoUPtr* pMoveInfoUPtr)
{
    ...
    if (motionPossible)
    {
        ...
        if (anyMotionChunkStationary(chunkIntersections, &chunkIntersectionData))
        {
            ...
            const PhysMotionChunk& chunk = currentConfigSpace().motionChunk(chunkIntersectionData.collisionChunkId());

            LOG_INSPECT(chunk);

            const MachLogCollisionInfo info(chunk.startPoint(), chunkIntersectionData.collisionObjectId());
    ...
    if (motionPossible)
    {
        ...
        MachPhysMachine::ProfilePtr profilePtr
            = pPhysMachine_->profile(fromTransform, orientationInfo, toPoint, *MachLogPlanet::instance().surface());
    ...
    if (motionPossible)
    {
        ...
        NETWORK_STREAM(
            "(" << pLogMobile_->id() << ") !! Added motion chunk with id " << motionChunkId << "callstack\n"
...
size_t MachLogMachineMotionSequencer::reserveMotionChunks(ObjectId* pCollisionObjectId)
{
    CB_MachLogMachineMotionSequencerData_DEPIMPL();
    ...
    size_t nPointsReached = reservePath(
        fromTransform,
    ...
    ctl_vector<size_t>* pNChunks)
{
    ...
    while (pNChunks->size() > 0 && moveInfos_.back().finalState() != MachPhys::AT_REST)
    {
        ...
        MoveResult moveResult = reserveChunk(
            fromTransform,
            MachPhysMachine::USE_ORIENTATION,
            toPoint,
            startTime,
    ...
    MoveInfoUPtr* pInfoAPtr) const
{
    ...
    *pInfoAPtr = pPhysMachine_->moveInfo(profilePtr, startTime, startSpeed, maxSpeed, finalState);
    ...
    bool motionPossible = pConfigSpace_->add(
        motionChunk,
        pLogMobile_->id(),
        ignoreIds,
        chunkIntersections,
...
MachLogMachineMotionSequencer::MoveInfoUPtr MachLogMachineMotionSequencer::getShuffleMoveInfo(
    ...
    const MexTransform3d& targetTransform)
{
    ...
    for (MachPhysPlanetSurface::Profile::iterator i = profilePtr->begin(); i != profilePtr->end(); ++i)
    {
    ...
    moveInfoAPtr
        = pPhysMachine_
              ->moveInfo(profilePtr, SimManager::instance().currentTime(), startSpeed, maxSpeed, MachPhys::AT_REST);
...
void MachLogMachineMotionSequencer::getIntersectingIds(PhysConfigSpace2d::ObjectIds* pIntersectingIds) const
{
    ...
    pConfigSpace_->findIntersections(motionChunk, ObjectId::invalidId(), &chunkIntersections);
