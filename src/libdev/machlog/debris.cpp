    ...
    , pBoundary_(_NEW(MexAlignedBox2d(boundary)))
{
    ...
    HAL_STREAM(
        "(" << id() << ") MLDebris::CTOR location " << location << " boundary " << boundary << " quant " << quantity
            << std::endl);
...
// virtual
PhysRelativeTime MachLogDebris::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR clearence)
{
...
void perRead(PerIstream& istr, MachLogDebris& actor)
{
    ...
    MachPhysDebris* pPhysDebris
        = MachLogDebris::pNewDebris(&actor.logRace(), actor.quantity_, trans.position(), *actor.pBoundary_);
...
void MachLogDebris::setLifeTime(MachPhys::BuildingMaterialUnits quantity)
{
    ...
    if (scavengersExist)
        lifetimeRemaining_
            = BURN_TIME + (quantity * 2); // debris from bigger (well, more costly...) things sticks around longer
    else
