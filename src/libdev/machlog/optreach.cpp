...
bool MachLogTreacheryOperation::doStart()
{
    ...
    if (pDirectObject_)
    {
        ...
        // HAL_STREAM(" Target contained in boundary " << pConstruction->globalBoundary().contains(
        // pDirectObject_->position() ) << std::endl); HAL_STREAM(" Target insideBuilding " << MachActor::asMachine(
        // pDirectObject_ ).insideBuilding() << std::endl ); if( MachActor::asMachine( pDirectObject_ ).insideBuilding()
        // ) HAL_STREAM(" Target insideWhichBuilding().id() " << MachActor::asMachine( pDirectObject_
...
// virtual
bool MachLogTreacheryOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
...
PhysRelativeTime MachLogTreacheryOperation::moveCloserToTarget()
{
    ...
    if (recalculateDestination)
    {
        ...
        foundSpace = MachLogSpacialManipulation::getNearestFreeSpacePoint(
            testTransform,
            innerRadius,
            useClearance,
            &attackPoint,
