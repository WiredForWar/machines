...
#ifndef _INLINE
#include "machlog/World/Planet.ipp"
#endif
...
void MachLogPlanet::readPlanetConfigSpaceFile(const SysPathName& spaceFilePath, IProgressReporter* pReporter)
{
    ...
    while (! parser.finished())
    {
        ...
        else if (parser.tokens()[0] == "DOMAIN")
        {
            ...
            // Add the domain
            PhysConfigSpace2d::DomainId domainId = pConfigSpace_->addDomain(MexAlignedBox2d(xMin, yMin, xMax, yMax));
            domainIds.push_back(domainId);
        }
        else if (parser.tokens()[0] == "PORTAL")
        {
            ...
            // Add the portal
            pConfigSpace_->addPortal(domainIds[index1], domainIds[index2], MexPoint2d(x1, y1), MexPoint2d(x2, y2));
        }
        else
        {
            ASSERT_FAIL("");
...
const MachLogPlanet::Sites& MachLogPlanet::sites() const
{
    CB_MachLogPlanet_DEPIMPL();
    return oreSites_;
}
...
const MachLogPlanet::DebrisSites& MachLogPlanet::debrisSites() const
{
    CB_MachLogPlanet_DEPIMPL();
    return debrisSites_;
}
...
bool MachLogPlanet::planetSurfaceOK(std::ostream& os) const
{
    ...
    for (size_t i = 0; i < triangles.size(); ++i)
    {
        ...
        ASSERT_INFO(triangles[i].point1() << " " << triangles[i].point2() << " " << triangles[i].point3());
    }
