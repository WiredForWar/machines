...
bool MachLogLowUnitCountCondition::doHasConditionBeenMet() const
{
    ...
    switch (objectType_)
    {
        case MachLog::AGGRESSOR:
            return races.aggressors(race_).size() < number_;
        case MachLog::ADMINISTRATOR:
            return races.administrators(race_).size() < number_;
        case MachLog::APC:
            return races.apcs(race_).size() < number_;
        case MachLog::CONSTRUCTOR:
            return races.constructors(race_).size() < number_;
        case MachLog::GEO_LOCATOR:
            return races.geoLocators(race_).size() < number_;
        case MachLog::SPY_LOCATOR:
            return races.spyLocators(race_).size() < number_;
        case MachLog::RESOURCE_CARRIER:
            return races.resourceCarriers(race_).size() < number_;
        case MachLog::TECHNICIAN:
            return races.technicians(race_).size() < number_;
...
std::ostream& operator<<(std::ostream& o, const MachLogLowUnitCountCondition& t)
{
