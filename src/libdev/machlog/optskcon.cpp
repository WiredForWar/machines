...
PhysRelativeTime MachLogTaskConstructOperation::doUpdate()
{
    ...
    // there are no constructions on the list and there are no extra incomplete buildings waiting so may as well get out
    // of the way and exit.
    if (complete_)
    {
    ...
    MachLogConstructor* pConstructor = nullptr;
    for (MachLogSquadron::Machines::iterator i = pActor_->machines().begin(); i != pActor_->machines().end(); ++i)
...
bool MachLogTaskConstructOperation::placeConstruction(
    ...
    MachLogConstruction** ppConstruction)
{
    ...
    {
        HAL_STREAM(
            " assigning to ppConstruction:: (void*)(*ppConstruction) before " << (void**)(ppConstruction) << std::endl);
        *ppConstruction = MachLogActorMaker::newLogConstruction(
            prod.type(),
            prod.subType(),
            prod.hwLevel(),
            prod.globalTransform().position(),
        ...
        HAL_STREAM(
            " assigning to ppConstruction:: (void*)(*ppConstruction) After " << (void**)(ppConstruction) << std::endl);
    }
...
bool MachLogTaskConstructOperation::couldPlaceConstruction(const MachLogProductionUnit& prod)
{
    ...
    MachPhysConstruction* pPhysConstruction = MachLogActorMaker::newPhysConstruction(
        prod.type(),
        prod.subType(),
        prod.hwLevel(),
        &MachLogPlanet::instance().hiddenRoot(),
        prod.globalTransform(),
        MachPhys::RED);
