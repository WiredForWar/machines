...
#ifndef _INLINE
#include "machlog/Actors/MissileEmplacement.ipp"
#endif
...
std::ostream& operator<<(std::ostream& o, const MachLogMissileEmplacement& t)
{
    ...
    o << "MachLogMissileEmplacement " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogMissileEmplacement " << static_cast<const void*>(&t) << " end" << std::endl;
        ...
        switch (subType())
        {
            case MachPhys::ICBM:
                if (nukeReady() && ! domeOpen())
                {
                    ...
                    {
                        pPhysMissileEmplacement()->prepForLaunch(
                            SimManager::instance().currentTime(),
                            MachPhysMissileEmplacement::DOME_OPEN);
                        inTransition_ = true;
                        interval = 3.5;
                    }
                }
                else if ((! nukeReady()) && domeOpen())
                {
                    ...
                    else
                    {
                        ...
                        if (timeNow - weapons().front()->lastFireTime() >= 2.5)
                        {
                            pPhysMissileEmplacement()->prepForLaunch(
                                timeNow + 2.0,
                                MachPhysMissileEmplacement::DOME_CLOSE);
                            inTransition_ = true;
                            interval = 2.0;
                        }
                    }
...
MexRadians MachLogMissileEmplacement::angleToTurnToFace(const MexPoint3d& pos)
{
...
void perRead(PerIstream& istr, MachLogMissileEmplacement& actor)
{
    ...
    MachPhysMissileEmplacement* pPhysMissileEmplacement = MachLogMissileEmplacement::pNewPhysMissileEmplacement(
        actor.subType_,
        &actor.logRace(),
        level,
        trans.position(),
...
// virtual
void MachLogMissileEmplacement::beHit(
    const int& damage,
...
// virtual
void MachLogMissileEmplacement::beHitWithoutAnimation(
    int damage,
