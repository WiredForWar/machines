...
#include "machphys/Weapons/WeaponUtility.hpp"
#include "machphys/Terrain/PlanetSurface.hpp"

#include "machlog/Actors/Machine.hpp"
#include "machlog/Internal/MachineImpl.hpp"
...
PhysRelativeTime MachLogMachine::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR)
{
    ...
    if (isIdle())
        sendMessage(nullptr, MachLogMessage(this, MachLogMessage::IDLE));
    // classify();
    ...
    if( changeRaceStartTime() > 0 )
    {
        ...
        {
            HAL_STREAM(" its time to change the race back again...race " << race() << " original race " <<
    logOriginalRace().race() << std::endl ); if( race() != logOriginalRace().race() )
            {
...
void MachLogMachine::doOutputOperator(std::ostream& o) const
{
    ...
    if (pSquadron_)
    {
        o << "(" << pSquadron_->id() << ") " << *pSquadron_
          << std::endl; /* ->squadronId() << " " << pSquadron_->strategy().currentOperationType() << std::endl;
    ...
    if (insideBuilding())
    {
        ...
        o << " interior config space is " << (void*)&insideWhichBuilding().interiorConfigSpace()
          << " motion seq c cspace " << (void*)&motionSeq().currentConfigSpace() << std::endl;
...
{
    CB_DEPIMPL(MachPhysStation*, pLockedStation_);
    return pLockedStation_ != nullptr;
}
...
void MachLogMachine::insideWhichBuilding(MachLogConstruction* pEnteredBuilding)
{
    ...
    if (pEnteredBuilding || pImpl_->pInOrOnPadConstruction_)
    {
        ...
        PhysConfigSpace2d* pNewSpace
            = &(pEnteredBuilding ? pEnteredBuilding->interiorConfigSpace() : MachLogPlanet::instance().configSpace());
...
void MachLogMachine::snapTo(const MexPoint3d& target)
{
    ...
    MexTransform3d localTransform;
    W4dDomain* pDomain = MachLogPlanetDomains::pDomainPosition(target, 0, &localTransform);
...
void MachLogMachine::CLASS_INVARIANT
{
    ...
    INVARIANT(insideAPC_ == (pImpl_->pAPCImInside_ != nullptr));
}
...
// virtual
void MachLogMachine::beHitWithoutAnimation(
    int damage,
        ...
        if (hp() <= 0)
        {
            ...
            if (pByActor)
                MachLogRaces::instance()
                    .actorDestroyedScoreAdjust(race(), objectData().cost(), *this, pByActor->race());
            else
...
void MachLogMachine::releaseHealingAuraReference()
{
    ...
    HAL_STREAM(
        "(" << id() << ") MLMachine::releaseHealingAuraReference at exit " << healAuraReferences_ << "references\n");
}
        ...
        for (Actors::iterator i = actorsThreateningMe().begin(); i != actorsThreateningMe().end(); ++i)
        {
            ...
            if (disposition == MachLogRaces::NEUTRAL || disposition == MachLogRaces::ENEMY)
            {
                ...
                else
                {
                    ...
                    ASSERT( (*i)->objectType() == MachLog::MISSILE_EMPLACEMENT, "Non-machine threat actor should only be
                a missile emplacement!" );
...
bool MachLogMachine::wontFindAnywhereSafer() const
{
         ...
         ++priority)
    {
        if (EPP.aggressivesPriority() == priority && EPP.turretsPriority() == priority)
        {
            ...
            MachLogMissileEmplacement* pMissileEmplacement = nullptr;
...
bool MachLogMachine::findSafestNearbyAggressiveMachine(MachLogMachine** ppSafestMachine) const
{
    ...
    *ppSafestMachine = pSafestMachine_;
    ...
    POST(found == (*ppSafestMachine != nullptr));
...
bool MachLogMachine::findSafestNearbyMissileEmplacement(MachLogMissileEmplacement** ppSafestMissileEmplacement) const
{
        ...
        || SimManager::instance().currentTime() - lastSafestMissileEmplacementEstimateTime_ > 6.0)
    {
        ...
        MachLogRaces::instance().findSafestFriendly(
            *this,
            &pNewSafestMissileEmplacement,
            MachLogRaces::MISSILE_EMPLACEMENTS);
        ...
        {
            ASSERT(
                pNewSafestMissileEmplacement->objectType() == MachLog::MISSILE_EMPLACEMENT,
                "Pointer returned is not pointer to missile emplacement.");
    ...
    *ppSafestMissileEmplacement = pSafestMissileEmplacement_;
    ...
    POST(found == (*ppSafestMissileEmplacement != nullptr));
...
// virtual
bool MachLogMachine::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
    CB_DEPIMPL(MachLogConstruction*, pInOrOnPadConstruction_);
    CB_DEPIMPL(MachLogMissileEmplacement*, pSafestMissileEmplacement_);
    CB_DEPIMPL(MachLogMachine*, pSafestMachine_);
    ...
    switch (event)
    {
        case W4dSubject::DELETED:
            ...
            else if (pSubject == (W4dSubject*)pInOrOnPadConstruction_)
            {
                ...
                CB_DEPIMPL(MachLogEntrance*, pLockedEntrance_);
                CB_DEPIMPL(MachPhysStation*, pLockedStation_);
                pLockedEntrance_ = nullptr;
                pLockedStation_ = nullptr;

                // Attach to the planet exterior domain at current position
                snapTo(position());
                pImpl_->pInOrOnPadConstruction_ = nullptr;
                ...
                if (races.actorExists(subjectId))
                {
                    ...
                    if (subjectActor.objectIsConstruction())
                    {
                        ...
                        if (pInteriorDomain == &motionSeq().spaceDomain())
                        {
                            ...
                            motionSeq().useSpaceDomain(nullptr);
                        }
                    }
                }
                remainAttached = false;
...
void perWrite(PerOstream& ostr, const MachLogMachine& actor)
{
    ...
    if (actor.pImpl_->pTempConstruction() == nullptr)
    {
        if (actor.pImpl_->pLockedEntrance() != nullptr)
...
{
    return pImpl_->pInOrOnPadConstruction_ != nullptr;
}

MachLogConstruction& MachLogMachine::inOrOnPadWhichBuilding()
{
    PRE(pImpl_->pInOrOnPadConstruction_ != nullptr);
    return *pImpl_->pInOrOnPadConstruction_;
}
...
{
    CB_DEPIMPL(MachLogEntrance*, pLockedEntrance_);
    return pLockedEntrance_ != nullptr;
}
...
void MachLogMachine::ignoreEntrancePolygon(bool setIgnore)
{
    ...
    PhysConfigSpace2d::PolygonId id
        = (&mSeq.currentConfigSpace() == &MachLogPlanet::instance().configSpace()
               ? pLockedEntrance_->exteriorPolygonId()
               : pLockedEntrance_->interiorPolygonId());
...
void MachLogMachine::checkAndDoClearEntrance(MachLogConstruction& constron)
{
    ...
    const MexPolygon2d& interiorPolygon = interiorConfigSpace.polygon(constron.entrance(0).interiorPolygonId());
