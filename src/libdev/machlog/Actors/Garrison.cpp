...
#include "machlog/Internal/GarrisonImpl.hpp"
...
#ifndef _INLINE
#include "machlog/Actors/Garrison.ipp"
#endif
...
MachPhysGarrison*
MachLogGarrison::pNewPhysGarrison(MachLogRace* pRace, uint level, const MexPoint3d& location, const MexRadians& zAngle)
{
...
void MachLogGarrison::healAll()
{
    ...
    while (! foundNoMachinesThatNeedHealingStopped)
    {
        ...
        for (MachLogConstruction::Machines::iterator i = machinesBeingHealed_.begin(); i != machinesBeingHealed_.end();
             ++i)
        {
    ...
    MachPhys::HitPointUnits addAmount
        = (int)((float)data().repairRate() * ((SimManager::instance().currentTime() - lastUpdateTime_) / 60.0));
...
void MachLogGarrison::stopHealing(MachLogMachine* pMachine)
{
    ...
    MachLogConstruction::Machines::iterator iToBeRemoved
        = find(machinesBeingHealed_.begin(), machinesBeingHealed_.end(), pMachine);
...
{
    CB_MachLogGarrison_DEPIMPL();
    MachLogConstruction::Machines::iterator i
        = find(machinesBeingHealed_.begin(), machinesBeingHealed_.end(), pMachine);
...
bool MachLogGarrison::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int)
{
    ...
    switch (event)
    {
        ...
        case W4dSubject::LEFT_DOMAIN:
            {
                MachLogConstruction::Machines::iterator i
                    = find(machinesBeingHealed_.begin(), machinesBeingHealed_.end(), pSubject);
...
void perRead(PerIstream& istr, MachLogGarrison& actor)
{
    ...
    MachPhysGarrison* pPhysGarrison = MachLogGarrison::pNewPhysGarrison(
        &actor.logRace(),
        level,
        trans.position(),
