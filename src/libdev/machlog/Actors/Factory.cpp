...
#include "machlog/Internal/FactoryImpl.hpp"
...
#ifndef _INLINE
#include "machlog/Actors/Factory.ipp"
#endif
...
void MachLogFactory::createNewMachine(
    ...
    MachLogRace* pRace)
{
    ...
    ASSERT(
        fabs(xBasis2.angleBetween(directionToPos).asScalar()) < Mathex::PI_DIV_2,
        "The angle from the factory xbasis to the creation point mag > 90 degrees\n");
#endif
    // HAL_STREAM("MLFactory::creatNewmachine\n" );
    // HAL_STREAM(" " << newMachine.type << std::endl );
    MachLogMachine* pMach = nullptr;
    pMach = MachLogActorMaker::newLogMachine(
        newMachine.type(),
        newMachine.subType(),
        newMachine.hwLevel(),
        newMachine.swLevel(),
...
bool MachLogFactory::buildMachine(
...
*/
bool MachLogFactory::buildMachineUnique(
    const MachLog::ObjectType type,
    const int subType,
    const MachLogMachine::Level hwLevel,
    const MachLogMachine::Level swLevel,
...
void MachLogFactory::cancelProductionUnit(const MachLogProductionUnit* pUnit)
{
    ...
    if (! productionLine_.empty())
    {
        ...
        {
            MachPhys::BuildingMaterialUnits refundedBMUs
                = ((float)(*i)->amountBuilt() * MachLogRaces::instance().stats().cancelledRefundablePercentage());
...
void MachLogFactory::moveProductionUnit(MachLogProductionUnit* pUnit, MachLogFactory::MoveDirection direction)
{
    CB_MachLogFactory_DEPIMPL();
...
MachPhys::BuildingMaterialUnits MachLogFactory::amountBuilt() const
{
    CB_MachLogFactory_DEPIMPL();
    return amountBuilt_;
}

MachPhys::BuildingMaterialUnits MachLogFactory::amountBuilt()
{
    CB_MachLogFactory_DEPIMPL();
    return amountBuilt_;
}
...
MachPhys::BuildingMaterialUnits MachLogFactory::currentTotalCost() const
{
    ...
    MachPhys::BuildingMaterialUnits cost
        = MachLogRaces::instance()
              .researchTree()
              .researchItem(current->type(), current->subType(), current->hwLevel(), current->weaponCombo())
              .factoryInstanceCost();
    return cost;
}
...
bool MachLogFactory::isAllowedToBuild(MachLog::ObjectType type, int subType, int hwLevel, MachPhys::WeaponCombo wc)
    const
{
    CB_MachLogFactory_DEPIMPL();
    bool found = false;
        ...
        if (lineSize > 0 && parser.tokens()[0] != "//")
        {
            ...
            while (working)
            {
                if (parser.tokens().size() > 0)
                {
                    ...
                    {
                        MachLogResearchItem* pRI = &MachLogRaces::instance().researchTree().researchItem(
                            MachLogScenario::objectType(parser.tokens()[0]),
                            0,
                            atoi(parser.tokens()[1].c_str()),
                            wc);
...
void perRead(PerIstream& istr, MachLogFactory& actor)
{
    ...
    MachPhysFactory* pPhysFactory = MachLogFactory::pNewPhysFactory(
        (MachPhys::FactorySubType)actor.subType(),
        &actor.logRace(),
        level,
        trans.position(),
...
void MachLogFactory::moveToDeployPoint(
    MachLogMachine* pMach,
    const MexPoint2d& entranceInternalPoint,
    const MexPoint2d& entranceExternalPoint,
    BuiltOrBlocking builtOrBlocking)
{
    CB_MachLogFactory_DEPIMPL();
