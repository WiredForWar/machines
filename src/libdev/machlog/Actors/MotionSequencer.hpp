...
private:
    ...
    using Point2ds = ctl_nb_vector<MexPoint2d>;
    using OrientationInfo = MachPhysMachine::OrientationInfo;
    ...
    // the move otherwise.
    PhysRelativeTime
    moveOutOfWay(const MexPoint2d& askerStartPoint, const MexPoint2d& askerEndPoint, MATHEX_SCALAR askerClearance);
    ...
    using MoveInfoUPtr = std::unique_ptr<MachPhysMachineMoveInfo>;
    ...
    void forceMotionToRest(const PhysConfigSpace2d::ObjectIds& ignoreIds, ctl_vector<size_t>* pNChunks);
    ...
    void stopShuffling();
    void shuffle();
    MoveInfoUPtr getShuffleMoveInfo(const MexTransform3d& sourceTransform, const MexTransform3d& targetTransform);
