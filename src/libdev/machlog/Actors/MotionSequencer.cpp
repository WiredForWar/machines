...
#ifndef _INLINE
#include "machlog/Actors/MotionSequencer.ipp"
#endif

//////////////////////////////////////////////////////////////////////////////////////////

MachLogMachineMotionSequencer::MachLogMachineMotionSequencer(
    MachLogMobile* pLogMobile,
    ...
    : pImpl_(new MachLogMachineMotionSequencerData(pLogMobile->id()))
{
    ...
    PRE(pLogMobile != nullptr);
    PRE(pPhysMachine != nullptr);
...
std::ostream& operator<<(std::ostream& o, const MachLogMachineMotionSequencer::State& t)
{
    switch (t)
    {
        case MachLogMachineMotionSequencer::ARRIVED:
            o << "ARRIVED";
            break;
        case MachLogMachineMotionSequencer::FAILED:
            o << "FAILED";
            break;
        case MachLogMachineMotionSequencer::PLANNING:
            o << "PLANNING";
            break;
        case MachLogMachineMotionSequencer::WAITING:
            o << "WAITING";
            break;
        case MachLogMachineMotionSequencer::MOVING:
            o << "MOVING";
            break;
        case MachLogMachineMotionSequencer::FIRST_PERSON:
            o << "FIRST PERSON";
            break;
    }
    return o;
...
std::ostream& operator<<(std::ostream& o, const MachLogMachineMotionSequencer::FailureReason& t)
{
    switch (t)
    {
        case MachLogMachineMotionSequencer::NO_ROUTE:
            o << "NO_ROUTE";
            break;
        case MachLogMachineMotionSequencer::DESTINATION_OCCUPIED:
            o << "DESTINATION_OCCUPIED";
            break;
    }
    return o;
}
...
std::ostream& MachLogMachineMotionSequencer::streamOut(std::ostream& o) const
{
    ...
    o << "  progress: domains " << nPortalPointsDone_ << "/" << domainPath_.size() << "  path " << nPathPointsDone_
      << "/" << path_.size() << std::endl;
...
PhysRelativeTime
MachLogMachineMotionSequencer::destination(const MexPoint2d& newDestination, const MachLogGroupMoveInfo& groupMoveInfo)
{
...
void MachLogMachineMotionSequencer::createFindPath()
{
    ...
    if (! haveTargetPoint)
    {
        ...
        LOG_STREAM(
            "(" << pLogMobile_->id() << ") McMotSeq::createFindPath have target point " << haveTargetPoint
                << " sqrEuclidain " << targetPoint.sqrEuclidianDistance(nowPoint) << std::endl);
    }
    ...
    if (haveTargetPoint)
    {
        ...
        if (! targetPositionContainedInSpace(targetPoint))
        {
            ...
            haveTargetPoint
                = pConfigSpace_->findSpace(nowPoint, targetPoint, useClearance_, 15.0, obstacleFlags(), &newTarget);
    ...
    if (haveTargetPoint)
    {
        ...
        {
            // Set up the pathfind
            findPathId_ = pConfigSpace_->addFindPath(
                nowPoint,
                targetPoint,
                useClearance_,
                logMobile().obstacleFlags(),
                pImpl_->pathFindingPriority_);
    ...
    else
    {
        ...
        CHANGE_STATE((pFollowSequencer_ == nullptr ? INTERNAL_RESTING : INTERNAL_WAITING), "arrived at target point");
        clearOldPaths();
...
void MachLogMachineMotionSequencer::addRestingObstacleWithoutEcho(const MexTransform3d& newTransform)
{
    ...
    DEBUG_STREAM(
        DIAG_NETWORK,
        "(" << pLogMobile_->id() << ") addRestingObstacleWithoutEcho currentLocation " << currentLocation()
            << std::endl);
    MexPoint2d nearPoint(location2d.x() + 0.01, location2d.y());
...
PhysRelativeTime MachLogMachineMotionSequencer::updateWhileMoving()
{
    ...
    LOG_STREAM(
        "(" << pLogMobile_->id() << ") Exit MachLogMachineMotionSequencer::updateWhileMoving " << interval
            << std::endl);
                ...
                case INTERNAL_WANT_DOMAIN_PATH:
                    {
                        ...
                        if (! targetPositionContainedInSpace(currentLocation()))
                        {
                                    ...
                                    &newTarget))
                            {
                                ...
                                const MATHEX_SCALAR newZ
                                    = MachLogPlanet::instance().surface()->terrainHeight(newTarget.x(), newTarget.y());
                                ...
                                LOG_STREAM(
                                    "In obstacle but have found close point to teleport to " << destinationPoint_
                                                                                             << std::endl);
                ...
                case INTERNAL_WANT_PATH:
                    {
                        ...
                        LOG_STREAM(
                            "(" << pLogMobile_->id()
                                << ") McMotSeq::Update on INTERNAL_WANT_PATH looking at distance to target\n loc "
                                << location << " dest " << destinationPoint_ << std::endl);
                        LOG_STREAM(
                            "fabs( location.x() - destinationPoint_.x() ) "
                            << fabs(location.x() - destinationPoint_.x()) << std::endl);
                        LOG_STREAM(
...
W4dDomain* MachLogMachineMotionSequencer::setDomain(const MexTransform3d& newGlobalTransform)
{
    ...
    DEBUG_STREAM(
        DIAG_NETWORK,
        "(" << pLogMobile_->id() << ") setDomain local before " << pPhysMachine_->localTransform() << std::endl);
    // Get current domain
    W4dDomain* pDomain = pPhysMachine_->containingDomain();
    ...
    // DEBUG_STREAM( DIAG_NETWORK, "(" << pLogMobile_->id() << ") setDomain global is now " <<
    // pLogMobile_->globalTransform() << std::endl ); DEBUG_STREAM( DIAG_NETWORK, "(" << pLogMobile_->id() << ")
    // setDomain local is now " << pPhysMachine_->localTransform() << std::endl ); LOG_STREAM(" Exit setDomain\n" );
...
void MachLogMachineMotionSequencer::createDomainFindPath(const MexPoint2d& pathStart, const MexPoint2d& pathDestination)
{
    ...
    domainFindPathId_ = pConfigSpace_->addDomainFindPath(
        pathStart,
        pathDestination,
        useClearance_,
        obstacleFlags(),
...
void MachLogMachineMotionSequencer::getOnPortalPoint()
{
    ...
    else
    {
        ...
        if (pConfigSpace_->findSpace(onPortalPoint_, useClearance_, radius, obstacleFlags(), &nearbyPoint))
        {
...
PhysRelativeTime MachLogMachineMotionSequencer::initiateMove()
{
    ...
    DEBUG_STREAM(
        DIAG_NETWORK,
        "(" << pLogMobile_->id() << ") MachLogMachineMotionSequencer::initiateMove " << std::endl);
    ...
    if (! motionInitiated)
    {
            ...
            && ! objectHasSameCommandId(collisionObjectId))
        {
            ...
            if (nPortalPointsDone_ == domainPath_.size())
            {
                ...
                MATHEX_SCALAR sqrDistanceToTarget = fromPoint2d.sqrEuclidianDistance(targetPoint);
                ...
                else
                {
                        ...
                        && races.actor(id).race() == logMobile().race())
                    {
                        ...
                        interval = obstructingSequencer.moveOutOfWay(currentLocation(), targetPoint, useClearance_);
        ...
        if (! done)
        {
            ...
            if (races.actorExists(actorId) && races.actor(actorId).objectIsMachine())
            {
                ...
                if (tryMove)
                {
                    ...
                    interval = pObstructingSequencer->moveOutOfWay(fromPoint2d, targetPoint, useClearance_);
...
bool MachLogMachineMotionSequencer::tryNearerPoint(
    const MexPoint2d& fromPoint,
    const MexPoint2d& toPoint,
...
void MachLogMachineMotionSequencer::follow(MachLogMobile* pLeadMobile, const MexPoint2d& offset)
{
    LOG_ENTER("follow");

    CB_MachLogMachineMotionSequencerData_DEPIMPL();

    PRE(pLeadMobile != nullptr);
...
void MachLogMachineMotionSequencer::updateFollowDestination()
{
    ...
    else
    {
        ...
        tryPoint = MexPoint2d(
            leadDestination.x() + followOffset_.x() * leadDirection.x() - followOffset_.y() * leadDirection.y(),
            leadDestination.y() + followOffset_.x() * leadDirection.y() + followOffset_.y() * leadDirection.x());
    }
        ...
        && (internalState_ == INTERNAL_RESTING || newDestination.sqrEuclidianDistance(destinationPoint_) > 1.0))
    {
        ...
        destinationPoint_ = newDestination;
        HAL_STREAM(
            "(" << pLogMobile_->id() << ") (following)has just changed destinationPoint_ to " << destinationPoint_
                << std::endl);
...
PhysRelativeTime MachLogMachineMotionSequencer::moveOutOfWay(
    const MexPoint2d& askerStartPoint,
    const MexPoint2d& askerEndPoint,
    MATHEX_SCALAR askerClearance)
{
    ...
    if (allowMoveOutOfWay())
    {
            ...
            && internalState_ != INTERNAL_PLANNING_DOMAIN_PATH && internalState_ != INTERNAL_1ST_PERSON)
        {
            ...
            askerNormal.setVector(askerNormal.x() * distance * side, askerNormal.y() * distance * side);
...
bool MachLogMachineMotionSequencer::isFollowing(MachLogMachineMotionSequencer* pRhsSequencer) const
{
    LOG_ENTER("isFollowing");

    CB_MachLogMachineMotionSequencerData_DEPIMPL();

    PRE(pRhsSequencer != nullptr);

    return pFollowSequencer_ == pRhsSequencer;
}
//////////////////////////////////////////////////////////////////////////////////////////

PhysRelativeTime MachLogMachineMotionSequencer::updateWhilePlanning(const PhysRelativeTime& maxCPUTime)
{
...
MATHEX_SCALAR MachLogMachineMotionSequencer::capSpeed() const
{
    ...
    if (pConvoy_ != nullptr)
    {
        ...
        else
        {
            ...
            PhysRelativeTime leaderTravelTime = leaderCurrentPosition.euclidianDistance(targetPosition) / convoySpeed;
...
bool MachLogMachineMotionSequencer::positionContainedInSpace(
    const MexPoint2d& targetPoint,
...
void MachLogMachineMotionSequencer::switchConfigSpace(PhysConfigSpace2d* pNewConfigSpace)
{
    ...
    NETWORK_STREAM(
        " old cspace " << (void*)pConfigSpace_ << " new config space " << (void*)pNewConfigSpace << " pSpaceDomain "
                       << (void*)pSpaceDomain_ << "\n");
...
bool MachLogMachineMotionSequencer::canMoveTo(const MexPoint2d fromPoint, const MexPoint2d toPoint)
{
    CB_MachLogMachineMotionSequencerData_DEPIMPL();
    ...
    bool result
        = pConfigSpace_->add(motionChunk, pLogMobile_->id(), ignoreIds, &tempId, &collisionObjectId, &collideTime);
...
bool MachLogMachineMotionSequencer::findAvoidancePoint(const MexPoint2d& fromPoint, MexPoint2d* pToPoint)
{
    CB_MachLogMachineMotionSequencerData_DEPIMPL();
    ...
    do
    {
        ...
        moveDirection += MexVec2(mexRandomScalar(&r, -1.0, 1.0), mexRandomScalar(&r, -1.0, 1.0));
    } while (moveDirection.isZeroVector());

    moveDirection.makeUnitVector();
    moveDirection.setVector(moveDirection.x() * useClearance_ * 2.0, moveDirection.y() * useClearance_ * 2.0);
...
bool MachLogMachineMotionSequencer::addMotionChunks(const ctl_vector<PhysMotionChunk>& chunks)
{
    ...
    for (size_t i = 0; worked && i < chunks.size(); ++i)
    {
        ...
        if (pConfigSpace_->add(chunks[i], pLogMobile_->id(), ignoreIds, &chunkId, &collisionObjectId, &collideTime))
        {
            motionChunkIds_.push_back(chunkId);
            NETWORK_STREAM(
                "(" << pLogMobile_->id() << ") !! Added motion chunk with id " << chunkId << "callstack\n"
...
bool MachLogMachineMotionSequencer::followingMachines(MachLogMachine::Machines* pMachines)
{
    ...
    const MachLogMachineConvoy::Sequencers& convoySequencers = pConvoy_->sequencers();
    for (MachLogMachineConvoy::Sequencers::const_iterator it = convoySequencers.begin(); it != convoySequencers.end();
         ++it)
    {
        MachLogMachineMotionSequencer* pConvoySequencer = *it;
        if (pConvoySequencer != this && pConvoySequencer->pFollowSequencer() == this)
...
bool MachLogMachineMotionSequencer::preventImpendingCollisions(
    const MexTransform3d& nextPosition,
    const PhysAbsoluteTime& startTime)
{
    ...
    PhysMotionChunk
        testChunk(startPoint, endPoint, useClearance_, motionProfile, startTime, 0.0, minHeight(), maxHeight());
    ...
    for (PhysConfigSpace2d::ChunkIntersections::iterator it = intersections.begin(); it != intersections.end(); ++it)
    {
        ...
        if (data.collisionTime_ < limitTime)
        {
            ...
            MachLogMachine& otherMachine
                = MachLogRaces::instance().actor(data.collisionObjectId_.asScalar()).asMachine();
    ...
    if (! keepEntranceLocked())
    {
        ...
        if (pMachine->hasLockedEntrance() && pMachine->inBuildingOrOnPad())
        {
            ...
            if (! onPolygons)
            {
                ...
                pMachine->entranceLocked(nullptr);

                // If outside, relinquish association with the building
                if (outside)
                    pMachine->inOrOnPadBuilding(nullptr);
            }
        }
    }
}
...
bool MachLogMachineMotionSequencer::isFollowing() const
{
    ...
    return pFollowSequencer_ != nullptr;
}
//////////////////////////////////////////////////////////////////////////////////////////
...
bool MachLogMachineMotionSequencer::straightLineMove(const MexPoint3d& toPoint, PhysRelativeTime* pInterval)
{
    CB_MachLogMachineMotionSequencerData_DEPIMPL();
    ...
    size_t pointsReached = reservePath(
        fromTransform,
        MachPhysMachine::USE_ORIENTATION,
        path,
        startTime,
...
void MachLogMachineMotionSequencer::mergeConvoys(MachLogMachineConvoy* pConvoy)
{
    ...
    if (pConvoy != pConvoy_)
    {
        ...
        for (size_t i = 0; i != nToMove; ++i)
        {
            ...
            pSequencer->pImpl_->pConvoy_ = nullptr; // Needed for precondition
...
void MachLogMachineMotionSequencer::updateConvoy(MachLogMachineConvoy* pOldConvoy)
{
    ...
    for (size_t i = 0; i != nSequencers; ++i)
        sequencers[i]->pImpl_->pConvoy_ = nullptr;
