...
#ifndef _INLINE
#include "machlog/Actors/Mine.ipp"
#endif
...
// static
MachPhysMine*
MachLogMine::pNewPhysMine(MachLogRace* pRace, uint level, const MexPoint3d& location, const MexRadians& zAngle)
{
...
// virtual
PhysRelativeTime MachLogMine::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR clearence)
{
    ...
    else
    {
        ...
        if (ore_ < data().capacity())
        {
            ...
            int tempInt
                = (int)((float)data().extractionRate() / 60.0 * (SimManager::instance().currentTime() - lastUpdateTime_));
            ...
            if (tempInt > 0)
            {
                ...
                /*
                if( MachLogRaces::instance().nOre( race() ) + canAttemptToExtract > MachLogRaces::instance().nMaxOre(
                race() ) ) canAttemptToExtract = MachLogRaces::instance().nMaxOre( race() ) -
                MachLogRaces::instance().nOre( race() );
                */
...
bool MachLogMine::validMineSite(const MexPoint3d& location, uint hwLevel, const MachLogMine::CheckDiscoveredFlag check)
{
    ...
    // HAL_STREAM("MLMine::validMineSite...number of sites to check " << MachLogPlanet::instance().sites().size() <<
    // std::endl );
    bool result = false;
         ...
         ++i)
    {
        ...
        {
            // this is for locations set up at the beginning of the scenario - don't need to test for having been
            // discovered
...
bool MachLogMine::outsideMinimumMineSmelterRange(const MexPoint3d& location, const MachLogConstruction* pTarget)
{
    ...
    // This method is also called from MachLogSmelter::validForAllSmelters(). pTarget is intended to be either a
    ...
    static MATHEX_SCALAR sqrMinSmelterDistance
        = MachPhysData::instance()
              .smelterData(1)
              .sqrMineDistance(); // minimum distance a mine must be away from the nearest
    // smelter
...
void perRead(PerIstream& istr, MachLogMine& actor)
{
    ...
    MachPhysMine* pPhysMine
        = MachLogMine::pNewPhysMine(&actor.logRace(), level, trans.position(), trans.rotationAsEulerAngles().azimuth());
