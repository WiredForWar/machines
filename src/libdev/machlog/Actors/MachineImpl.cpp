...
MachLogMachineImpl::MachLogMachineImpl(
    ...
    , evading_(false)
    , camouflaged_(false)
    , lastStrengthEstimateTime_(
          SimManager::instance().currentTime() - MachLogMachine::militaryStrengthEstimateInterval())
    , // guarantees an evaluation on first calling
    lastSafestMachineEstimateTime_(
        SimManager::instance().currentTime() - MachLogMachine::militaryStrengthEstimateInterval())
    , // guarantees an evaluation on first calling
    lastSafestMissileEmplacementEstimateTime_(
        SimManager::instance().currentTime() - MachLogMachine::militaryStrengthEstimateInterval())
    , // guarantees an evaluation on first calling
...
std::ostream& operator<<(std::ostream& o, const MachLogMachineImpl& t)
{

    o << "MachLogMachineImpl " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogMachineImpl " << static_cast<const void*>(&t) << " end" << std::endl;
...
void perWrite(PerOstream& ostr, const MachLogMachineImpl& actorImpl)
{
    ...
    int hasLockedEntrance = actorImpl.pLockedEntrance_ != nullptr;
    PER_WRITE_RAW_OBJECT(ostr, hasLockedEntrance);

    // Write flag and station coords if have a locked station
    int hasLockedStation = actorImpl.pLockedStation_ != nullptr;
...
void perRead(PerIstream& istr, MachLogMachineImpl& actorImpl)
{
    ...
    if (hasLockedEntrance)
    {
        ...
        actorImpl.pLockedEntrance_ = nullptr;
    ...
    if (hasLockedStation)
    {
        ...
        MachPhysConstructionData& consData
            = const_cast<MachPhysConstructionData&>(pTempConstruction->constructionData());
        ...
        actorImpl.pLockedStation_ = nullptr;
}

MachLogMachineImpl::MachLogMachineImpl(PerConstructor)
    : pLockedStation_(nullptr) // this is something that we must initialise here - can't do in perRead
{
}
