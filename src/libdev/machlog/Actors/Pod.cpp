...
#ifndef _INLINE
#include "machlog/Actors/Pod.ipp"
#endif
...
std::ostream& operator<<(std::ostream& o, const MachLogPod& t)
{
    ...
    o << "MachLogPod " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogPod " << static_cast<const void*>(&t) << " end" << std::endl;
...
// static
MachPhysPod*
MachLogPod::pNewPhysPod(MachLogRace* pRace, uint level, const MexPoint3d& location, const MexRadians& zAngle)
{
...
PhysRelativeTime MachLogPod::update(const PhysRelativeTime& alteredMaxCPUTime, MATHEX_SCALAR junk)
{
    ...
    else if (complete)
    {
        ...
        else
        {
            ...
            // the thing is, this way, we guarantee having incremental charge bars updating, but it's pretty expensive
            // stuff.
...
void perRead(PerIstream& istr, MachLogPod& actor)
{
    ...
    MachPhysPod* pPhysPod
        = MachLogPod::pNewPhysPod(&actor.logRace(), level, trans.position(), trans.rotationAsEulerAngles().azimuth());
        ...
        if (isDead() && addedBMUStorageToRace_)
        {
            ...
            if (destroyedByFriendly)
                MachLogRaces::instance().reduceCapacityNoPenalty(
                    race(),
                    MachPhysData::instance().podData(level()).capacity());
            else
...
// virtual
void MachLogPod::beHitWithoutAnimation(
    int damage,
