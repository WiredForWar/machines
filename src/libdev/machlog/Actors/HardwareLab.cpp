...
#ifndef _INLINE
#include "machlog/Actors/HardwareLab.ipp"
#endif
...
void MachLogHardwareLab::doOutputOperator(std::ostream& o) const
{
    ...
    {
        o << " Current Research " << pRi->objectType() << " sub" << pRi->subType() << " hw " << pRi->hwLevel() << " wc "
          << pRi->weaponCombo() << " tech " << pRi->technologyLevel() << std::endl;
        o << " amount researched " << pRi->amountResearched(race()) << "/" << pRi->researchCost() << " built "
          << pRi->amountBuilt(race()) << "/" << pRi->buildingCost() << std::endl;
    }
    else
        o << " Not researching.\n";
}
        ...
        if (! currentResearchQueue_.empty())
        {
            ...
            if (availableResearchUnits > 0)
            {
                    ...
                    ;
                {
                    ...
                    // But the idleness is not a good measure to use when you are looking at the small "single" person
                    // labs as timing will cause the research queue not to be extended.
                    if (races.controller(myRace).type() == MachLogController::AI_CONTROLLER)
                        for (MachLogResearchTree::ResearchItems::iterator i = availableResearchItems().begin();
                             i != availableResearchItems().end();
                             ++i)
                            addResearchItem((**i));
                }
            }
...
void MachLogHardwareLab::initialiseResearchItems()
{
    ...
    for (MachLogRaces::Technicians::const_iterator j = MachLogRaces::instance().technicians(race()).begin();
         j != MachLogRaces::instance().technicians(race()).end();
         ++j)
...
bool MachLogHardwareLab::addResearchItem(const MachLogResearchItem& mlri)
{
    ...
    // mlri.subType() << " hw " << mlri.hwLevel() << std::endl );
    CB_MachLogHardwareLab_DEPIMPL();
    bool found = false;
    for (MachLogResearchTree::ResearchItems::iterator i = availableResearchItems_.begin();
    ...
    if (found)
        if (currentResearchQueue_.size() == 1)
        {
            ...
            // we'll actually use and cache the total research rate value for efficiency - if it's 0, there's no
            // technicians, if more than 0, we can use the value to determine which of two subpossibility voicemails to
            // play.
            float researchRate = totalResearchRate();
...
void MachLogHardwareLab::removeResearchItem(const MachLogResearchItem& mlri)
{
    ...
    MachLogResearchTree::ResearchItems::iterator i
        = find(currentResearchQueue_.begin(), currentResearchQueue_.end(), &mlri);
...
void MachLogHardwareLab::moveResearchItem(const MachLogResearchItem& mlri, MachLogHardwareLab::MoveDirection direction)
{
    CB_MachLogHardwareLab_DEPIMPL();
...
void perRead(PerIstream& istr, MachLogHardwareLab& actor)
{
    ...
    MachPhysHardwareLab* pPhysHardwareLab = MachLogHardwareLab::pNewPhysHardwareLab(
        (MachPhys::HardwareLabSubType)actor.subType(),
        &actor.logRace(),
        level,
        trans.position(),
