...
#include "machphys/Constructions/SoftwareLabData.hpp"
#include "machphys/Machines/TechnicianData.hpp"

#include "machlog/World/MachLog.hpp"
#include "machlog/Actors/SoftwareLab.hpp"
...
MachLogSoftwareLab::MachLogSoftwareLab(
    ...
    : MachLogConstruction(
        pRace,
        pNewPhysSoftwareLab(pRace, level, location, angle),
...
void MachLogSoftwareLab::doOutputOperator(std::ostream& o) const
{
    MachActor::doOutputOperator(o);
    o << *this;
}

std::ostream& operator<<(std::ostream& o, const MachLogSoftwareLab& t)
{
    ...
    {
        for (MachLogResearchTree::ResearchItems::const_iterator i = t.availableResearchItems_.begin();
             i < t.availableResearchItems_.end();
             ++i)
    ...
    {
        o << " Current Research " << pRi->objectType() << " sub" << pRi->subType() << " hw " << pRi->hwLevel()
          << " tech " << pRi->technologyLevel() << std::endl;
        o << " amount researched " << pRi->amountResearched(t.race()) << " built " << pRi->amountBuilt(t.race())
          << std::endl;
    }
    else
        o << " Not researching.\n";
    ...
    if (complete)
    {
        if (currentResearchQueue_.size())
        {
            ...
            else
            {
                ...
                availableResearchUnits
                    = (SimManager::instance().currentTime() - lastUpdateTime_) * availableResearchUnits / 60;
...
void MachLogSoftwareLab::initialiseResearchItems()
{
    ...
    for (MachLogResearchTree::ResearchItems::const_iterator i = items.begin(); i != items.end(); ++i)
    {
        ...
        //      if( pRi->available( race() ) ) HAL_STREAM(" checking swAvailable and swResearched flags for the
        //      following RI\n" << (**i) << std::endl );
        ...
        if (foundAtLeastOne and pRi->available(race()) and pRi->researched(race()) and pRi->swLevel(race()) < 5
            and pRi->swTechnologyLevel(pRi->swLevel(race()) + 1) <= highestTechLevel)
        {
...
bool MachLogSoftwareLab::removeResearchItem(const MachLogResearchItem& mlri)
{
    ...
    for (MachLogResearchTree::ResearchItems::iterator i = currentResearchQueue_.begin();
         i != currentResearchQueue_.end() and not found;
         ++i)
        if ((*i) == &mlri)
        {
            found = true;
