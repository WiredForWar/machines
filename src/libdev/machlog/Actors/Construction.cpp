...
#include "machlog/Actors/Constructor.hpp" //needed by persistence mechanism
...
#ifndef _INLINE
#include "machlog/Actors/Construction.ipp"
#endif
...
void MachLogConstruction::removeDoorPoint()
{
    ...
    if (nEntrances() > 0)
    {
        ...
        MachLogBuildPoint* pClosestPoint = nullptr;
        ...
        for (BuildPoints::iterator i = buildPoints_.begin(); i != buildPoints_.end(); ++i)
        {
            ...
            MATHEX_SCALAR sqrDistanceToEntrancePoint
                = pCandidateBuildPoint->position().sqrEuclidianDistance(entranceExternalPoint(0));
...
bool MachLogConstruction::bestAvailableBuildPoint(
    const MachLogConstructor& enquirer,
    MachLogBuildPoint** ppBestBuildPoint)
{
    CB_MachLogConstruction_DEPIMPL();
    ...
    MachLogBuildPoint* pBestSoFar = nullptr;
    ...
    for (BuildPoints::iterator i = buildPoints_.begin(); i != buildPoints_.end(); ++i)
    {
        ...
        if (! reservationsPreventMeFromTaking)
        {
            ...
            MexTransform3d constructionPointAsTransform = MachLogSpacialManipulation::sourceToDestination3dTransform(
                position(),
                buildPointLocation,
                MachLogSpacialManipulation::DESTINATION_END_IS_CENTRE);

            MATHEX_SCALAR radius = enquirer.highClearence() * 1.8;
            bool actorCanFitThere = MachLogSpacialManipulation::getNearestFreeSpacePoint(
                constructionPointAsTransform,
                radius,
                enquirer.highClearence(),
                &dummy,
                enquirer.highClearence() * MachLogLabourOperation::proximityLeewayMultiplier());
...
bool MachLogConstruction::behemothsRestrict(const MachLogBuildPoint& candidatePoint)
{
    ...
    MachLogBuildPoint* pBest = nullptr;
    MachLogBuildPoint* pSecondBest = nullptr;
    ...
    for (BuildPoints::iterator i = buildPoints_.begin(); i != buildPoints_.end(); ++i)
    {
        ...
        {
            MATHEX_SCALAR sqrDistanceToTestPoint
                = candidatePoint.position().sqrEuclidianDistance(pTestPoint->position());
...
void MachLogConstruction::usedPickupPoint(size_t whichPoint)
{
    ...
    ASSERT(
        whichPoint < 6,
        "If you have a building that genuinely has >6 pickup or putdown points, you'll need to change the "
        "hard-wired hack in MachLogConstruction::initialiseFromConstructionData. Sorry!");
...
void MachLogConstruction::doOutputOperator(std::ostream& o) const
{
    ...
    o << " isComplete " << isComplete() << " constr units " << constructedUnits_ << " constructionData().cost() "
      << constructionData().cost() << std::endl;
...
void MachLogConstruction::addObstaclePolygon(const MexAlignedBox2d& boundary)
{
    ...
    MATHEX_SCALAR height
        = physConstruction().globalTransform().position().z() + constructionBoundingVolume.maxCorner().z();
...
MachPhys::BuildingMaterialUnits MachLogConstruction::bmuValueOfHitPoints(MachPhys::HitPointUnits HPs) const
{
    MATHEX_SCALAR proportionOfTotalConstruction = static_cast<double>(HPs) / constructionData().hitPoints();

    return proportionOfTotalConstruction * constructionData().cost();
}
...
PhysRelativeTime MachLogConstruction::update(const PhysRelativeTime& maxCPUTime, MATHEX_SCALAR clearence)
{
...
void MachLogConstruction::setupInteriorConfigSpace()
{
    ...
    const MachPhysConstructionData::Polygons& interiorObstacles
        = physConstruction().constructionData().interiorObstacles();

    for (MachPhysConstructionData::Polygons::const_iterator i = interiorObstacles.begin(); i != interiorObstacles.end();
         ++i)
    {
...
// virtual
void MachLogConstruction::beHit(
    const int& damage,
...
// virtual
void MachLogConstruction::beHitWithoutAnimation(
    int damage,
...
bool MachLogConstruction::onOutsideSideOfThreshold(size_t entranceIndex, const MexPoint2d& location) const
{
    ...
    const MachPhysConstructionData::EntranceData& entranceData = constructionData().entrances()[entranceIndex];
