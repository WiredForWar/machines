    ...
    , pImpl_(new MachActorImpl(this, pRace, ot))
{
    ...
    else
    {
        ...
        if (MachLogNetwork::instance().isNetworkGame())
        {
            ...
            id(MachLogRaces::allocateId(minId, maxId));
        }
        else
        {
...
MachActor::~MachActor()
{
    ...
    if (id() != 0)
    {
        ...
        ASSERT(
            MachLogRaces::idGenerator().isAllocated(id()),
            "id for actor has not been allocated and trying to free it\n");
...
MachLogRace& MachActor::logRace()
{
    CB_DEPIMPL(MachLogRace*, pRace_);
    return *pRace_;
}

const MachLogRace& MachActor::logRace() const
{
    CB_DEPIMPL(MachLogRace*, pRace_);
    return *pRace_;
}
...
// virtual
void MachActor::beHitWithoutAnimation(
    int damage,
...
void MachActor::doOutputOperator(std::ostream& o) const
{
    ...
    o << "My military strength: " << militaryStrength() << " Local strength estimate: " << localStrengthValue()
      << std::endl;
    if (hasThreats())
...
void MachActor::removeThreat(UtlId threatId /*const MachActor* pThreat*/)
{
    ...
    {
        CB_DEPIMPL(Actors, actorsThreateningMe_);
        Actors::iterator i = find(actorsThreateningMe_.begin(), actorsThreateningMe_.end(), threatId);
...
bool MachActor::hasThisActorAsAThreat(UtlId threatId /*const MachActor* pThreat*/) const
{
    ...
    Actors::const_iterator i = find(actorsThreateningMe_.begin(), actorsThreateningMe_.end(), threatId);
    ...
    for (Actors::iterator i = actorsThreateningMe_.begin(); i != actorsThreateningMe_.end(); ++i)
    {
            ...
            && (position().sqrEuclidianDistance(actor.position()) < sqrStrongThreatRange))
        {
            ...
            else
            {
                ...
                ASSERT(
                    actor.objectType() == MachLog::MISSILE_EMPLACEMENT,
                    "Non-machine threat actor should only be a missile emplacement!");
...
const MexPoint3d& MachActor::predictedTargetOffset(PhysRelativeTime t)
{
    ...
    if (objectIsMachine())
    {
        ...
        if (timeNow >= nextPredictedPositionRecalculateTime_)
        {
            ...
            MexPoint3d heightAdjustedDestinationPoint
                = MachLogSpacialManipulation::heightAdjustedLocation(predictedDestinationPoint);
