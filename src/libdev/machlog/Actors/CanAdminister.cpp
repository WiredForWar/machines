...
#include "machlog/World/FindSpace.hpp"
...
void MachLogCanAdminister::handleIdleAggressor(MachLogCommsId pObj)
{
    ...
    moveOutOfTheWay(obj);
    //  else
    //      obj->newOperation( new MachLogSeekAndDestroyOperation( obj ) );
}
...
void MachLogCanAdminister::moveOutOfTheWay(MachLogMachine* pObj)
{
    ...
    bool foundGarrison
        = races.findFriendlyGarrisonClosestTo(*pObj, &pGarrison, &pStation, MachLogRaces::FREE_BAYS_ONLY);
    ...
    if (pObj->objectType() == MachLog::AGGRESSOR)
    {
        ...
        ASSERT(
            index < races.aggressorAssemblyPoints(pObj->race()).size(),
            "trying to move aggressor to outside n_points\n");
    ...
    if (pObj->objectType() == MachLog::ADMINISTRATOR)
    {
        ...
        ASSERT(
            index < races.administratorAssemblyPoints(pObj->race()).size(),
            "trying to move administrator to outside n_points\n");
