         ...
         ++i)
    {
            ...
            && ((*i)->isIdle() || (*i)->strategy().currentOperationType() == MachLogOperation::ENTER_OPERATION || (*i)->strategy().currentOperationType() == MachLogOperation::FOLLOW_OPERATION))
        {
            ...
            if ((*i)->objectType() == MachLog::CONSTRUCTOR /* and  allConstructorsIdle*/)
            {
                ...
                pConstructor->newOperation(std::make_unique<MachLogRepairOperation>(pConstructor, pConstruction_));
...
// virtual
bool MachLogAdminRepairOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
