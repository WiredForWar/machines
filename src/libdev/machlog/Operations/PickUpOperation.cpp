...
#include "machlog/Internal/PickUpOperationImpl.hpp"
...
PhysRelativeTime MachLogPickUpOperation::doUpdate()
{
    CB_MachLogPickUpOperation_DEPIMPL();
    ...
    MATHEX_SCALAR sqrDistanceToClosestFreePad
        = 1000000000; // used in cases of mines and smelters to determine which loading bay is closest
    ...
    else
    {
        ...
        for (size_t pointNum = 0; pointNum < smeltingBuilding.nPutDownPoints(); ++pointNum)
        {
            ...
            if (sqrDistanceToTestPad
                <= sqrDistanceToClosestFreePad /*and not smeltingBuilding.pickupPointUsedRecently( pointNum )*/)
            {
...
bool MachLogPickUpOperation::attemptToGuaranteeClearPickupPoint(
    ...
    MexPoint2d* pClearPickupPoint)
{
    ...
    const MexTransform3d testTransform = (actorPos == *pClearPickupPoint)
    ...
    const bool foundSpace = MachLogSpacialManipulation::getNearestFreeSpacePoint(
        testTransform,
        0.0,
        carrierClearance,
        pClearPickupPoint,
