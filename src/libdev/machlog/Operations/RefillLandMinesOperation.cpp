...
PhysRelativeTime MachLogRefillLandMinesOperation::doUpdate()
{
    ...
    MachPhysStation* pStation = nullptr; // dummy value - we don't actually care about the station
    bool foundGarrison = MachLogRaces::instance().findFriendlyGarrisonClosestTo(
        *pActor_,
        &pGarrison,
        &pStation,
        MachLogRaces::FREE_BAYS_UNIMPORTANT);
    ...
    subOperation(pActor_, std::make_unique<MachLogEnterBuildingOperation>(pActor_, pGarrison, nullptr));
    return 10.0;
}
