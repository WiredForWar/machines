...
void MachLogTaskLocateOperation::doOutputOperator(std::ostream& o) const
{
    ...
    o << "nDesiredSites_ " << nDesiredSites_ << " totalWeDiscovered " << totalWeDiscovered_ << " totalDiscovered "
      << totalDiscovered_ << " totalPossibleToDiscover_ " << totalPossibleToDiscover_ << std::endl;
}
...
PhysRelativeTime MachLogTaskLocateOperation::doUpdate()
{
    ...
    MachLogGeoLocator* pLoc = nullptr;
    ...
    if (foundAGoodOne)
    {
        ...
        if (pActor_->hasCommander())
            pActor_->commander().newOperation(std::make_unique<MachLogAdminLocateOperation>(&pActor_->commander(), dest));
        else
        {
            // Check we have a locator
            MachLogGeoLocator* pLoc = nullptr;
            for (MachLogSquadron::Machines::iterator i = pActor_->machines().begin(); i != pActor_->machines().end();
                ...
                {
                    pLoc = &(*i)->asGeoLocator();
                    pLoc->newOperation(std::make_unique<MachLogLocateOperation>(pLoc, dest));
                }
        }
    }
