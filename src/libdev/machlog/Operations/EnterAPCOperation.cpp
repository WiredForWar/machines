...
// virtual
bool MachLogEnterAPCOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
...
PhysRelativeTime MachLogEnterAPCOperation::doUpdate()
{
    ...
    if (status_ == GOING_TO_APC)
    {
        if (! closeEnoughToEnter)
        {
            ...
            MexTransform3d apcToMachineTransform = MachLogSpacialManipulation::sourceToDestination3dTransform(
                apcPosition,
                machinePosition,
            ...
            if (! foundSpace)
                ...
                subOperation(pActor_, std::make_unique<MachLogMoveToOperation>(pActor_, loadPoint, true, 5.0)); // 5m tolerance
        }
        else
        {
