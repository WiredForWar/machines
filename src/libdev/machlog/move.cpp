...
//  MachLogPlanet::instance().surface()->pSpace()->yCells() )
{
    ...
    NETWORK_STREAM(
        "(" << pActor->id() << ") MachLogMoveToOperation::MachLogMoveToOperation (1) " << (void*)this
            << " callstack is:\n"
...
//  MachLogPlanet::instance().surface()->pSpace()->yCells() )
{
    ...
    dest_ = path_.front();
    NETWORK_STREAM(
        "(" << pActor->id() << ") MachLogMoveToOperation::MachLogMoveToOperation (2) " << (void*)this
            << " callstack is:\n"
            << ProStackTracer() << std::endl);
}
...
//  MachLogPlanet::instance().surface()->pSpace()->yCells() )
{
    ...
    path_.push_back(dest_);
    NETWORK_STREAM(
        "(" << pActor->id() << ") MachLogMoveToOperation::MachLogMoveToOperation (3) " << (void*)this
            << " callstack is:\n"
            << ProStackTracer() << std::endl);
}
...
//  MachLogPlanet::instance().surface()->pSpace()->yCells() )
{
    ...
    dest_ = path_.front();
    NETWORK_STREAM(
        "(" << pActor->id() << ") MachLogMoveToOperation::MachLogMoveToOperation (4) " << (void*)this
            << " callstack is:\n"
            << ProStackTracer() << std::endl);
}
...
bool MachLogMoveToOperation::doStart()
{
    ...
    HAL_STREAM(
        "(" << pActor_->id() << ") MLMoveToOperation::doStart move to 3D " << dest_ << " 2D " << dest << std::endl);
