...
// virtual
bool MachLogHealOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int clientData)
{
...
PhysRelativeTime MachLogHealOperation::moveCloserToTarget()
{
    ...
    else
    {
        ...
        if (not pActor_->asCanAttack().withinSqrMaximumWeaponRange(sqrDistanceBetweenDestinationAndTarget))
        {
            ...
            if (distanceTargetMovedSinceAttackerChoseDestination / distanceToTargetNow
                > 0.45) // "suck it and see" value
                recalculateDestination = true;
        }
    }
