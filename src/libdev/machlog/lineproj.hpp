...
public:
    ...
    ~MachLogLinearProjectile() override;
    ...
    friend std::ostream& operator<<(std::ostream& o, const MachLogLinearProjectile& t);

    const MexPoint3d& globalDestructionPosition() const;

    int damage() const;

    static void genericCheckForDamage(
        const MexPoint3d&,
        MATHEX_SCALAR range,
        DamageCurveShape,
        int damage,
        MachPhys::WeaponType wt = MachPhys::N_WEAPON_TYPES,
        MexLine3d* line = nullptr,
