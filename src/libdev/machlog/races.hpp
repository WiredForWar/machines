...
public:
    ...
    {
        AGGRESSIVE_MACHINES,
        MISSILE_EMPLACEMENTS,
        GARRISONS,
        PODS,
        AGGRESSIVE_MACHINES_AND_MISSILE_EMPLACEMENTS
    };
    ...
    using Objects = ctl_list<MachActor*>;
    Objects& objects();
    ...
    using Aggressors = ctl_list<MachLogAggressor*>;
    using Administrators = ctl_list<MachLogAdministrator*>;
    using Technicians = ctl_list<MachLogTechnician*>;
    using Constructors = ctl_list<MachLogConstructor*>;
    using GeoLocators = ctl_list<MachLogGeoLocator*>;
    using SpyLocators = ctl_list<MachLogSpyLocator*>;
    using ResourceCarriers = ctl_list<MachLogResourceCarrier*>;
    using APCs = ctl_list<MachLogAPC*>;

    ///////////////////////////////

    using Garrisons = ctl_list<MachLogGarrison*>;
    using HardwareLabs = ctl_list<MachLogHardwareLab*>;
    using Factories = ctl_list<MachLogFactory*>;
    using Smelters = ctl_list<MachLogSmelter*>;
    using Mines = ctl_list<MachLogMine*>;
    using MissileEmplacements = ctl_list<MachLogMissileEmplacement*>;
    using Pods = ctl_list<MachLogPod*>;
    using Holographs = ctl_list<MachLogOreHolograph*>;
    using Beacons = ctl_list<MachLogBeacon*>;
    ...
    enum FindGarrisonCriterion
    {
        FREE_BAYS_ONLY,
        FREE_BAYS_UNIMPORTANT,
        FREE_BAYS_PLUS_NO_ENEMY_CANATTACKS
    };
    bool findFriendlyGarrisonClosestTo(
        const MachLogMachine&,
    ...
    enum CreateDefaultSquadrons
    {
        CREATE_SQUADRONS,
        DO_NOT_CREATE_SQUADRONS
    };
    ...
    bool findAlternativeTargetClosestTo(
        const MachActor& actor,
        MachActor** ppResult,
        MATHEX_SCALAR minEuclidianDistance = 0) const;
    ...
    int findSafestFriendly(const MachActor& actor, MachActor** ppResult, SafeActorType safeActorType);

    void allFriendlyAggressiveMachinesWithinRange(
        const MachActor* pActor,
        MATHEX_SCALAR range,
        ctl_pvector<MachActor>* pActors,
        MATHEX_SCALAR minRange = 0);

    void allFriendlyAggressivesWithinRange(
        const MachActor* pActor,
        MATHEX_SCALAR range,
        ctl_pvector<MachActor>* pActors,
        MATHEX_SCALAR minRange = 0);
    ...
    // these methods remove value passed as second parameter from race's BMU total, after clipping for being over
    // maximum or less than 0 return value == value actually added or subtracted after clipping
    ...
    reduceCapacityWithPenalty(MachPhys::Race r, MachPhys::BuildingMaterialUnits amountReducedBy);
    MachPhys::BuildingMaterialUnits
    reduceCapacityNoPenalty(MachPhys::Race r, MachPhys::BuildingMaterialUnits amountReducedBy);
    MachPhys::BuildingMaterialUnits
    increaseRaceCapacity(MachPhys::Race r, MachPhys::BuildingMaterialUnits amountIncreasedBy);
    ...
    void actorDestroyedScoreAdjust(
        MachPhys::Race losingRace,
        int value,
        const MachActor& victimActor,
        MachPhys::Race gainingRace = MachPhys::N_RACES);
