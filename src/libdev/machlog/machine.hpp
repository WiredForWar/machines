...
public:
    ...
    bool setSquadron(MachLogSquadron*);
    ...
    void smeltMe(); // will execute a vapourisation animation on associated physobject, and set up a dying event without
                    // debris

    virtual MachPhys::MachineType type() const;

    using Self = MachLogMachine;

    friend bool operator<(const Self& a, const Self& b) { return a.id() < b.id(); }
    ...
    // inherited from MachActor
    void populateStrongThreats(Actors* pActors) override;
    ...
    bool beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int) override;
...
private:
    ...
    // inherited from MachActor
    void dispatchSOS(Actors& strongThreats) override;
