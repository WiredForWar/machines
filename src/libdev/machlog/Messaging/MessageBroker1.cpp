...
void MachLogMessageBroker::sendMachineMoveMessage(
    UtlId whichId,
    const MATHEX_SCALAR& topSpeed,
    const ctl_vector<MachPhysMachineMoveInfo>& infos,
...
)
{
    ...
    else
    {
        ...
        pMoveMessage->header_.totalLength_ = sizeof(MachLogMachineMoveMessage) - MachLogMachineMoveMessage::dataSize()
        ...
        for (std::size_t i = 0; i < infos.size(); ++i)
        {
            ...
            memcpy(
                (void*)(&pMoveMessage->data_[0] + i * sizeof(MachLogSingleMoveInfo)),
                (void*)&moveInfo,
                sizeof(MachLogSingleMoveInfo));
        }
...
void MachLogMessageBroker::processMachineMoveMessage(NetMessage* pMessage)
{
    ...
    MachLogMachineMoveMessage* pMoveMessage
        = reinterpret_cast<MachLogMachineMoveMessage*>(const_cast<uint8*>(&pMessage->body().body()[0]));
        ...
        if (actor.objectIsMachine())
        {
            ...
            for (int i = 0; i < pMoveMessage->nMoveInfos_; ++i)
            {
                ...
                MachPhysMachine::ProfilePtr profilePtr = pPhysMachine->profile(
                    pMoveInfo->startTransform_,
                    MachPhysMachine::USE_ORIENTATION,
                    pMoveInfo->endPosition_,
...
void MachLogMessageBroker::processMachineSetDomainMessage(NetMessage* pNetMessage)
{
    ...
    MachLogMachineSetDomainMessage* pMessage
        = reinterpret_cast<MachLogMachineSetDomainMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processMachineTurnMessage(NetMessage* pNetMessage)
{
    ...
    MachLogMachineTurnMessage* pMessage
        = reinterpret_cast<MachLogMachineTurnMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processAddRestingObstacleMessage(NetMessage* pNetMessage)
{
    ...
    MachLogRestingObstacleMessage* pMessage
        = reinterpret_cast<MachLogRestingObstacleMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processAddRestingObstacleShortMessage(NetMessage* pNetMessage)
{
    ...
    // &pNetMessage->body().body()[0] );
    MachLogRestingObstacleShortMessage* pMessage
        = reinterpret_cast<MachLogRestingObstacleShortMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::sendRequestAddRestingObstacleMessage(
    UtlId whichId,
...
// ); old   DEBUG_STREAM( DIAG_NETWORK, "MLMessageBroker::sendAddMovingObstacleMessage DONE " << std::endl ); old} old
// oldvoid MachLogMessageBroker::processAddMovingObstacleMessage( NetMessage* pNetMessage )
// old{
// old   MachLogMovingObstacleMessage* pMessage = _REINTERPRET_CAST( MachLogMovingObstacleMessage*,
// &pNetMessage->body().body()[0] ); old   DEBUG_STREAM( DIAG_NETWORK,"processAddMovingObstacleMessage " <<
// pMessage->whichId_ << std::endl ); old   ASSERT( MachLogRaces::instance().actorExists( pMessage->whichId_ ), "Actor
// does not exists" ); old   if( MachLogRaces::instance().actorExists( pMessage->whichId_ ) ) old   { old MachActor&
// actor = MachLogRaces::instance().actor( pMessage->whichId_ ); old       ASSERT( actor.objectIsMachine(), "actor is
// not a machine - oops.\n" ); old       if( actor.objectIsMachine() ) old       { old MachLogMachineMotionSequencer&
// motionSeq = actor.asMachine().motionSeq(); old           motionSeq.changeStatus( pMessage->status_ ); old//  TBD:
// Remove this entire function old//         motionSeq.addMovingObstacle( pMessage->start_, pMessage->end_ ); old //by
// this time the original intersecting relationship has been removed by adding a temporary resting obstacle old //while
// the node was waiting for a confirm. (Or other nodes other than host). old motionSeq.setIntersectingDomains(); old }
// old   }
// old}
// old
...
// pMessage->end_ = end; old   pMessage->status_ = status; old   doSend( pLogNetMessage ); old   DEBUG_STREAM(
// DIAG_NETWORK, "MLMessageBroker::sendRequestAddMovingObstacleMessage DONE " << std::endl ); old} old oldvoid
// MachLogMessageBroker::processRequestAddMovingObstacleMessage( NetMessage* pNetMessage ) old{ old
// MachLogMovingObstacleMessage* pMessage = _REINTERPRET_CAST( MachLogMovingObstacleMessage*,
// &pNetMessage->body().body()[0] ); old   DEBUG_STREAM( DIAG_NETWORK,"processRequestAddMovingObstacleMessage " <<
// pMessage->whichId_ << std::endl ); old   if( MachLogNetwork::instance().isNodeLogicalHost() ) old   { old ASSERT(
// MachLogRaces::instance().actorExists( pMessage->whichId_ ), "Actor does not exists" ); old       if(
// MachLogRaces::instance().actorExists( pMessage->whichId_ ) ) old       { old           MachActor& actor =
// MachLogRaces::instance().actor( pMessage->whichId_ ); old           ASSERT( actor.objectIsMachine(), "actor is not a
// machine - oops.\n" ); old           if( actor.objectIsMachine() ) old           { old
// actor.asMachine().motionSeq().changeStatus( pMessage->status_ ); old//  TBD: Remove this entire function old//
// actor.asMachine().motionSeq().addMovingObstacle( pMessage->start_, pMessage->end_ ); old
// sendAddMovingObstacleMessage( pMessage->whichId_, pMessage->start_, pMessage->end_, pMessage->status_ ); old } old }
// old   }
// old}
// old
void MachLogMessageBroker::sendAddMotionChunksMessage(
    UtlId whichId,
    const ctl_vector<PhysMotionChunk>& chunks,
    int status)
{
    ...
    pMessage->header_.totalLength_ = sizeof(MachLogMotionChunksMessage) - MachLogMotionChunksMessage::dataSize()
        + chunks.size() * sizeof(PhysCompressedMotionChunk);
    pMessage->whichId_ = whichId;
    ...
    for (std::size_t i = 0; i < chunks.size(); ++i)
    {
        ...
        memcpy(
            (void*)(&pMessage->data_[0] + i * sizeof(PhysCompressedMotionChunk)),
            (void*)&chunk,
            sizeof(PhysCompressedMotionChunk));
    }
    doSend(pLogNetMessage);
...
void MachLogMessageBroker::processAddMotionChunksMessage(NetMessage* pNetMessage)
{
    ...
    MachLogMotionChunksMessage* pMessage
        = reinterpret_cast<MachLogMotionChunksMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::sendRequestAddMotionChunksMessage(
    UtlId whichId,
    const ctl_vector<PhysMotionChunk>& chunks,
    int status)
{
...
void MachLogMessageBroker::processMotionChunksAddFailedMessage(NetMessage* pNetMessage)
{
    ...
    // &pNetMessage->body().body()[0] );
    MachLogMotionChunksAddFailedMessage* pMessage
        = reinterpret_cast<MachLogMotionChunksAddFailedMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processClearRestingObstacleMessage(NetMessage* pNetMessage)
{
    ...
    // &pNetMessage->body().body()[0] );
    MachLogClearRestingObstacleMessage* pMessage
        = reinterpret_cast<MachLogClearRestingObstacleMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processClearMovingChunksMessage(NetMessage* pNetMessage)
{
    ...
    MachLogMovingObstacleMessage* pMessage
        = reinterpret_cast<MachLogMovingObstacleMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::sendStopDeadMessage(UtlId whichId, const MexTransform3d& transform)
{
    ...
    DEBUG_STREAM(
        DIAG_NETWORK,
        " transform " << transform << " domain at info " << MachLogPlanetDomains::pDomainAt(transform.position())
                      << std::endl);
    DEBUG_STREAM(
        DIAG_NETWORK,
        " local transfrom here is "
            << const_cast<const MachActor&>(MachLogRaces::instance().actor(whichId)).physObject().localTransform()
            << std::endl);
    doSend(pLogNetMessage);
...
void MachLogMessageBroker::processStopDeadMessage(NetMessage* pNetMessage)
{
    ...
    MachLogStopDeadMessage* pMessage
        = reinterpret_cast<MachLogStopDeadMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processAdjustHPAndArmourMessage(NetMessage* pNetMessage)
{
    ...
    MachLogAdjustHPAndArmourMessage* pMessage
        = reinterpret_cast<MachLogAdjustHPAndArmourMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processCreateActorMessage(NetMessage* pMessage)
{
    ...
    MachLogCreateActorMessage* pCreateMessage
        = reinterpret_cast<MachLogCreateActorMessage*>(const_cast<uint8*>(&pMessage->body().body()[0]));
    ...
    switch (pCreateMessage->objectType_)
    {
        ...
        case MachLog::ORE_HOLOGRAPH:
            MachLogActorMaker::newLogOreHolograph(
                pCreateMessage->race_,
                pCreateMessage->hwLevel_,
                pCreateMessage->swLevel_,
                pCreateMessage->position_,
        ...
        case MachLog::ARTEFACT:
            if (pCreateMessage->hwLevel_ == 0)
                MachLogActorMaker::newLogArtefact(
                    pCreateMessage->subType_,
                    pCreateMessage->position_,
                    pCreateMessage->orientation_,
                    pCreateMessage->whichId_);
            else
                MachLogActorMaker::newDumbArtefact(
                    pCreateMessage->subType_,
                    pCreateMessage->position_,
                    pCreateMessage->orientation_);
            break;
...
void MachLogMessageBroker::processCreateSpecialActorMessage(NetMessage* pLogMessage)
{
    ...
    MachLogCreateSpecialActorMessage* pMessage
        = reinterpret_cast<MachLogCreateSpecialActorMessage*>(const_cast<uint8*>(&pLogMessage->body().body()[0]));
...
void MachLogMessageBroker::processReadyMessage(NetMessage* pMessage)
{
    ...
    MachLogReadyMessage* pReadyMessage
        = reinterpret_cast<MachLogReadyMessage*>(const_cast<uint8*>(&pMessage->body().body()[0]));
...
void MachLogMessageBroker::processResyncTimeMessage(NetMessage* pMessage)
{
    ...
    MachLogResyncTimeMessage* pResyncMessage
        = reinterpret_cast<MachLogResyncTimeMessage*>(const_cast<uint8*>(&pMessage->body().body()[0]));
...
void MachLogMessageBroker::processUseSpaceDomainMessage(NetMessage* pNetMessage)
{
    ...
    MachLogUseSpaceDomainMessage* pMessage
        = reinterpret_cast<MachLogUseSpaceDomainMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processFirstPersonEndMessage(NetMessage* pNetMessage)
{
    ...
    MachLogFirstPersonEndMessage* pMessage
        = reinterpret_cast<MachLogFirstPersonEndMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processFirstPersonUpdateMessage(NetMessage* pNetMessage)
{
    ...
    MachLogFirstPersonUpdateMessage* pMessage
        = reinterpret_cast<MachLogFirstPersonUpdateMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
...
void MachLogMessageBroker::processEnteredBuildingMessage(NetMessage* pNetMessage)
{
    ...
    MachLogEnteredBuildingMessage* pMessage
        = reinterpret_cast<MachLogEnteredBuildingMessage*>(const_cast<uint8*>(&pNetMessage->body().body()[0]));
    DEBUG_STREAM(
        DIAG_NETWORK,
    ...
    if (races.actorExists(pMessage->whichId_))
    {
        ...
        if (! actor.isIn1stPersonView())
        {
            ...
            {
                if (actor.objectIsMachine())
                    actor.asMachine().insideWhichBuilding(nullptr);
            }
        }
    }
}
...
std::ostream& operator<<(std::ostream& o, const MachLogSingleMoveInfo& t)
{
    o << "startTransform_ " << std::endl
      ...
      << "endPosition_ " << t.endPosition_ << std::endl;
    o << "startSpeed_ " << t.startSpeed_ << " startTime_ " << t.startTime_ << " finalState_ " << int(t.finalState_)
      << std::endl;
    return o;
};
