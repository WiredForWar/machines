...
bool MachLogSquadron::addToControl(MachLogMachine* p)
{
    ...
    // Needs doing.
    //   HAL_STREAM(" Administrator (" << id() << ") is gaining machine " << p->id() << " " << p->objectType() <<
    //   std::endl ); HAL_STREAM(" Administrator (" << id() << ") objects.size BEFORE " << objects().size() << std::endl
    //   );
    CB_MachLogSquadron_DEPIMPL();
    ...
    MachLogProductionUnit
        prod(p->objectType(), p->subType(), p->hwLevel(), p->swLevel(), MachLogProductionUnit::UNCHECKED_PRIORITY);
...
template <class CONTAINER, class T> void squadron_ctl_erase(CONTAINER* cPtr, const T& t)
{
...
void MachLogSquadron::releaseAllMachines()
{
    ...
    {
        (*machines_.begin())->setSquadron(nullptr);
    }
    ...
    if (checkIfMachineMissing(&prod))
    {
        if (! MachLogRaces::instance().addMatchingMachineToSquadron(race(), prod, this))
        {
            ...
            if (MachLogRaces::instance().getSuitableFactory(race(), prod, &mlf))
                mlf->buildMachineUniqueWithSquadIndex(
                    prod.type(),
                    prod.subType(),
                    prod.hwLevel(),
                    prod.swLevel(),
