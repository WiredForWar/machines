...
void MachLogTaskAttackOperation::doOutputOperator(std::ostream& o) const
{
    ...
    o << " initialDelay_ " << initialDelay_ << " processed " << initialDelayProcessed_ << " use: Ion C "
      << useIonCannon_ << " Nuke " << useNuke_ << std::endl;
    ...
    if (specificObjectType_)
        ...
        o << std::endl;
    o << " assembly " << assemblyPoint_ << " minimium " << nMinimumMachines_ << " do proximity check "
      << doProximityCheck_ << std::endl;
...
PhysRelativeTime MachLogTaskAttackOperation::doUpdate()
{
    ...
    if (! done && ! initialDelayProcessed_)
    {
        ...
        {
            for (MachLogSquadron::Machines::iterator i = pActor_->machines().begin(); i != pActor_->machines().end();
                 ++i)
            {
                if ((*i)->isIdle())
...
PhysRelativeTime MachLogTaskAttackOperation::doUpdateWithoutCommander()
{
    ...
    if (state_ == RAMPAGING)
    {
        ...
        if (pCanAttackActor)
        {
            ...
            if (MachLogRaces::instance().findTargetClosestTo(
                    *pCanAttackActor,
                    &pTarget,
                    targetSystemType_,
                    objectType_,
                    pCanAttackActor->position()))
            {
...
void MachLogTaskAttackOperation::tryNukeAttack()
{
    ...
    for (MachLogRaces::MissileEmplacements::iterator i = races.missileEmplacements(pActor_->race()).begin();
         i != races.missileEmplacements(pActor_->race()).end();
         ++i)
    {
        ...
        if (emplacement.subType() == MachPhys::ICBM)
        {
            if (emplacement.domeOpen())
            {
                ...
                if (races.findMostValuableTarget(
                        emplacement,
                        &pTarget,
                        nukeTargetType,
                        objectType_,
...
PhysRelativeTime MachLogTaskAttackOperation::attackSpecificTarget(const MachActor& targetActor)
{
    ...
    //  weaponRange " << ( maxWeaponRange * maxWeaponRange ) << std::endl );
...
MexPoint2d MachLogTaskAttackOperation::positionOfRampagingMachineNearestTargetPoint(const MexPoint2d& targetPoint) const
{
    ...
    ASSERT(pClosestMachine != nullptr, "No machine was found!");
