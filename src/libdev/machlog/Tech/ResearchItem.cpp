...
#include "machphys/Machines/AdministratorData.hpp"
    ...
    : pImpl_(new MachLogResearchItemImpl())
{
    ...
    switch (objectType)
    {
        case MachLog::AGGRESSOR:
            ...
            //          ) )
            factoryInstanceCost_
                = MachPhysData::instance().aggressorData((MachPhys::AggressorSubType)subType, hwLevel, swLevel).cost();
            break;
        case MachLog::ADMINISTRATOR:
            //          if( MachPhysLevels::instance().levelValid( (MachPhys::AdministratorSubType)subType, hwLevel,
            //          swLevel ) )
            factoryInstanceCost_ = MachPhysData::instance()
                                       .administratorData((MachPhys::AdministratorSubType)subType, hwLevel, swLevel)
                                       .cost();
            break;
        ...
        case MachLog::CONSTRUCTOR:
            //          if( MachPhysLevels::instance().levelValid( (MachPhys::ConstructorSubType)subType, hwLevel,
            //          swLevel )
            //          )
            factoryInstanceCost_ = MachPhysData::instance()
                                       .constructorData((MachPhys::ConstructorSubType)subType, hwLevel, swLevel)
                                       .cost();
            break;
        ...
        case MachLog::TECHNICIAN:
            //          if( MachPhysLevels::instance().levelValid( (MachPhys::TechnicianSubType)subType, hwLevel,
            //          swLevel )
            //          )
            factoryInstanceCost_ = MachPhysData::instance()
                                       .technicianData((MachPhys::TechnicianSubType)subType, hwLevel, swLevel)
                                       .cost();
            break;
        default:; // we don't need to do anything for things not machines
    }
}
...
int MachLogResearchItem::subType() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
const MachLogMachine::Level& MachLogResearchItem::hwLevel() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
const MachPhys::WeaponCombo MachLogResearchItem::weaponCombo() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
bool MachLogResearchItem::hasWeaponCombo() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
const MachLogMachine::Level& MachLogResearchItem::swLevel(MachPhys::Race race) const
{
    CB_MachLogResearchItem_DEPIMPL();
...
void MachLogResearchItem::swLevel(MachPhys::Race race, int level)
{
    ...
    HAL_STREAM(
        "MachLogResearchItem [" << objectType_ << "," << subType_ << "," << hwLevel_ << "] swLevel for " << race
...
bool MachLogResearchItem::hasParentDependancy() const
{
    ...
    return pParentDependancy_ != nullptr;
}

MachLogResearchItem& MachLogResearchItem::parentDependancy() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
int MachLogResearchItem::technologyLevel() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
const MachPhys::ResearchUnits& MachLogResearchItem::researchCost() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
const MachPhys::BuildingMaterialUnits& MachLogResearchItem::buildingCost() const
{
    CB_MachLogResearchItem_DEPIMPL();
...
void MachLogResearchItem::isAvailable(MachPhys::Race race, bool value)
{
    ...
    //  HAL_STREAM("MLRI::available " << objectType_ << " sub " << subType_ << " hw " << hwLevel_ << " value " << value
    //  << std::endl );
...
const MachPhys::ResearchUnits& MachLogResearchItem::swResearchCost(int level) const
{
    CB_MachLogResearchItem_DEPIMPL();
...
bool MachLogResearchItem::swAvailable(MachPhys::Race race, int level) const
{
    CB_MachLogResearchItem_DEPIMPL();
...
bool MachLogResearchItem::swResearched(MachPhys::Race race, int level) const
{
    CB_MachLogResearchItem_DEPIMPL();
...
void MachLogResearchItem::swAdvanceResearch(
    MachPhys::Race race,
    int level,
    MachPhys::ResearchUnits rAmount,
    MachPhys::BuildingMaterialUnits bAmount)
{
    CB_MachLogResearchItem_DEPIMPL();
...
std::ostream& operator<<(std::ostream& o, const MachLogResearchItem& t)
{
    ...
    if (t.pParentDependancy())
        o << t.parentDependancy().objectType() << " " << t.parentDependancy().subType() << " "
          << t.parentDependancy().hwLevel() << std::endl;
    else
        o << "NULL\n";
    ...
    for (int i = 0; i < MachLogResearchItem::N_RESEARCH_RACES; ++i)
    {
        for (int j = 0; j < MachLogResearchItem::MAX_SW_LEVELS; ++j)
        {
            ...
            o << " swAmount researched[" << i << "," << j << "] " << t.swAmountResearched((MachPhys::Race)i, j)
              << std::endl;
