...
#include "machlog/Controllers/Internal/AIControllerImpl.hpp"
...
static bool mineSiteIsOccupied(const MexPoint3d& dest)
{
    ...
    if (found)
    {
        ...
        const MexAlignedBox2d& localBoundary
            = MachPhysData::instance().mineData(1).localBoundary(); // MachLogMine::constructionData().localBoundary();
...
void MachLogAIController::handleEnemyContact(const MachLogMessage& msg)
{
    ...
    if (senderActor.objectIsMachine())
    {
        ...
        if (! senderMachine.insideBuilding())
        {
            ...
            MachLogMachine* pIdleAggressiveMachine = nullptr;
            ...
            if (pIdleAggressiveMachine == nullptr)
            {
            ...
            if (pIdleAggressiveMachine != nullptr)
            {
                ...
                if (MachLogSpacialManipulation::getNearestFreeSpacePoint(
                        ...
                        5.0,
                        pIdleAggressiveMachine->highClearence(),
                        &targetPosition))
                {
...
void MachLogAIController::handleIdleTechnician(MachLogCommsId pObj)
{
    ...
    MachLogHardwareLab* pChosenLab = nullptr;
...
void MachLogAIController::handleIdleAdministrator(MachLogCommsId pObj)
{
    ...
    //  //HAL_STREAM( " Race: " << obj->race() << " aggressors " << MachLogRaces::instance().aggressors( obj->race()
    //  ).size() << std::endl );
...
void MachLogAIController::doOutputOperator(std::ostream& o) const
{
    ...
    if (constructionProductionList_.size() > 0)
    {
        for (MachLogAIController::ProductionList::const_iterator i = constructionProductionList_.begin();
             i != constructionProductionList_.end();
             ++i)
        {
            o << "  " << (*i)->type() << " sub " << (*i)->subType() << " Pri: " << (*i)->priority();
            o << " at:" << (*i)->globalTransform().position() << " place "
              << MachLogTaskConstructOperation::couldPlaceConstruction((**i)) << std::endl;
        }
    }
    ...
    if (strategicProductionList_.size() > 0)
    {
        for (MachLogAIController::StrategicProductionList::iterator i = strategicProductionList_.begin();
             i != strategicProductionList_.end();
             ++i)
...
void MachLogAIController::machineCreated(const MachLogMachine& mlm)
{
    ...
    for (DesiredMachineList::iterator i = desiredMachineList_.begin(); i != desiredMachineList_.end(); ++i)
...
bool MachLogAIController::getHighestPriorityConstruction(MachLogProductionUnit* pProd)
{
    ...
    if (constructionProductionList_.size() > 0)
    {
        ...
        for (ProductionList::iterator i = constructionProductionList_.begin(); i != constructionProductionList_.end();
             ++i)
        {
            ...
            bool canBuild = MachLogRaces::instance().constructionTree().activated(
                race(),
            ...
            if (canBuild)
            {
                ...
                {
                    bool ableToPlaceConstruction
                        = MachLogTaskConstructOperation::couldPlaceConstruction(candidateProdUnit);
                    HAL_STREAM("Can we place the construction?" << ableToPlaceConstruction << std::endl);

                    if (ableToPlaceConstruction)
                    {
                        bool thisIsNotDuplicate
                            = MachLogRaces::instance().noAlternativeAlreadyBuilt(race(), candidateProdUnit);
                        HAL_STREAM(
                            "Have we confirmed no alternative has already been built?" << thisIsNotDuplicate
                                                                                       << std::endl);
