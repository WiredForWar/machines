...
{
    CB_MachLogCanAttack_DEPIMPL();
    return pCurrentTarget_ != nullptr;
}
...
bool MachLogCanAttack::currentTargetIsOneOfThese(const Actors& strongThreats)
{
    ...
    ASSERT(
        hasCurrentTarget(),
        "Calling MachLogCanAttack::currentTargetIsOneOfThese when we don't even HAVE a current target.");
...
// virtual
bool MachLogCanAttack::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int)
{
    CB_MachLogCanAttack_DEPIMPL();
    switch (event)
    {
...
void MachLogCanAttack::checkAndAttackCloserTarget(MachLogMissileEmplacement* pActor, MachActor* pFiredAtMe)
{
    ...
    {
        found = races.findTargetClosestTo(
            *pActor,
            &pMach,
...
void MachLogCanAttack::checkAndHealCloserTarget(MachLogMachine* pActor)
{
    ...
    for (MachActor* pObject : objects)
    {
        ...
        if (pTarget != nullptr && distance == bestDistanceSquared && pObject->id() >= pTarget->id())
            continue;
    ...
    if (pTarget)
    {
        ...
        pActor->strategy().newOperation(std::make_unique<MachLogHealOperation>(pAsAdmin, pTarget));
    }
}
...
bool MachLogCanAttack::inWeaponRange(const MachActor& other, MachLogCanAttack::SpecialWeaponRange specialType) const
{
    ...
    {
        case NONE:
            weaponRange = getMaximumWeaponRange() * 0.95;
            break;
        case ORB_OF_TREACHERY:
            weaponRange = getMaximumTreacheryWeaponRange() * 0.95;
            break;
        case SUPERCHARGE:
            weaponRange = getMaximumHealWeaponRange() * 0.95;
            break;
        case GORILLA_PUNCH:
            weaponRange = getMaximumPunchWeaponRange() * 0.95;
            break;
        default:
    ...
    {
        result = MachLogSpacialManipulation::withinRangeOfMexAlignedBox2d(
            other.asConstruction().globalBoundary(),
            pMe_->position(),
            weaponRange);
    }
    else
...
bool MachLogCanAttack::targetBehindSoftCover(MachActor* pTarget, MachLogCanAttack::SoftCoverAccuracy precision)
{
    ...
    else
    {
        ...
        if( pMe_->asMachine().insideBuilding() and not planetSpace )
        {
            ...
            {
                cSpace.isPolygonEnabled( pMe_->asMachine().insideWhichBuilding().entrance( i ).interiorPolygonId(),
    false ); HAL_STREAM("  remove polygon " << pMe_->asMachine().insideWhichBuilding().entrance( i ).interiorPolygonId()
    << " for entrance " << i << std::endl );
            }
        }
    }
    ...
    ); DEBUG_STREAM( DIAG_HAL,"  add polygon " << pMe_->asMachine().insideWhichBuilding().entrance( i
    ).interiorPolygonId() << " for entrance " << i << std::endl );
            }
        }
    }
...
MachLogCanAttack::WeaponDisposition MachLogCanAttack::rangeData(
    const MexTransform3d& attackFrame,
    const W4dEntity& target,
    const MachPhysWeaponData& weaponData,
    MATHEX_SCALAR heightModifier,
    MexPoint3d* pAimPoint)
{
    MISC_STREAM("MachLogCanAttack::rangeData entry" << std::endl);
    WeaponDisposition disposition;

    // Get the bounding volume of the target
    const MexAlignedBox3d& targetBoundary
        = (target.isComposite() ? target.asComposite().compositeBoundingVolume() : target.boundingVolume());
...
MachLogCanAttack::WeaponDisposition
MachLogCanAttack::behindCover(const MexLine3d& projectilePath, SoftCoverAccuracy precision) const
{
    CB_MachLogCanAttack_DEPIMPL();
...
MachLogCanAttack::WeaponDisposition MachLogCanAttack::testActorBehindCover(
    MachActor* pSubjectActor,
    MachActor* pTestActor,
    const MexLine3d& projectilePath,
    SoftCoverAccuracy precision)
{
    ...
    bool hit = pDomain->findNearerEntity(
        projectilePath,
...
bool MachLogCanAttack::IHaveClearWeaponPathToHim(MachActor* pSubjectActor, MachActor* pTargetActor)
{
    ...
    MexPoint3d myPosition
        = (meEntity.isComposite() ? meEntity.asComposite().compositeBoundingVolume().centroid()
    ...
    MexPoint3d globalAimPoint
        = (targetEntity.isComposite() ? targetEntity.asComposite().compositeBoundingVolume().centroid()
    ...
    MachLogCanAttack::WeaponDisposition disposition
        = testActorBehindCover(pSubjectActor, pTargetActor, projectilePath, MachLogCanAttack::SAME_RACE_OK);
...
void MachLogCanAttack::diminishAlertnessAndInaccuracy()
{
    ...
    if (reduceAlertnessBy > 0)
    {
        ...
        alertness_ -= reduceAlertnessBy; // arbitrary for now. Should really be passed relative time parameter and
...
bool MachLogCanAttack::switchFromExistingTargetToThisOne(
    MachActor* pAlternativeActor,
    MATHEX_SCALAR sqrDistanceToCurrentTarget,
    MachActor* pFiredAtMe)
{
    CB_MachLogCanAttack_DEPIMPL();
