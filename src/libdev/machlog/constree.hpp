...
public:
    ...
    MachLogConstructionTree();
    // PRE  A text file called "global.cst" exists (in the data/ path) with a list of construction items specified in
    // the
    //      following format:
    //
    ...
    bool activated(
        ...
        MachPhys::WeaponCombo wc = MachPhys::N_WEAPON_COMBOS) const;

    void updateAvailableConstructions(
        MachPhys::Race r,
        MachPhys::ConstructionType constructionType,
        int subType,
        size_t hwLevel,
    ...
    // POST All MachLogConstructionItem elements up to and including the matched element in the (linear) tree, in the
    // order
