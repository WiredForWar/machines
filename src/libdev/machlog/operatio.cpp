...
ostream& operator<<(ostream& o, const MachLogOperation& op)
{
    ...
    if (op.pSubOperation() != nullptr)
    {
        ...
        if (op.pSubOperation()->pSubOperation() != nullptr)
        {
            op.pSubOperation()->pSubOperation()->doOutputOperator(o);
            if (op.pSubOperation()->pSubOperation()->pSubOperation() != nullptr)
...
// virtual
bool MachLogOperation::beNotified(W4dSubject*, W4dSubject::NotificationEvent, int)
{
    TEST_INVARIANT;
