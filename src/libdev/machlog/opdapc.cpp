...
// virtual
bool MachLogDeployAPCOperation::beNotified(W4dSubject*, W4dSubject::NotificationEvent, int)
{
    return true;
}
...
PhysRelativeTime MachLogDeployAPCOperation::doUpdate()
{
    CB_MachLogDeployAPCOperation_DEPIMPL();
    ...
    if (status_ == DEPLOY_MACHINES)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!pAPC_->nextDeployPoint(
                        &deployPoint,
                        (*i)->highClearence())) // Returns updated deployPoint as side-effect if found.
                {
