...
bool findTargetClosestTo(const MachLogMachine& actor, MachActor** ppResult)
{
    bool found = false;
...
PhysRelativeTime MachLogSeekAndDestroyOperation::doUpdate()
{
    ...
    if (found && pTarget->objectIsMachine())
    {
        ...
        if (found)
        {
            ...
            {
                ASSERT(
                    pActor_->objectType() == MachLog::ADMINISTRATOR,
                    "Attacking machine was neither aggressor nor administrator!")
...
// virtual
bool MachLogSeekAndDestroyOperation::beNotified(
    W4dSubject* pSubject,
...
void MachLogSeekAndDestroyOperation::administratorUpdateWithTarget(MachActor* pTarget)
{
    ...
    if (administrator.squadron())
    {
        ...
        // of the target
        bool issueAttack
            = pActor_->position().euclidianDistance(pTarget->position()) > pActor_->effectiveScannerRange();

        int convoyIndex = 0;
        for (MachLogSquadron::Machines::iterator i = administrator.squadron()->machines().begin();
             i != administrator.squadron()->machines().end();
             ++i)
        {
