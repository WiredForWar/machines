...
void MachLogEvadeOperation::doOutputOperator(std::ostream& o) const
{
    ...
    {
        case SEEK_GARRISON:
            o << "[ Moving to garrison. ]" << std::endl;
            break;
        case SEEK_AGGRESSIVE:
            o << "[ Moving to aggressive: ]" << std::endl;
            break;
        case SEEK_TURRET:
            o << "[ Moving to turret. ]" << std::endl;
            break;
        case SEEK_POD:
            o << "[ Moving to pod. ]" << std::endl;
            break;
        default:
            {
...
PhysRelativeTime MachLogEvadeOperation::doUpdate()
{
    ...
    ASSERT(
        ...
        "MachLogEvadeOperation::doUpdate : Hey, we shouldn't have a null pointer for the cached op by now!");
         ...
         ++priority)
    {
        ...
        {
            bool foundGarrison = MachLogRaces::instance().findFriendlyGarrisonClosestTo(
                *pActor_,
                &pGarrison,
                &pStation,
                MachLogRaces::FREE_BAYS_PLUS_NO_ENEMY_CANATTACKS);
        ...
        {
            MachLogMachine* pFriendlyMachine = nullptr;
        ...
        {
            MachLogMissileEmplacement* pFriendlyMissileEmplacement = nullptr;
        ...
        {
            int podMilitaryStrength
                = MachLogRaces::instance().findSafestFriendly(*pActor_, &pPodActor, MachLogRaces::PODS);
    ...
    switch (currentEvadeOpType_)
    {
        ...
        case SEEK_AGGRESSIVE:
            {
                ASSERT(
                    pEvadeDestinationActor,
                    "MachLogEvadeOperation::doUpdate : We have an unexpected NULL pointer for the destination actor.");
                ASSERT(
                    pEvadeDestinationActor->objectIsMachine(),
                    "MachLogEvadeOperation::doUpdate : cannot follow a non-machine");
        ...
        case SEEK_TURRET:
            {
                ...
                // !! We should really use the direction vector of the aggressor from the turret, then pop on the far
                // side of it.
                ...
                MexTransform3d destinationPointAsTransform(
                    enemyAsTransform.xBasis(),
                    enemyAsTransform.yBasis(),
                    enemyAsTransform.zBasis(),
                    pointBehindTurret);
        ...
        case SEEK_POD:
            {
                ...
                ASSERT(
                    pPodActor,
                    "MachLogEvadeOperation::doUpdate : We have an unexpected NULL pointer for pPodActor.");

                const MexTransform3d& podAsTransform = pPodActor->asPod().physConstruction().globalTransform();
                MexTransform3d destinationPointAsTransform(
                    podAsTransform.xBasis(),
                    podAsTransform.yBasis(),
                    podAsTransform.zBasis(),
                    pPodActor->position());
        ...
        default:
            {
                ...
                // okay, find the strongest threat (this takes into account both the military strength of the actor, and
                // its distance). We'll use its global transform to help define the space search. This means if we draw
                // a circle around ourselves, our preferred evasion point is in a direction from ourselves aligned with
                // the facing of the aggressing machine. As that machine is most likely pointing toward us, this should
                // take us away from said aggressor.
                ...
                ASSERT(
                    pNearestEnemy,
                    "MachLogEvadeOperation::doUpdate : We have an unexpected NULL pointer for pNearestEnemy.");

                MexTransform3d destinationPointAsTransform = MachLogSpacialManipulation::sourceToDestination3dTransform(
                    pNearestEnemy->position(),
                    pActor_->position(),
