...
std::ostream& operator<<(std::ostream& o, const MachLogNuclearBomb& t)
{

    o << "MachLogNuclearBomb " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogNuclearBomb " << static_cast<const void*>(&t) << " end" << std::endl;
...
// static
MachPhysNuclearBomb*
MachLogNuclearBomb::pNewPhysNuclearBomb(const MexPoint3d& startPosition, MachPhysNuclearBomb** ppPhysNuclearBomb)
{
...
bool MachLogNuclearBomb::hitVictimSecondWave(const MachActor& victim) const
{
    MATHEX_SCALAR checkRadiusSize
        = MachPhysNuclearBomb::shockwaveRadius(SimManager::instance().currentTime() - secondWaveStartTime());
...
void MachLogNuclearBomb::inflictDamageSecondWave(MachActor* pDamagedVictim)
{
    ...
    ASSERT(
        secondWaveFinishTime() > secondWaveStartTime(),
        "This would cause a division by zero. Something must be wrong with code elsewhere allowing this method to "
        "be called.");
...
{
    MATHEX_SCALAR checkRadiusSize
        = MachPhysNuclearBomb::shockwaveRadius(SimManager::instance().currentTime() - secondWaveStartTime()) - 5.0;
