...
MachLog1stPersonMachineHandler::MachLog1stPersonMachineHandler(
    ...
    MachLog1stPersonHandler::NetworkType networkType)
    : MachLog1stPersonHandler(pMachine, pNewPhysDriver(pMachine, networkType), networkType)
{
    ...
    {
        pData_->pMachine_->setSquadron(nullptr);
...
MachLog1stPersonMachineHandler::~MachLog1stPersonMachineHandler()
{
    ...
    machine.pFirstPersonMachineHandler(nullptr);
...
void MachLog1stPersonMachineHandler::doUpdate()
{
    ...
    MachLogConstruction* pEnteringConstruction = nullptr;
    ...
    NETWORK_STREAM(
        "(" << machine.id() << ") MachLog1stPersonMachineHandler::doUpdate wasInside " << wasInside << " onEntrancePad "
            << onEntrancePad << std::endl);
    NETWORK_INDENT(2);
    ...
    if (onEntrancePad)
    {
        ...
        if (haveLockedEntrance)
        {
            ...
            if (not remotelyControlled())
            {
                ...
                isOutside
                    = pEnteringConstruction->onOutsideSideOfThreshold(entranceIndex, MexPoint2d(machine.position()));
    ...
    if (moveMachine and not onEntrancePad and machine.hasLockedEntrance())
    {
        ...
        oldEntrance.unlock();
        machine.entranceLocked(nullptr);

        // Don't need to observe building any more if left completely
        if (outside)
            machine.inOrOnPadBuilding(nullptr);
    }
