...
PhysRelativeTime MachLogRecycleOperation::doUpdate()
{
    ...
    else
    {
        ...
        if (actorPos.sqrEuclidianDistance(nearestPadPosition()) > sqr(machineClearance * 2.2))
        {
            ...
            subOperation(
                pActor_,
                std::make_unique<MachLogMoveToOperation>(pActor_, destinationPadPosition_, true, machineClearance * 2.0));
            interval = 1.0;
        }
        else
        {
            ...
            MachPhys::BuildingMaterialUnits amountToAdd
                = pActor_->machineOriginalCost() * MachLogRaces::instance().stats().secondhandRefundablePercentage();
...
// virtual
bool MachLogRecycleOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int /*clientData*/)
{
...
bool MachLogRecycleOperation::attemptToGuaranteeClearRecyclePoint(
...
bool MachLogRecycleOperation::selectBestRecyclePosition()
{
    ...
    for (int j = 0; j < pSmeltingBuilding_->nPutDownPoints(); ++j)
    {
        ...
        {
            // okay, it's closer than the nearest viable pad found so far. But is this pad itself useable (does it have
            // enough space)?
            ...
            {
                // note that destinationPadPosition_ may have its value reassigned several times as better candidates
                // are found
            ...
            else
            {
                ...
                {
                    // note that destinationPadPosition_ may have its value reassigned several times as better
