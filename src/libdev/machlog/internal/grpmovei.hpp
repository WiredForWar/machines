...
public:
    ...
    using Machines = ctl_vector<MachLogMachine*>;
...
private:
    ...
    void positionControllingClump(const Clump& controllingClump, Machines* pUnlocatedMachines);

    void positionMachinesOnGrid(const Machines& unlocatedMachines);

    bool canPlaceAtDestination(MachLogMachine* pMachine, const MexPoint3d& desiredDestination);
    ...
    CTL_QUAD(
        ...
        clearance,
        MATHEX_SCALAR,
        sqrDistanceToGroupDestination);

    using DestData = ctl_vector<MachineData>;
