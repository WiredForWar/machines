...
private:
    ...
    MATHEX_SCALAR
    lowClearance_; // The clearance used for navigation when space is tight - ie the minimum to avoid collisions
    ...
    PhysConfigSpace2d::DomainFindPathId
        domainFindPathId_; // id of current domain findpath algorithm. Valid if internalState_ == PLANNING_DOMAIN_PATH
    MachLogMachineMotionSequencer::PortalPoints
        domainPath_; // Sequence of points on inter-domain portals forming the gross path navigation.
    ...
    MachLogMachineMotionSequencer::PolygonId
        restingObstacleId_; // Id of polygon added to config space when at rest (including waiting)
    ...
    bool restingObstacleExists_ : 1; // true if an obstacle for the resting position has been added to the config space.
                                     // If so its id is restingObstacleId_, and restingChunkId_ is also valid.
    ...
    bool movedOffPath_ : 1; // True if moved to a point off computed path. This is used to take evasive action when path
                            // is blocked.
