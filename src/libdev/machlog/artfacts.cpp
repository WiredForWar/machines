...
std::ostream& operator<<(std::ostream& o, const MachLogArtefacts& t)
{
    ...
    o << "MachLogArtefacts " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogArtefacts " << static_cast<const void*>(&t) << " end" << std::endl;
...
void MachLogArtefacts::addSubType(
    ...
    const ctl_vector<MexPoint2d>& localBoundary)
{
    CB_MachLogArtefacts_DEPIMPL();
...
W4dEntity* MachLogArtefacts::newPhysArtefact(int subType, const MexPoint3d& location, const MexRadians& angle) const
{
    ...
    W4dDomain* pDomain = MachLogPlanetDomains::pDomainPosition(location, angle, &localTransform);
...
void MachLogArtefacts::parseArtefactsSection(
    ...
    MachLogArtefacts::CreateArtefactInstances doCreate)
{
    CB_MachLogArtefacts_DEPIMPL();
    ...
    while (! done)
    {
        ...
        else if (pParser->tokens()[0] == "ONDESTROY")
        {
            if (doCreate == CREATE_ARTEFACT_INSTANCES)
            {
                ...
                ASSERT_INFO(tagId);
                ASSERT(pDyingArtefact != nullptr, "Dying artefact id not in use");
                ...
                ASSERT_INFO(tagId);
                ASSERT(pDamageArtefact != nullptr, "Damage artefact id not in use");
