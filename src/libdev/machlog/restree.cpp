...
const MachLogResearchTree::ResearchItems& MachLogResearchTree::researchItems() const
{
    CB_MachLogResearchTree_DEPIMPL();
...
MachLogResearchItem& MachLogResearchTree::researchItem(
    ...
    MachPhys::WeaponCombo wc)
{
    ...
    // ); if the hardware level is zero then just match on the first one that you find. this is designed for te sw data
    // which is stored only in the first instance of a RI for obtype/sub type and not on hwLevel.
    bool found = false;
            ...
            if (parser.tokens()[0] == "MACHINE" or parser.tokens()[0] == "CONSTRUCTION")
            {
                ...
                while (doingMachine)
                {
                    ...
                    if (lineSize > 0)
                    {
                        ...
                        {
                            ASSERT(
                                parser.tokens().size() == 3,
                                "Factory_cost line must be 3 tokens (absolute/relative and amount)");
                        ...
                        else if (parser.tokens()[0] == "PARENT")
                        {
                            ...
                            if (lineSize >= 4)
                                parentSubType
                                    = MachLogScenario::objectSubType(parentObType, parser.tokens()[3].c_str());
                            if (lineSize >= 5)
                ...
                MachLogResearchItem* pParentRI = nullptr;
                if (parentHwLevel)
...
bool MachLogResearchTree::activated(
    MachPhys::Race r,
    MachLog::ObjectType objectType,
    int subType,
    size_t hwLevel,
