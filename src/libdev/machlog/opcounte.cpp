...
// virtual
bool MachLogCounterattackOperation::beNotified(
    W4dSubject* pSubject,
...
PhysRelativeTime MachLogCounterattackOperation::doUpdate()
{
    ...
    ASSERT(
        ...
        "MachLogCounterattackOperation::doUpdate : Hey, we shouldn't have a null pointer for the cached op by now!");
    ...
    ASSERT(pTarget_ != nullptr, "pTarget_ shouldn't be NULL if we've decided not to break off the attack.");
    ...
    else if (initiatedAttackOp_)
    {
        ...
        else if (pTarget_->objectIsMachine())
        {
            ...
            MATHEX_SCALAR speedDifferential
                = targetMachine.physMachine().machineData().speed() - pActor_->physMachine().machineData().speed();
