...
std::ostream& operator<<(std::ostream& o, const MachLogIonBeam& t)
{
    ...
    o << "MachLogIonBeam " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachLogIonBeam " << static_cast<const void*>(&t) << " end" << std::endl;
...
// static
MachPhysIonBeam* MachLogIonBeam::pNewPhysIonBeam(const MexPoint3d& startPosition, MachPhysIonBeam** ppPhysIonBeam)
{
...
bool MachLogIonBeam::hitVictimSecondWave(const MachActor& victim) const
{
    MATHEX_SCALAR checkRadiusSize
        = MachPhysBeam::outerRadius(SimManager::instance().currentTime() - secondWaveStartTime());
...
bool MachLogIonBeam::hitVictimThirdWave(const MachActor& victim) const
{
    MATHEX_SCALAR basicWaveRadius
        = MachPhysBeam::waveRadius(SimManager::instance().currentTime() - thirdWaveStartTime());
