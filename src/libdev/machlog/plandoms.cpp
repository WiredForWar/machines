...
W4dDomain* MachLogPlanetDomains::pDomainPosition(
    const MexPoint3d& globalLocation,
    const MexRadians& zAngle,
    MexTransform3d* pLocalTransform)
{
    PRE(pLocalTransform != nullptr);
...
{
    NETWORK_STREAM(
        "MachLogPlanetDomains::intersecting boundary " << boundary << " domains.size " << pDomains->size()
                                                       << std::endl);
    ...
    if (valid)
    {
        ...
        else
        {
            ...
            NETWORK_STREAM(
                "xDomains = " << xDomains << " xDiff " << xDiff << " tileXSpacing " << tileXSpacing << " "
                              << (uint)(xDiff / tileXSpacing) << std::endl);
        }
        ...
        else
        {
            ...
            NETWORK_STREAM(
                "yDomains = " << yDomains << " yDiff " << yDiff << " tileYSpacing " << tileYSpacing << " "
                              << (uint)(yDiff / tileYSpacing) << std::endl);
        }

        MATHEX_SCALAR dy = yStart;

        NETWORK_STREAM(
            " valid " << valid << " xDomains " << xDomains << " yDomains " << yDomains << " dy " << dy << std::endl);
    ...
    const MexPoint3d& endPoint)
{
    ...
    W4dDomain* pEntityDomain = nullptr;
...
void MachLogPlanetDomains::addIntersectRelations(W4dEntity* pEntity)
{
    ...
    const MexAlignedBox3d& boundingBox
...
// static
MexAlignedBox2d
MachLogPlanetDomains::convertBoundary(const MexAlignedBox3d& box3d, const MexTransform3d& globalTransform)
{
