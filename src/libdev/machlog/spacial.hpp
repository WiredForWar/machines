...
public:
    enum UseWhichEnd
    {
        SOURCE_END_IS_CENTRE,
        DESTINATION_END_IS_CENTRE
    };
    enum UseWhichPattern
    {
        USE_CLOCK_PATTERN,
        USE_FLANKING_PATTERN
    };

    static bool inExtendedMexAlignedBox2d(const MexAlignedBox2d& box, const MexPoint3d& pos, MATHEX_SCALAR radius);

    static bool getNearestFreeSpacePoint(
    ...
    static bool
    pointInsideConfigSpaceIsFree(const MexPoint2d& testPoint, MATHEX_SCALAR clearance, const PhysConfigSpace2d& cs);
    ...
    static MexTransform3d sourceToDestination3dTransform(
    ...
    static void
    genericCheckForIntersections(const MexPoint3d& epicentre, MATHEX_SCALAR range, ctl_pvector<MachActor>* pActors);
    ...
    static MexPoint3d
    heightAdjustedLocation(MachLog::ObjectType type, int subType, int hwLevel, const MexTransform3d& transform);
    static MexPoint3d
    heightAdjustedLocation(const MachPhysConstructionData& constructionData, const MexTransform3d& transform);
...
private:
    ...
    static bool getNextInCircularPattern(
        const MexTransform3d& orTrans,
    ...
    static bool getNextInFlankingPattern(
        const MexTransform3d& orTrans,
        const MATHEX_SCALAR radius,
        MATHEX_SCALAR clearance,
        MexPoint2d* pResult);
