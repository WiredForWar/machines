    ...
    else if (pActor_->insideBuilding() and &pActor_->insideWhichBuilding() == pConstruction_)
    {
        ...
        if (not onInternalEntrancePolygon())
        {
            ...
            if (pActor_->hasLockedEntrance())
            {
                ...
                pActor_->entranceLocked(nullptr);
...
PhysRelativeTime MachLogEnterBuildingOperation::doUpdate()
{
    ...
    enum Action
    {
        NONE,
        MOVE_TO_STATION,
        MOVE_TO_INTERNAL_POINT,
        MOVE_TO_EXTERNAL_POINT
    };
    Action action = NONE;
        ...
        case MOVE_TO_STATION:
            {
                ...
                else
                {
                    ...
                    if (pActor_->motionSeq().findAvoidancePoint(pActor_->position(), &point2mForwardInDirectionFaced))
                    {
                        subOperation(
                            pActor_,
                            _NEW(MachLogMoveToOperation(pActor_, point2mForwardInDirectionFaced, false)));
                ...
                if (sqrRangeToEntrance <= close * close)
                {
                    ...
                    if (pActor_->hasLockedEntrance() or pConstruction_->entrance(0).lock())
                    {
                        ...
                        else
                        {
                            ...
                            pConstruction_->entrance(0).unlock();
                            status_ = CLEAR;
                            if (MachLogRaces::instance().controller(pActor_->race()).type()
                                == MachLogController::AI_CONTROLLER)
...
// virtual
bool MachLogEnterBuildingOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int)
{
...
void perWrite(PerOstream& ostr, const MachLogEnterBuildingOperation& op)
{
    ...
    bool needToGetAStation = (op.pStation_ != nullptr);
...
MachLogLeaveBuildingOperation::~MachLogLeaveBuildingOperation()
{
    ...
    else if (not pActor_->insideBuilding())
    {
        ...
        if (not onExternalEntrancePolygon())
        {
            ...
            if (pActor_->hasLockedEntrance())
            {
                ...
                pActor_->inOrOnPadBuilding(nullptr);
            }
        }
    ...
    pActor_->motionSeq().useSpaceDomain(nullptr);
    pActor_->motionSeq().keepEntranceLocked(false);
    pActor_->motionSeq().stop();
    pActor_->motionSeq().update(0.001);
        ...
        case GOING_CLEAR:
            {
                if (not moving)
                {
                    ...
                    const MexPolygon2d& exteriorPolygon = MachLogPlanet::instance().configSpace().polygon(
                        pConstruction_->entrance(0).exteriorPolygonId());
        ...
        case MOVE_TO_EXTERNAL_POINT:
            {
                ...
                else
                {
                    ...
                    pActor_->insideWhichBuilding(nullptr);
        ...
        case MOVE_CLEAR:
            {
                ...
                if (MachLogSpacialManipulation::getNearestFreeSpacePoint(
                        MexTransform3d(MexPoint3d(destination)),
                        10.0,
                        mSeq.highClearance(),
                        &destination))
                {
...
// virtual
bool MachLogLeaveBuildingOperation::beNotified(W4dSubject* pSubject, W4dSubject::NotificationEvent event, int)
{
    // HAL_STREAM("(" << pActor_->id() << ") MLEnterBuilding::beNotified\n" );
    switch (event)
    {
