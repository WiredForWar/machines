                ...
                if (event.key() == DevKey::ENTER)
                {
                    ...
                    else
                    {
                        ...
                        else if (checkEnteredText(pPromptText2, pPromptTextKey2, text())) // Instant heal
                        {
                            ...
                            for (MachInGameScreen::Actors::const_iterator iter = actors.begin(); iter != actors.end();
                                 ++iter)
                            {
                        ...
                        {
                            MachLogRaces::Pods& pods
                                = MachLogRaces::instance().pods(MachLogRaces::instance().pcController().race());
                        ...
                        {
                            MachLogRaces::MissileEmplacements& silos = MachLogRaces::instance().missileEmplacements(
                                MachLogRaces::instance().pcController().race());
                            for (MachLogRaces::MissileEmplacements::iterator iter = silos.begin(); iter != silos.end();
                                 ++iter)
                            {
                        ...
                        else if (checkEnteredText(pPromptText8, pPromptTextKey8, text())) // a test
                        {
                            ...
                            if (testFile.existsAsFile())
                            {
                                ...
                                if (RenSurfaceManager::extractBMPText(testFile, &test))
                                    W4dManager::instance().sceneManager()->pDevice()->horizontalBannerText(
                                        test.c_str(),
                                        200,
                                        2);
                            }
                        ...
                        {
                            MachLogResearchItem& resItem = MachLogRaces::instance().researchTree().researchItem(
                                MachLog::AGGRESSOR,
                                MachPhys::BALLISTA,
                                4,
                        ...
                        else if (checkEnteredText(pPromptText5, pPromptTextKey5, text())) // Kill *all* AI Machines/ME
                        {
                            ...
                            for (MachPhys::Race i = MachPhys::RED; i != MachPhys::N_RACES; ++((int&)i))
                            {
                                if (races.raceInGame(i)
                                    and races.controller(i).type() == MachLogController::AI_CONTROLLER)
                                {
                                    for (MachLogRaces::Objects::iterator j = races.raceObjects(i).begin();
                                         j != races.raceObjects(i).end();
                                         ++j)
                                    {
                                        MachActor& actor = **j;
                                        if (actor.objectIsMachine()
                                            or actor.objectType() == MachLog::MISSILE_EMPLACEMENT)
                                        {
                        ...
                        else if (checkEnteredText(
                                     pPromptText6,
                                     pPromptTextKey6,
                                     text())) // Set off a nuke underneath some actors
                        {
                            MachLogRaces& races = MachLogRaces::instance();
                            for (MachPhys::Race i = MachPhys::RED; i != MachPhys::N_RACES; ++((int&)i))
                            {
