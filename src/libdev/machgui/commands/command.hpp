...
public:
    ...
    std::string getBindDisplayString() const;
    ...
    virtual MachGui::Cursor2dType
    cursorOnTerrain(const MexPoint3d& location, bool ctrlPressed, bool shiftPressed, bool altPressed)
        = 0;

    // Respond to cursor on pActor in the world view window.
    // ctrl/shift/altPressed indicate which modifier keys are pressed.
    // Returns 2d cursor to be displayed.
    virtual MachGui::Cursor2dType cursorOnActor(MachActor* pActor, bool ctrlPressed, bool shiftPressed, bool altPressed)
        = 0;
...
protected:
    ...
    bool findClosestPointValidOnTerrain(
    ...
    bool findClosestPointValidOnTerrain(
        const MexPoint2d& endPoint, // Point that we are trying to get close to
    ...
    // PRE( pPath );
    // PRE( pPath->size() == 0 );
    bool convertPointsToValidPoints(
        ObstacleModifier modifier,
        MachLogMachine* pMachine,
