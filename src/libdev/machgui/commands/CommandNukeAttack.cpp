...
std::ostream& operator<<(std::ostream& o, const MachGuiNukeAttackCommand& t)
{
    ...
    o << "MachGuiNukeAttackCommand " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "MachGuiNukeAttackCommand " << static_cast<const void*>(&t) << " end" << std::endl;

    return o;
}

// virtual
void MachGuiNukeAttackCommand::pickOnTerrain(
...
// virtual
void MachGuiNukeAttackCommand::pickOnActor(MachActor* pActor, bool ctrlPressed, bool shiftPressed, bool altPressed)
{
...
std::string MachGuiNukeAttackCommand::getCursorPromptText() const
{
    ...
    const GuiString launchCost = std::format("{}{}", GuiBmpFont::bmuPointsIndex(), cost);

    return promptText + " " + Gui::formatResourceString(IDS_LAUNCH_COST_BMU, launchCost);
}
