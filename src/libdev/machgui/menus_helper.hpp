...
namespace helper {
    namespace menus {
        ...
        // Get an x value starting from the screen's left. x:[0, WIDTH] - Optionally divide if getting the top-left of a
        // centered image
        int x_from_screen_left(int distance, int divisor = 1);
        // Get a y value starting from the screen's bottom. y:[0, HEIGHT] - Optionally divide if getting the top-left of
        // a centered image
