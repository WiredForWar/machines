...
template <class GRA_GRAPH, class VERTEX_MAP> GraAStarAlg__::~GraAStarAlg()
{
    TEST_INVARIANT;
...
template <class GRA_GRAPH, class VERTEX_MAP> void GraAStarAlg__::CLASS_INVARIANT
{
...
template <class GRA_GRAPH, class VERTEX_MAP> std::ostream& operator<<(std::ostream& o, const GraAStarAlg__& t)
{
    ...
    {
        o << (t.state_ == GraAStarAlg__::RUNNING ? "RUNNING" : "FINISHED") << " from " << t.startVertexId_ << " to "
          << t.endVertexId_ << std::endl;
        if (t.havePath_)
...
template <class GRA_GRAPH, class VERTEX_MAP>
void GraAStarAlg__::start(const Graph& graph, const VertexId& startVertex, const VertexId& endVertex)
{
    ...
    pAStarVertices_->add(typename AStarVertices::KeyValueAssoc(
        startVertex,
        AStarVertex(Weight(0), estimatedCost(startItem, endItem), ArcId(0))));
...
template <class GRA_GRAPH, class VERTEX_MAP> typename GraAStarAlg__::State GraAStarAlg__::update()
{
    ...
    for (typename Arcs::const_iterator ia = arcs.begin(); ia != arcs.end(); ++ia)
    {
        ...
        if (vertexAvailable(pGraph_->vertex(otherVertexId)))
        {
            ...
            if (newVertex)
            {
                ...
                pAStarVertices_->add(typename AStarVertices::KeyValueAssoc(
                    otherVertexId,
                    AStarVertex(costToOtherVertex, estimatedCost(otherItem, endItem), arcId)));
            }
            ...
            if (newVertex or costToOtherVertex < otherAStarVertex.costFromStart())
            {
                ...
                else if (newVertex)
                {
                    ...
                    Weight estimatedTotalCost = costToOtherVertex + otherAStarVertex.estimatedCostToEnd();
...
template <class GRA_GRAPH, class VERTEX_MAP> typename GraAStarAlg__::State GraAStarAlg__::state() const
{
    return state_;
}
//////////////////////////////////////////////////////////////////////////////////////////

template <class GRA_GRAPH, class VERTEX_MAP> bool GraAStarAlg__::isFinished() const
{
    return state_ == FINISHED;
}
//////////////////////////////////////////////////////////////////////////////////////////

template <class GRA_GRAPH, class VERTEX_MAP> bool GraAStarAlg__::isRunning() const
{
    return state_ == RUNNING;
}
//////////////////////////////////////////////////////////////////////////////////////////

template <class GRA_GRAPH, class VERTEX_MAP> bool GraAStarAlg__::isDefined() const
{
    return state_ != UNDEFINED;
}
//////////////////////////////////////////////////////////////////////////////////////////

template <class GRA_GRAPH, class VERTEX_MAP> typename GraAStarAlg__::Vertices GraAStarAlg__::output() const
{
    PRE(isFinished());
    ...
    Vertices vertices;
    if (havePath_)
        outputVertex(endVertexId_, &vertices);
...
void GraAStarAlg__::outputVertex(const VertexId& id, Vertices* pVertices) const
{
    ...
    if (id != startVertexId_)
    {
        ...
        if (previousVertexId == id)
            previousVertexId = pGraph_->toVertex(previousArcId);
