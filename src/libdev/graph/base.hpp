...
template <class VERTEX_ID, class VERTEX_ITEM, class ARC_ID, class ARC_ITEM, class CONST_ARC_IDS, class WEIGHT>
class GraGraphBase
// cannonical form revoked
{
public:
    using GraphBase = GraGraphBase;
    using VertexId = VERTEX_ID;
    using VertexItem = VERTEX_ITEM;
    using ArcId = ARC_ID;
    using ArcItem = ARC_ITEM;
    using ArcIds = CONST_ARC_IDS;
    using Weight = WEIGHT;
    using Version = FtlSerialId;

    using ArcCount = unsigned long;
    using VertexCount = unsigned long;
    ...
    void addArc(const VertexId& from, const VertexId& to, const ArcId& arcId, const ArcItem& arcItem);
    // PRE( from != to );
    // PRE( containsVertex( from ) );
    // PRE( containsVertex( to ) );
...
protected:
    ...
    // PRE( containsVertex( to ) );
    // PRE( not containsArc( arcId ) );
    virtual void doAddArc(const VertexId& from, const VertexId& to, const ArcId& arcId, const ArcItem& arcItem) = 0;
    // POST( arc( arcId ) == arcItem );
...
#include "graph/base_templates.hpp"
// #endif
