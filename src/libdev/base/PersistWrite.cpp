...
#include "base/internal/PersistLog.hpp"
...
void PersistenceImplementationWrite::writePointerPost(
    PerOstream& ostr,
    const void* pOb,
    const char* className,
    const void* /* pMostDerivedOb */)
...
void PersistenceImplementationWrite::writeNamedObject(PerOstream& ostr, const char* className, const void* ptr)
{
    ...
    else
    {
        ...
        ASSERT_FAIL("Could not find write function for object. You must call 'PER_REGISTER_DERIVED_CLASS' for all "
                    "derived classes.");
    }
}
