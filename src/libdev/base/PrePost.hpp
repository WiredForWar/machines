...
#include "private/DiagInternal.hpp"
...
public:
    ...
    using AssertionAction_fn_AssertionInfo = AssertionAction (*)(const AssertionInfo&);
...
#define ASSERT_FILE_EXISTS(f)                                                                                          \
    BaseAssertion::AssertionLock::instance().isLocked() ? (void)0                                                      \
                                                        : BaseAssertion::assertFileExists(f, __FILE__, _STR(__LINE__))

////////////////////////////////////////////////////////////

#define ASSERT(exp, xmsg) ASSERT_EXPRESSION(exp, BaseAssertion::assertFail(__FILE__ ":" _STR(__LINE__) " " #exp, xmsg))
...
#define ASSERT_BAD_CASE_INFO(exp)                                                                                      \
    {                                                                                                                  \
        ASSERT_INFO(exp);                                                                                              \
        ASSERT(false, logic_error("bad case"))                                                                         \
    }
...
#define INVARIANT(exp) ASSERT_EXPRESSION(exp, BaseAssertion::invariantFail(#exp, __FILE__, _STR(__LINE__), file, line))
