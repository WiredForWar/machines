...
public:
    ...
    using PerReadIdentifier = std::pair<PerIdentifier, uint>;
...
private:
    ...
    using ReadIdToPointerMap = std::unordered_map<PerReadIdentifier, PerMapPtrType, PairHash>;
    using IdToPointerMap = std::unordered_multimap<PerIdentifier, PerMapPtrType>;
    using ReadFnMap = std::unordered_map<std::string, PerReadFnPtr>;

    using UnfixedPointer = std::pair<void**, PerReadIdentifier>;

    using UnfixedPointers = ctl_vector<UnfixedPointer>;
