...
void PersistenceImplementationRead::readNamedObject(PerIstream& istr)
{
    ...
    else
    {
        ...
        ASSERT_FAIL("Could not find read function for object. You must call 'PER_REGISTER_DERIVED_CLASS' for all "
                    "derived classes.");
    }
}
...
void PersistenceImplementationRead::finishRead()
{
    ...
    if (inputCount_ == 0)
    {
        ...
        PER_READ_INDENT_STREAM(
            "------- Finish read -----------------------------------------------------" << std::endl);
    }
}

Persistence::PointerReadResult
PersistenceImplementationRead::readPointerPre(PerIstream& istr, void** /* ptr */, const char* className)
{
...
void persistenceImplementationReadWriteTimingData(std::ostream& ostr)
{
    ...
    {
        ostr << std::setw(15) << std::setprecision(10) << (*i).time() << " " << std::setw(8) << (*i).count() << " "
             << (*i).name() << std::endl;
        totalTime += (*i).time();
    }
