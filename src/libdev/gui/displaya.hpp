...
//
// GuiDisplayable( parent, Gui::Boundary( topLeftPixel.x(), topLeftPixel.y(), topLeftPixel.x() + width, topLeftPixel.y()
// + height ) );
//
...
protected:
    ...
    void hollowRectangle(const Gui::Box& rel, const Gui::Colour& colour, unsigned thickness);
    // PRE( boundary().contains( translate( rel ) ) );

    ///////////////////////////////

    void line(const Gui::Coord& rel1, const Gui::Coord& rel2, const Gui::Colour&, unsigned thickness);
    // PRE( boundary().contains( translate( rel1 ) ) );
    // PRE( boundary().contains( translate( rel2 ) ) );

    void horizontalLine(const Gui::Coord& rel1, unsigned length, const Gui::Colour&, unsigned thickness);
    // PRE( boundary().contains( translate( rel1 ) ) );
    // PRE( boundary().contains( translate( rel1.x() + length, rel1.y() ) ) );

    void verticalLine(const Gui::Coord& rel1, unsigned height, const Gui::Colour&, unsigned thickness);
    // PRE( boundary().contains( translate( rel1 ) ) );
