...
bool GuiButton::isDisabled() const
{
    return isDisabled_;
}
bool GuiButton::isDepressed() const
{
    return isDepressed_;
}
...
void GuiButton::doHandleMouseClickEvent(const GuiMouseEvent& e)
{
    if (isVisible())
    {
        if (e.leftButton() == Gui::RELEASED)
        {
            ...
            // Keep copy because `this` can be deleted in doBeReleased() overrides
            Callback callback = clickedCallback_;
...
const GuiBorderMetrics& GuiButtonWithBorder::borderMetrics() const
{
    return borderMetrics_;
}

const GuiBorderDimensions& GuiButtonWithBorder::borderDimensions() const
{
    return borderDimensions_;
}

const Gui::Coord& GuiButtonWithBorder::depressedInteriorOffset() const
{
...
void GuiButtonWithBorder::doDisplayDepressedEnabled()
{
    ...
    unsigned offsetX = m.frameThickness() + m.highlightThickness() + depressedInteriorOffset_.x();
    unsigned offsetY = m.frameThickness() + m.highlightThickness() + depressedInteriorOffset_.y();

    doDisplayInteriorEnabled(Gui::Coord(absoluteCoord().x() + offsetX, absoluteCoord().y() + offsetY));
}
...
void GuiButtonWithBorder::doDisplayReleasedEnabled()
{
    ...
    doDisplayInteriorEnabled(Gui::Coord(absoluteCoord().x() + offset, absoluteCoord().y() + offset));
}
...
void GuiButtonWithFilledBorder::doDisplayBorderDepressedEnabled()
{
    ...
    const GuiBorderMetrics& m = borderMetrics();
    GuiBorderMetrics depressedMetrics(
        m.frameThickness(),
...
GuiFilledBorder& GuiButtonWithFilledBorder::border()
{
    return border_;
}
const GuiFilledBorder& GuiButtonWithFilledBorder::border() const
{
    return border_;
}
...
// static
Gui::Box GuiBitmapButtonWithFilledBorder::exteriorRelativeBoundary(
    const GuiBitmap& bitmap,
    const GuiBorderMetrics& m,
    const Gui::Coord& rel)
{
    return Gui::Box(rel, m.totalHorizontalThickness() + bitmap.width(), m.totalVerticalThickness() + bitmap.height());
}
