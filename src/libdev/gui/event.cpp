...
GuiEventBase::ModifierKeys GuiEventBase::modifierKeys() const
{
...
GuiMouseEvent::GuiMouseEvent(const Gui::Coord& at, Gui::ButtonState left, Gui::ButtonState right, ModifierKeys keys)
    : GuiEventBase(keys)
    , coord_(at)
    , leftButton_(left)
    , rightButton_(right)
...
Gui::ButtonState GuiMouseEvent::leftButton() const
{
    return leftButton_;
}

Gui::ButtonState GuiMouseEvent::rightButton() const
{
    return rightButton_;
}

Gui::ScrollState GuiMouseEvent::scrollDirection() const
{
    return scroll_;
}

const Gui::Coord& GuiMouseEvent::coord() const
{
    return coord_;
}
...
GuiKeyEvent::GuiKeyEvent(const GuiKey& k, Gui::ButtonState s, ModifierKeys keys, const DevButtonEvent& buttonEvent)
    ...
    , keyWithMods_(
          ...
          KeyModifierFlags::fromAltCtrlShiftState(
...
const GuiKey& GuiKeyEvent::key() const
{
    return key_;
}

Gui::ButtonState GuiKeyEvent::state() const
{
    return state_;
}
