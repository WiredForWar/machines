...
{
    o << "PedTileEditor " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "PedTileEditor " << static_cast<const void*>(&t) << " end" << std::endl;
...
void PedTileEditor::displayTileCoords()
{
    ...
    else if (selectedTiles_.size() == 1)
    {
        ...
        pSceneManager_->out() << "Selected Tile (" << pOnlySelTile->xPos_ << "," << pOnlySelTile->yPos_ << ")"
                              << std::endl;
...
void PedTileEditor::processAttatchCeilingArtefact(bool attatch)
{
    ...
    MachPhysPlanetSurface* pSurface = pPlanet_->surface();
    MachPhysTerrainTile* terrainTile = nullptr;
