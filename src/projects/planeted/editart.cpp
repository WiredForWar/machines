    ...
    if (nArtefactGroups) // determine if there is any artefact data
    {
        for (int col = 0; col < nArtefactGroups; col++)
        {
            ...
            if (nArtefacts)
            {
                ...
                for (ArtefactGroup::iterator row = artefacts_[col].begin(); row != artefacts_[col].end(); row++)
                {
...
void PedArtefactEditor::rotateAfterMove()
{
    if (artefactStore_.nGenerics())
    {
        ...
        PRE(pSelectedArtefact_ != nullptr);
...
void PedArtefactEditor::createEntity(
    W4dEntity& Entity,
    ...
    MexDegrees orientation)
{
    ...
    W4dEntity* pCurrentArtefact = nullptr;
...
std::ostream& operator<<(std::ostream& o, const PedArtefactEditor& t)
{
    ...
    o << "PedArtefactEditor " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "PedArtefactEditor " << static_cast<const void*>(&t) << " end" << std::endl;
...
void PedArtefactEditor::createCeilingArtefact(const MachPhysTerrainTile& terrainTile)
{
    ...
    if (key != "")
    {
        createEntity(
            *(terrainTile.pParent()),
            terrainTile.localTransform(),
            key,
            terrainTile.localTransform().rotationAsEulerAngles().roll());
    }
}
...
void PedArtefactEditor::deleteCeilingArtefact(const MachPhysTerrainTile& terrainTile)
{
    ...
    if (key != "")
    {
        ...
        for (j = artefacts_[artefactGroup].begin(); j != artefacts_[artefactGroup].end(); ++j)
        {
