...
std::ostream& operator<<(std::ostream& o, const PedObstacleEditor& t)
{
    ...
    o << "PedObstacleEditor " << static_cast<const void*>(&t) << " start" << std::endl;
    o << "PedObstacleEditor " << static_cast<const void*>(&t) << " end" << std::endl;
...
void PedObstacleEditor::readCspFile(const SysPathName& cspFileName)
{
    ...
    while (not parser.finished())
    {
        ...
        if (parser.tokens()[0] == "OBSTACLE")
        {
            ...
            {
                ASSERT(
                    parser.tokens().size() == 2,
                    ("Points not found ad line: " + std::to_string(parser.lineNumber())).c_str());
