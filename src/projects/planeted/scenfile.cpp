...
PedScenarioFile::Constructions PedScenarioFile::constructions() const
{
    ...
    {
        for (Constructions::const_iterator i = (*raceIter).constructions.begin(); i != (*raceIter).constructions.end();
             ++i)
        {
...
PedScenarioFile::Machines PedScenarioFile::machines() const
{
    ...
    for (RacesInfo::const_iterator raceIter = racesInfo_.begin(); raceIter != racesInfo_.end(); ++raceIter)
    {
        for (Machines::const_iterator i = (*raceIter).machines.begin(); i != (*raceIter).machines.end(); ++i)
        {
...
PedScenarioFile::Construction PedScenarioFile::parseConstruction(const UtlLineTokeniser::Tokens& tokens)
{
    ...
    WAYNE_STREAM(
        "construction: " << construction.type << std::endl
...
void PedScenarioFile::writeCamera(std::ofstream& out, const Camera& camera) const
{
    ...
    switch (camera.type)
    {
        case (MachLogCamera::ZENITH):
            out << cameraZenithToken_;
            break;
        case (MachLogCamera::GROUND):
            out << cameraGroundToken_;
            break;
    }
    out << std::endl;
}
...
void PedScenarioFile::writeConstruction(std::ofstream& out, const Construction& construction) const
{
    ...
    for (ctl_vector<string>::const_iterator buildIter = construction.build.begin();
         buildIter != construction.build.end();
         ++buildIter)
    {
        out << " " << *buildIter;
    }
