...
int main()
{
    ...
    for (int i = 1; i < 11; ++i)
    {
        ...
        for (int j = 1; j < 11; ++j)
        {
            std::cout << "AGG " << i << " " << j << std::endl
                      << MachPhysData::instance().aggressorData(i, j) << std::endl;
            std::cout << "ADM " << i << " " << j << std::endl
                      << MachPhysData::instance().administratorData(i, j) << std::endl;
            std::cout << "TRAN " << i << " " << j << std::endl
                      << MachPhysData::instance().transporterData(i, j) << std::endl;
            std::cout << "LOC " << i << " " << j << std::endl
                      << MachPhysData::instance().locatorData(i, j) << std::endl;
            std::cout << "TECH " << i << " " << j << std::endl
                      << MachPhysData::instance().technicianData(i, j) << std::endl;
            std::cout << "CON " << i << " " << j << std::endl
                      << MachPhysData::instance().constructorData(i, j) << std::endl;
        }
    }
    return 0;
}
