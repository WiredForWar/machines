...
template <class T> bool ControlTargetVector<T>::processEvent(const DevButtonEvent& event)
{
...
template <class T> T* ControlTargetVector<T>::current() const
{
    if (this->size() == 0)
        return NULL;
    else
...
template <class T> bool ControlTargetVector<T>::currentEnabled() const
{
