...
PhysTest::PhysTest()
{
    ...
    // Run the tests
    std::cout << "PhysTimedSpinPlan test " << (testPhysTimedSpinPlan(outputStream) ? "passed" : "failed") << std::endl;

    std::cout << "PhysAlignedBSPTree2d test " << (testAlignedBSPTree2d(outputStream) ? "passed" : "failed")
              << std::endl;

    std::cout << "PhysLinearTravelPlan test " << (testPhysLinearTravelPlan(outputStream) ? "passed" : "failed")
              << std::endl;

    std::cout << "PhysAccelerationTumblePlan test "
              << (testPhysAccelerationTumblePlan(outputStream) ? "passed" : "failed") << std::endl;

    {
        PhysTestConfigSpace2d spaceTest(outputStream); // fails
    }

    std::cout << "Linear motion interpolation test " << (testPhysLinearMotionPlan(outputStream) ? "passed" : "failed")
              << std::endl;

    // fails
    std::cout << "PhysMotionChunk test " << (testPhysMotionChunk(outputStream) ? "passed" : "failed") << std::endl;

    std::cout << "PhysRampAcceleration distance / time test "
              << (testPhysRampAccelerationByDistanceTime(outputStream) ? "passed" : "failed") << std::endl;

    std::cout << "PhysRampAcceleration speed test "
              << (testPhysRampAccelerationBySpeed(outputStream) ? "passed" : "failed") << std::endl;
...
bool PhysTest::testPhysLinearTravelPlan(ostream& out) const
{
    ...
    if (okay)
    {
        ...
        okay = abs(segmentTimes[0] - 1.0) < 0.00001 and abs(segmentTimes[1] - 2.5) < 0.00001;
    }
...
bool PhysTest::testPhysAccelerationTumblePlan(ostream& out) const
{
    ...
    PhysAccelerateTumblePlan plan(transformsPtr, timesPtr, acceleration);
...
bool PhysTest::testAlignedBSPTree2d(ostream& out) const
{
    ...
    if (okay and tree.overlapping(b1, &ps) != 5)
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l3) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l6) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l7) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l8) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l9) == ps.end())
        okay = false;
    ps.erase(ps.begin(), ps.end());

    if (okay and tree.overlapping(b2, &ps) != 0)
        okay = false;
    ...
    if (okay and tree.overlapping(b3, &ps) != 4)
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l11) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l13) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l14) == ps.end())
        okay = false;
    if (okay and std::find(ps.begin(), ps.end(), &l15) == ps.end())
        okay = false;
...
bool PhysTest::testPhysMotionChunk(ostream&) const
{
    ...
    if (okay)
    {
        ...
        PhysMotionChunk aChunk(
            MexPoint2d(714.719, 849.588),
            MexPoint2d(904.082, 822.201),
            3.0,
            aProfile,
            168.397,
            3.83267,
        ...
        PhysMotionChunk bChunk(
            MexPoint2d(904.969, 829.639),
            MexPoint2d(905.179, 823.643),
            3.0,
            bProfile,
            173.517,
            2.04342,
        ...
        okay = aChunk.intersects(bChunk, &collideTime) and bChunk.intersects(aChunk, &collideTime);
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and collideTime == 50.0
            and bChunk.intersects(aChunk, &collideTime) and collideTime == 50.0;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 51.0) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 51.0) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 52.0) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 52.0) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 50.0) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 50.0) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 53.5) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 53.5) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            MexPoint2d(312.5, 180),
            MexPoint2d(312.5, 227.5),
            0.5,
            bProfile,
            52.5,
            1.0,
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 55.5) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 55.5) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            MexPoint2d(312.5, 150),
            MexPoint2d(312.5, 197.5),
            0.5,
            bProfile,
            48.5,
            1.0,
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 54.0) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 54.0) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = aChunk.intersects(bChunk, &collideTime) and fabs(collideTime - 57.0) < 0.0001
            and bChunk.intersects(aChunk, &collideTime) and fabs(collideTime - 57.0) < 0.0001;
    }
    ...
    if (okay)
    {
        ...
        PhysMotionChunk bChunk(
            ...
            MexDouble::minusInfinity(),
            MexDouble::plusInfinity());
        okay = not aChunk.intersects(bChunk, &collideTime) and not bChunk.intersects(aChunk, &collideTime);
    }
...
bool PhysTest::testPhysRampAccelerationBySpeed(ostream&) const
{
    MATHEX_SCALAR testValues[][6] = {
        ...
        1.565,   0,       8,       8, 10,      10,      0,       0,       1.309, 0,       10, 10,
        180.542, 20.0836, 99.1364, 0, 10.4717, 15.8405, 309.448, 42.4347, 0,     50.4486, 98, 98,
    };
