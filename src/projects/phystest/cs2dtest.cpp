    ...
    : nPolygons_(0)
{
    ...
    if (okay)
        okay = addPermanentPolygons(&space);

    // Add the temporary obstacles, checking works ok
    if (okay)
        okay = addTemporaryPolygons(&space);

    // Add the portals, checking works ok
    if (okay)
        okay = addPortals(&space);

    out << space;

    // Check containment methods
    if (okay)
        okay = testContainsPoint(&space);
    if (okay)
        okay = testContainsLine(&space);
    if (okay)
        okay = testContainsPolygon(&space);

    // Check path finding
    if (okay)
        okay = testFindPaths(&space, out);
    if (okay)
        okay = testDomainFindPaths(&space, out);

    if (okay)
        okay = testIntersects(out);
...
bool PhysTestConfigSpace2d::addDomains(PhysConfigSpace2d* pSpace)
{
    ...
    aPortalIds[0] = pSpace->addPortal(aDomainIds[0], aDomainIds[6], MexPoint2d(444, 444), MexPoint2d(555, 555));
    ...
    // Check exist
    if (okay)
        okay = not pSpace->domainExists(aDomainIds[0]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[1]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[2]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[3]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[4]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[5]);
    if (okay)
        okay = not pSpace->domainExists(aDomainIds[6]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[7]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[8]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[9]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[10]);
    if (okay)
        okay = pSpace->domainExists(aDomainIds[11]);
    if (okay)
        okay = not pSpace->portalExists(aPortalIds[0]);
...
bool PhysTestConfigSpace2d::addPortals(PhysConfigSpace2d* pSpace)
{
    ...
    // Add each portal
    aPortalIds[1] = pSpace->addPortal(aDomainIds[9], aDomainIds[10], MexPoint2d(700, 100), MexPoint2d(700, 700));
    aPortalIds[2] = pSpace->addPortal(aDomainIds[10], aDomainIds[8], MexPoint2d(1400, 700), MexPoint2d(1400, 100));
    aPortalIds[3] = pSpace->addPortal(aDomainIds[10], aDomainIds[11], MexPoint2d(800, 800), MexPoint2d(1300, 900));
    aPortalIds[4] = pSpace->addPortal(aDomainIds[8], aDomainIds[2], MexPoint2d(1500, 800), MexPoint2d(2000, 800));
    aPortalIds[5] = pSpace->addPortal(aDomainIds[1], aDomainIds[7], MexPoint2d(2200, 800), MexPoint2d(2700, 800));
    aPortalIds[6] = pSpace->addPortal(aDomainIds[2], aDomainIds[11], MexPoint2d(1300, 900), MexPoint2d(1300, 1600));
    aPortalIds[7] = pSpace->addPortal(aDomainIds[4], aDomainIds[11], MexPoint2d(700, 900), MexPoint2d(700, 1600));
    aPortalIds[8] = pSpace->addPortal(aDomainIds[2], aDomainIds[7], MexPoint2d(2000, 800), MexPoint2d(1500, 1600));
    aPortalIds[9] = pSpace->addPortal(aDomainIds[4], aDomainIds[5], MexPoint2d(200, 1700), MexPoint2d(600, 1700));
    aPortalIds[10] = pSpace->addPortal(aDomainIds[5], aDomainIds[3], MexPoint2d(700, 1800), MexPoint2d(700, 2700));

    // Check we have some
    if (okay)
        okay = pSpace->portalExists(aPortalIds[1]);
    if (okay)
        okay = pSpace->portalExists(aPortalIds[8]);
...
bool PhysTestConfigSpace2d::addPermanentPolygons(PhysConfigSpace2d* pSpace)
{
    ...
    MexPoint2d aPoints[] = {
        MexPoint2d(0, 0), // 0
        MexPoint2d(200, 0),     MexPoint2d(300, 0),     MexPoint2d(1900, 0),    MexPoint2d(2800, 0),
        MexPoint2d(300, 100), // 5
        MexPoint2d(1900, 100),  MexPoint2d(200, 200), // 7
        MexPoint2d(2000, 200),  MexPoint2d(2200, 200),  MexPoint2d(2700, 200),  MexPoint2d(2800, 200),
        MexPoint2d(0, 600), // 12
        MexPoint2d(200, 600),   MexPoint2d(300, 700), // 14
        MexPoint2d(700, 700),   MexPoint2d(1400, 700),  MexPoint2d(800, 800), // 17
        MexPoint2d(1500, 800),  MexPoint2d(2000, 800),  MexPoint2d(2200, 800),  MexPoint2d(300, 900), // 21
        MexPoint2d(700, 900),   MexPoint2d(1300, 900),  MexPoint2d(0, 1000), // 24
        MexPoint2d(200, 1000),  MexPoint2d(700, 1600), // 26
        MexPoint2d(1100, 1600), MexPoint2d(1300, 1600), MexPoint2d(1500, 1600), MexPoint2d(600, 1700), // 30
        MexPoint2d(700, 1800), // 31
        MexPoint2d(1100, 1800), MexPoint2d(1300, 2000), // 33
        MexPoint2d(0, 2500), // 34
        MexPoint2d(200, 2500),  MexPoint2d(400, 2700), // 36
        MexPoint2d(1300, 2700), MexPoint2d(1500, 2700), MexPoint2d(2700, 2700), MexPoint2d(2800, 2700),
        MexPoint2d(0, 2800), // 41
        MexPoint2d(400, 2800),  MexPoint2d(2800, 2800), MexPoint2d(800, 800),   MexPoint2d(1000, 800), // 45
        MexPoint2d(1000, 900),  MexPoint2d(800, 900),
    };

    size_t aPolygonVertices[] = { 0,  4,  5,  999, // To be deleted
                                  0,  1,  13, 12,  999, // H1
                                  1,  2,  5,  7,   999, // H2
                                  2,  3,  6,  5,   999, // H3
                                  3,  4,  11, 8,   6,   999, // H4
...
bool PhysTestConfigSpace2d::addTemporaryPolygons(PhysConfigSpace2d* pSpace)
{
    ...
    MexPoint2d aPoints[]
        = { MexPoint2d(900, 1100), // T1
            MexPoint2d(1100, 1100), MexPoint2d(1100, 1300), MexPoint2d(900, 1300),  MexPoint2d(2000, 1200), // T2
            MexPoint2d(2400, 1200), MexPoint2d(2400, 1400), MexPoint2d(2000, 1400), MexPoint2d(1600, 1600), // T3
            MexPoint2d(1800, 1600), MexPoint2d(1800, 1700), MexPoint2d(1600, 1700), MexPoint2d(1900, 2000), // T4
            MexPoint2d(2300, 1700), MexPoint2d(2000, 2200), MexPoint2d(2400, 2400), // T5
            MexPoint2d(2200, 2400), MexPoint2d(2200, 2100), MexPoint2d(2400, 2100) };

    size_t aPolygonVertices[] = { 0,   1,  2,  3,   999, // T16
                                  4,   5,  6,  7,   999, // T17
                                  8,   9,  10, 11,  999, // T18
                                  12,  13, 14, 999, // T19
                                  15,  16, 17, 18,  999, // T20
...
bool PhysTestConfigSpace2d::testContainsPoint(PhysConfigSpace2d* pSpace)
{
    ...
    if (okay)
        okay = not pSpace->contains(MexPoint2d(250, 125), flags, &polygonId, PhysConfigSpace2d::USE_ALL)
            and polygonId == aPolygonIds[2];

    if (okay)
        okay = pSpace->contains(MexPoint2d(250, 175), flags, &polygonId, PhysConfigSpace2d::USE_ALL);

    if (okay)
        okay = not pSpace->contains(MexPoint2d(2200, 1300), flags, &polygonId, PhysConfigSpace2d::USE_TEMPORARY_ONLY)
    ...
    if (okay)
        okay = pSpace->contains(MexPoint2d(2200, 1300), flags, &polygonId, PhysConfigSpace2d::USE_PERMANENT_ONLY);
...
bool PhysTestConfigSpace2d::testContainsLine(PhysConfigSpace2d* pSpace)
{
    ...
    if (okay)
        okay = pSpace->contains(
            MexPoint3d(400, 2600, 10),
            MexPoint3d(1100, 1900, 10),
            flags,
            &polygonId,
            PhysConfigSpace2d::USE_ALL);

    if (okay)
        okay = not pSpace->contains(
                   MexPoint3d(1200, 800, 10),
                   MexPoint3d(1700, 700, 10),
                   flags,
                   &polygonId,
    ...
    if (okay)
        okay = pSpace->contains(
            MexPoint3d(1200, 800, 1000),
            MexPoint3d(1700, 700, 1000),
            flags,
            &polygonId,
            PhysConfigSpace2d::USE_PERMANENT_ONLY);
    ...
    if (okay)
        okay = not pSpace->contains(
                   MexPoint3d(1300, 700, 10),
                   MexPoint3d(1700, 1650, 10),
                   flags,
                   &polygonId,
                   PhysConfigSpace2d::USE_TEMPORARY_ONLY)
            // and polygonId == aPolygonIds[18];
...
bool PhysTestConfigSpace2d::testContainsPolygon(PhysConfigSpace2d* pSpace)
{
    ...
    MexPoint2d aPoints[]
        = { MexPoint2d(300, 1900), // 1
            MexPoint2d(1100, 1900), MexPoint2d(1200, 2000), MexPoint2d(1200, 2600), MexPoint2d(400, 2600),
            MexPoint2d(300, 2500),  MexPoint2d(1800, 2000), // 2
            MexPoint2d(1800, 1900), MexPoint2d(2000, 1900), MexPoint2d(2000, 2000), MexPoint2d(1399, 799), // 3
            MexPoint2d(1401, 799),  MexPoint2d(1401, 801),  MexPoint2d(1399, 801) };

    size_t aPolygonVertices[] = { 0,   1,  2,  3,  4, 5,
                                  999, // 1
                                  6,   7,  8,  9,
                                  999, // 2
                                  10,  11, 12, 13,
    ...
    // Perform the tests
    if (okay)
        okay = pSpace->contains(*(polygons[0]), flags, &polygonId, PhysConfigSpace2d::USE_ALL);

    if (okay)
        okay = pSpace->contains(*(polygons[1]), flags, &polygonId, PhysConfigSpace2d::USE_PERMANENT_ONLY);

    if (okay)
        okay = not pSpace->contains(*(polygons[1]), flags, &polygonId, PhysConfigSpace2d::USE_TEMPORARY_ONLY)
            // and polygonId == aPolygonIds[19];
            and polygonId == aPolygonIds[20];

    if (okay)
        okay = pSpace->contains(*(polygons[2]), flags, &polygonId, PhysConfigSpace2d::USE_TEMPORARY_ONLY);

    if (okay)
        okay = not pSpace->contains(*(polygons[2]), flags, &polygonId, PhysConfigSpace2d::USE_PERMANENT_ONLY)
            and polygonId == aPolygonIds[9];
...
bool PhysTestConfigSpace2d::testFindPaths(PhysConfigSpace2d* pSpace, ostream& out)
{
    ...
    if (okay)
    {
        ...
        okay = testFindPath(
            out,
            pSpace,
            MexPoint2d(2000, 2400),
            MexPoint2d(2400, 1900),
            50.0,
            5,
            xRight,
            yRight,
    ...
    if (okay)
    {
        ...
        okay = testFindPath(
            out,
            pSpace,
            MexPoint2d(1550, 1515),
            MexPoint2d(2300, 2000),
            50.0,
            5,
            xRight,
            yRight,
    ...
    MATHEX_SCALAR maxError[])
{
    ...
    PhysConfigSpace2d::FindPathId findPathId
        = pSpace->addFindPath(startPoint, endPoint, clearance, flags, Phys::defaultPathFindingPriority());
    ...
    okay = pSpace->findPath(findPathId, &path) and path.size() == nAnswerPoints;
    ...
    if (okay)
    {
        ...
        {
            okay = fabs(path[i].x() - xRight[i]) <= maxError[i] and fabs(path[i].y() - yRight[i]) <= maxError[i];
        }
    }
...
bool PhysTestConfigSpace2d::testDomainFindPaths(PhysConfigSpace2d* pSpace, ostream& out)
{
    ...
    if (okay)
    {
        PhysConfigSpace2d::PortalId portalIdRight[] = { aPortalIds[9], aPortalIds[7], aPortalIds[3], aPortalIds[1] };
        //        MATHEX_SCALAR distanceRight[] = {300,600,100,500};
        MATHEX_SCALAR distanceRight[] = { 392, 692, 8, 592 };

        okay = testDomainFindPath(
            out,
            pSpace,
            MexPoint2d(600, 2000),
            MexPoint2d(400, 600),
    ...
    if (okay)
    {
        PhysConfigSpace2d::PortalId portalIdRight[] = { aPortalIds[4], aPortalIds[8], aPortalIds[5] };
    ...
    if (okay)
    {
        PhysConfigSpace2d::PortalId portalIdRight[] = { aPortalIds[7], aPortalIds[6], aPortalIds[8] };
    ...
    MATHEX_SCALAR distanceRight[])
{
    ...
    PhysConfigSpace2d::DomainFindPathId domainFindPathId
        = pSpace->addDomainFindPath(startPoint, endPoint, clearance, flags, Phys::defaultPathFindingPriority());
    ...
    okay = pSpace->domainFindPath(domainFindPathId, &portalPoints) and portalPoints.size() == nAnswerPoints;
    ...
    if (okay)
    {
        ...
        for (size_t i = 0; okay and i != nAnswerPoints; ++i)
        {
            okay = portalPoints[i].first == portalIdRight[i] and fabs(portalPoints[i].second - distanceRight[i]) < 1.0;
        }
    }
    ...
    if (not okay and portalPoints.size() != 0)
    {
        ...
        {
            out << "  " << portalPoints[i].first.asScalar() << "  " << portalPoints[i].second << std::endl;
        }
        out << "End found path" << std::endl;
        out << "Start expected path" << std::endl;
        for (size_t i = 0; i != portalPoints.size(); ++i)
        {
            out << "  " << portalIdRight[i].asScalar() << "  " << distanceRight[i] << std::endl;
        }
        out << "End expected path" << std::endl;
    }
...
bool PhysTestConfigSpace2d::oneTest(ostream& out)
{
    ...
    bool okay
        = testFindPath(out, &space, MexPoint2d(643.007, 227), MexPoint2d(829, 233), 3.0, 3, xRight, yRight, maxError);
...
bool PhysTestConfigSpace2d::testIntersects(ostream&)
{
    ...
    MexPoint2d endPoint(startPoint.x(), startPoint.y() + motionProfile.distance(motionProfile.totalTime()));
    ...
    if (okay)
    {
        ...
        MexPoint2d endPoint2(startPoint.x() + (endPoint.y() - startPoint.y()) / 2, (endPoint.y() + startPoint.y()) / 2);
    ...
    if (okay)
    {
        ...
        MexPoint2d endPoint2(startPoint2.x(), startPoint2.y() + endPoint.y() - startPoint.y());
        ...
        MexPoint2d endPoint3(startPoint.x() + (endPoint.y() - startPoint.y()) / 2, (endPoint.y() + startPoint.y()) / 2);
