        ...
        case INLOOP:
            {
                ...
                bool f12Hit = ! f12Down && DevKeyboard::instance().keyCode(DevKey::F12);
                if (f12Hit)
                    f12Down = true;
                else
                    f12Down = false;
                ...
                if (f12Hit)
                {
                    ...
                    MEMORY_STREAM(
                        "Current newed value " << DbgMemChkAllocationData().currentMemoryNewed() << std::endl);
#else
                    // Write marker to stream
                    MEMORY_STREAM("================================================================" << std::endl);
                    MEMORY_STREAM("Marker set to " << (++logValue) << std::endl);
        ...
        case SAMPLE:
            {
                if (++sampleFrame > rate)
                {
                    ...
                    GlobalMemoryStatus( &status );
                    MEMORY_STREAM("=====================================================" << std::endl );
                    MEMORY_STREAM("Sample Frame Number " << sampleFrameNumber << " at sample rate " << rate << " sim
                    time " << SimManager::instance().currentTime() << std::endl ); MEMORY_STREAM(" System data:" <<
                    std::endl); MEMORY_STREAM("  memory load    " << status.dwMemoryLoad << std::endl ); MEMORY_STREAM("
                    total physical " << status.dwTotalPhys << std::endl); MEMORY_STREAM("  avail physical " <<
                    status.dwAvailPhys << std::endl); DWORD dwUsedPhys = status.dwTotalPhys - status.dwAvailPhys;
                    MEMORY_STREAM("  total virtual  " << status.dwTotalVirtual << std::endl);
                    MEMORY_STREAM("  avail virtual  " << status.dwAvailVirtual << std::endl);
